...
The length-dependent optimal temperature
\begin{equation}
\tau^*(L) = \frac{d_{\mathrm{head}}}{\sqrt{L}}
\end{equation}
is supported by 99 training runs across 27 configurations (Appendix~\ref{sec:tau-scaling}, Figure~\ref{fig:tau-sweep}). The broadband surrogate provides the $d_{\mathrm{head}}$ dependence ($\alpha \approx 1/d_{\mathrm{head}}$, so $\sqrt{\beta/\alpha} \propto \sqrt{d_{\mathrm{head}}}$) but only $L^{-0.11}$, too weak by a factor of~5. The missing $L$-dependence originates from the softmax attention map.
...
The $\chi^2$ stiffness reflects attention-load asymmetry: diluted channels ($\rho < 1$) bear per-channel cost ${\propto}1/\rho$. KL, Jeffreys, and all R\'{e}nyi divergences produce $L$-exponents far from $-0.5$; only the $\chi^2$ family succeeds (Appendix~\ref{sec:stiffness-derivation}, Table~\ref{tab:stiffness-sweep}). When $L$ is short, $\tau^*(L)$ is large and EVQ departs visibly from geometric (the PE-dominant regime); as $L$ grows, $\tau^*$ shrinks and EVQ converges back, predicting bounded in-range cost with progressively narrower long-range correction.
