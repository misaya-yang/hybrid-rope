...
\end{equation}
as an empirical law supported by multi-regime experiments (99 runs across 27 configurations; Appendix~\ref{sec:tau-scaling}, Figure~\ref{fig:tau-sweep}), not as an unconditional theorem. The broadband surrogate partially motivates this formula: numerical analysis of the exact kernel's discrete surrogate coefficients (Appendix~\ref{sec:tau-scaling}) shows $\alpha \approx 1/d_{\mathrm{head}}$ and $\beta = O(1)$, yielding $\sqrt{\beta/\alpha} \propto \sqrt{d_{\mathrm{head}}}$---the correct dependence on $d_{\mathrm{head}}$. The $L^{-1/2}$ exponent is a finite-channel correction that the continuous variational theory does not resolve, arising from oscillatory inter-channel correlations in the exact kernel that penalize over-concentration at low frequencies.
