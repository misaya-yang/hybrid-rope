...
**Phase 22 QA-mix (reassessed — limited value)**: Originally planned to mix QA into pretraining. However, QA-mix answer tokens (~3 tokens "Answer: (X)") contribute negligible gradient; 95%+ of QA chunk loss is article reading (standard LM). The model won't "learn to do QA" from this — options\_nll accuracy comes from language modeling quality (reading comprehension), not from having seen QA format. Since EVQ's advantage IS in language modeling quality at long contexts (PPL −52%, Gold NLL −30%), the correct eval strategy is to directly test options\_nll on pretrained checkpoints without task-specific training, which is exactly what Gold NLL already measures. **Phase 22 is deprioritized — the Gold NLL evidence line already captures the signal that QA-mix was designed to surface.**
