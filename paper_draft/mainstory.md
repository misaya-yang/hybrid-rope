...
> **最后更新**：2026-03-09（Phase16 τ 公式全面 sweep 并入核心叙事）
...
1. **Closed-form near-optimality**
RoPE frequency allocation 是一个变分逆问题；其解满足二阶 ODE，并得到闭式 `EVQ-cosh`。Geometric 是 `τ→0` 的零温极限，而非天然最优设计；`tau*=d_head/sqrt(L)` 在 99-run sweep 中稳定落在近最优带内。
...
### Phase 16 全面 sweep：从“几个点对”升级到“系统近似最优”

我们在 `local_wikitext` 上做了一个专门验证 `τ` 公式的 sweep：

- `L ∈ {256, 512, 1024}`
- `H ∈ {4, 8, 16}`，因此 `d_head ∈ {128, 64, 32}`
- `45` pilot + `54` confirm = **`99` runs**
- `3` 个 seed

结果不是“处处精确命中”，但 pattern 非常稳定：

- theory `tau` 精确最优：`3/9`
- theory `tau` 前二：`6/9`
- theory `tau` 前三：`8/9`
- 所有经验最优值都在 theory 的 `1.5x` 以内
- 最优值平均约为 theory 的 `1.20x`
...
- “公式给出一个稳健 near-optimal 中心线；真正最优值只是在它右侧做小幅修正”

因此正文最稳的 claim 不是 “exact law”，而是：

> `tau*=d_head/sqrt(L)` is a robust near-optimal scaling law with a mild finite-capacity right-shift.
