...
> **最后更新**：2026-03-09c（v11 三重挤压叙事重构 + 四层主故事 + opening 重写 + MLA/YaRN/progressive 三协同框架）
...
- **与 YaRN 超线性协同**：`EVQ+YaRN >> Geo+YaRN`（-86%），说明 EVQ 是解锁 inference-time scaling 上限的训练底座
- **与 progressive training 兼容**：工业标准的 short→long 训练课程让 EVQ 优势放大（PPL@16K -45.9%），而非被"洗掉"
...
3. **EVQ unlocks YaRN（超线性协同）**
当前最强系统级结论不是 `EVQ > Geo`，而是 **`EVQ + YaRN >> Geo + YaRN`**，且是**超线性**的：
   - `retrieval@8K`: `100%` vs `61%-65%`（6-seed zero variance）
   - `PPL@8K`: `70.9 vs 82.9`，`PPL@16K`: `107.5 vs 157.7`
   - 固定短窗口训练 454M：EVQ+YaRN vs Geo+YaRN **-86%** across 4K-32K

   这说明 allocation 是 inference-time scaling 的**训练时前提**——底座频率分配不对，推理时再怎么补都有天花板。

4. **与工业训练 pipeline 天然兼容**
Progressive short→long 训练（SkyLadder, LLaMA）是工业标准。Phase 15 (750M 2K→4K) 证明 EVQ 在此 regime 下优势**放大**（PPL@16K -45.9%，AR exact 0%→77.5%）。EVQ 不需要特殊 training recipe，天然嵌入现有 pipeline。
...
- **训练课程主锤**：Phase 15 progressive (2K→4K) `PPL@16K -45.9%`，Phase 17 same-length+YaRN `-86%`——两种现实 regime 下 EVQ 都赢
...
>
> **We show this default is a degenerate point (τ=0) of a variational optimum. Our closed-form solution, EVQ-Cosh, requires zero extra parameters and composes supralinearly with YaRN: EVQ+YaRN reduces perplexity by 86% over Geo+YaRN across 4K-32K. Progressive training (the industry-standard short→long curriculum) further amplifies EVQ's advantage to -45.9% PPL at 16K. EVQ is the missing piece in the RoPE optimization chain — orthogonal to base tuning, synergistic with YaRN inference-time scaling, and increasingly critical as MLA compresses the frequency budget.**
...
**Modern LLMs face a triple squeeze on RoPE (L↑, base↑, d_head↓ via MLA). EVQ is the first closed-form solution for the missing dimension — within-band frequency allocation — and composes supralinearly with YaRN and progressive training.**
...
3. **系统级杀手锏（三个协同）**：
   - **× YaRN**：超线性协同，`EVQ+YaRN >> Geo+YaRN` (-86%)。Allocation 是 inference-time scaling 的训练时前提。
   - **× Progressive training**：工业标准 short→long 课程让 EVQ 优势放大 (-45.9%@16K)，不是被洗掉。
...
8. **🔴 EVQ+YaRN 超线性协同**：这是当前最值钱的系统级结论，必须成为主图中心。
9. **训练课程 ablation**：同长度训练 raw 退化是 expected（同 SkyLadder）；但 (a) EVQ+YaRN 仍 -86%，(b) progressive short→long 让 EVQ 优势放大 (-45.9%@16K)。**EVQ wins in both realistic regimes.**
...
| "更多训练会洗掉 EVQ" | **Phase 15+17 training curriculum ablation 直接回答**：(a) 同长度继续训练 → raw 退化是 expected（同 SkyLadder），但 EVQ+YaRN 仍 -86%；(b) progressive short→long → EVQ 优势放大 -45.9%@16K。两种 regime 都赢 |
