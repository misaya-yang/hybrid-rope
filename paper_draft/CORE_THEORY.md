...
| **256** | **4.0** | **4.0 > 2.0 ✅** | **🆕 Phase 11: 454M 3-seed，τ=4.0 在所有 OOD 长度全面优于 τ=2.0** |
| 512 | 2.83 | 4.0 | 偏高 41% |
| 1024 | 2.0 | 2.0 | 精确匹配 |
| 2048 | 1.41 | 1.5 | 偏差 6% |
...
| 指标 | Geo | EVQ r=0 | Δ |
|------|-----|---------|---|
| 8K retrieval / AR | `100% / 0%` | `100% / 77.5%` | `AR +77.5pp` |
| Global retrieval / AR | `100% / 66.67%` | `100% / 92.5%` | `AR +25.8pp` |

**结论**：
- continue 到 `4K` 后，EVQ 在 `2K/4K` 仍只付出很小代价，但在真正长程段继续大幅获益
- 这条设置里 retrieval 已经 ceiling 到 `100%`，因此**最有区分度的指标是 `16K PPL` 和 `AR exact`**
- 当前仍是 **single-seed supporting evidence**；另一个 caveat 是 EVQ downstream NLL 因 LongBench 远端下载失败而未形成公平对比
...
### 11.7B Figure 2 EVQ × YaRN 主图描述

**文件**：`paper_draft/figs/fig2_evq_yarn_synergy.png` / `paper_draft/figs/fig2_evq_yarn_synergy.pdf`

| Panel | 内容 | 核心发现 |
|-------|------|----------|
| (a) | 10% mix, fair scale=8 下的 8K retrieval（带 3-seed dots） | EVQ+YaRN = 100%，Geo+YaRN = 61%，直接显示正交协同 |
| (b) | retrieval vs extrapolation length | EVQ+YaRN 在 4K-16K 全段占优，8K 达到 100% |
| (c) | PPL vs extrapolation length | EVQ+YaRN 在 8K/16K 分别为 70.9/107.5，显著低于 Geo+YaRN 的 82.9/157.7 |
...
> **数据源**: `docs/exp/2026-03-04_phase11_L256_results.md`, `scripts/core_text_phases/phase11_yarn_eval.py`
...
**注**：早期 seed=42 的 `4K: 42%→82% (+40pp)` 是单 seed 极值，不应继续作为论文主 headline。论文正文应使用 3-seed mean；更强、也更稳健的 retrieval headline 是 `EVQ+YaRN@8K = 100% across 6/6 seeds`。
...
1. **2K 都是 100%**：10% mix 让两种方法都学会了检索。差异主要来自外推能力
...
3. **Scaling law**："τ*(L) = d_head/√L, validated across 5 context lengths + Phase 11 L=256 direct confirmation (τ=4.0 > τ=2.0, 454M 3-seed)"
...
7. **Geo 动态退化（支持性证据）**："In the single-seed 750M checkpoint trajectory, Geometric passkey@8K regresses after 50%, while Hybrid continues improving. This is supporting dynamics evidence rather than a primary multi-seed claim."
8. **Retrieval divergence（支持性证据）**："PPL waterbed can coexist with divergent retrieval trajectories; this is clearly visible in the 750M checkpoint trajectory, but should be framed as illustrative because the run is single-seed and uses Hybrid."
9. **Capability-preserving**："Waterbed cost is frequency-local, doesn't leak to capability space"
10. **Passkey mix（multi-seed）**："In 10% passkey mix, raw EVQ improves retrieval by +10.0pp@4K and +12.7pp@8K over Geo, while EVQ+YaRN reaches 100%@8K across 6/6 seeds."
11. **5%→10% robustness gap**："Denser passkey supervision hurts Geo much more than EVQ (8K: -13.3pp vs -3.3pp), indicating that frequency allocation quality, not data quantity alone, governs length generalization."
12. **🆕🔴 EVQ+YaRN 超线性协同（冲击 Spotlight 核心武器）**："Training-time frequency optimization (EVQ) × inference-time position scaling (YaRN) = orthogonal optimization. EVQ+YaRN@8K: 100% (6/6 seeds, zero variance). Geo+YaRN@8K: 61%-65%. In the 10% fair scale=8 comparison, PPL@8K is 70.9 vs 82.9. EVQ unlocks the training-time bottleneck for long-context extrapolation."
...
15. **🆕🔴 Video temporal 维度泛化（3D RoPE）**："EVQ-cosh 的频率分配不仅适用于 1D text RoPE，也可迁移到 3D Video RoPE 的 temporal 维度。固定 spatial geometric、仅替换 temporal `inv_freq_t` 的 2-seed 控制实验中，EVQ 在 raw temporal extrapolation 上于大部分 OOD 长度优于 Geo，而 temporal-YaRN 在 EVQ 上被显著放大；当前最佳观测配置是 `τ=4 + temporal-YaRN`。该结果支持跨模态迁移，但暂不足以把视频单独写成 `τ*=2.0` 的直接确认。"

**论文 Figure 规划**：

| Figure | 内容 | 文件 | 状态 |
|--------|------|------|------|
| Figure 1（supporting） | 三联图：(a) 频率分配 (b) PPL 训练动态 (c) Passkey 训练动态 | `paper_draft/figs/fig1_frequency_dynamics.pdf` | ✅ 已完成 |
| **Figure 2（主图）** | EVQ × YaRN：8K retrieval bars + retrieval length curves + PPL length curves | `paper_draft/figs/fig2_evq_yarn_synergy.pdf` | ✅ 已完成 |
| **Figure 3（主图）** | PE-dominant regime / scaling law direct validation：128-token extreme extrapolation + Phase 11 raw/YaRN 双确认 | `paper_draft/figs/fig3_pe_dominant_scaling.pdf` | ✅ |
| Figure 4（待做） | 碰撞块占比 vs base + dead zone | 待生成 | ⏳ |

**理论-叙事-图表矩阵**：见 `paper_draft/figs/README.md`。其中已拆分为“理论严谨矩阵”和“叙事-图表矩阵”两层：前者约束 theorem / proposition / empirical law / appendix caution 的身份，后者负责把理论核心、叙事核心与正文图表资产对齐。
...
- ✅ τ* scaling law 多规模验证
- ✅ **Passkey mix（3-seed）**：raw retrieval +10.0pp@4K、+12.7pp@8K；EVQ+YaRN@8K 在 6/6 seeds 上达到 100%
- ✅ Capability-preserving：未训练 retrieval 任务上无破坏性证据；已训练 retrieval 任务上存在稳定 mean-level 增益
- ✅ **750M checkpoint trajectory（single-seed supporting evidence）**：Geo passkey@8K 70%→60%（regression），Hybrid 65%→80%（单调递增）
- ✅ **750M continue @4K（single-seed supporting evidence）**：Full EVQ `PPL@16K 24.4 vs Geo 45.1` (`-45.9%`)，且 `8K AR exact 77.5% vs 0%`
- ✅ **Hybrid@50% > Geo@100%（single-seed, r=16 Hybrid）**：PPL@8K -9.3%, PPL@16K -10.9%
- ✅ **AR exact match 近乎翻倍**：30% vs 16.25%（+13.75pp）
- ✅ **5%→10% robustness gap（3-seed）**：Geo 8K -13.3pp，EVQ 仅 -3.3pp
- ✅ **PI inference-time baseline 崩溃**：PPL@2K=191.7（几乎随机）
- ✅ **检索距离重分布**：40-trial 全局 retrieval 相同 81.87%，但 EVQ 向 L=8K 推移 +12.5pp
- ✅ **🔴 EVQ+YaRN 超线性协同（6-seed）**：EVQ+YaRN@8K 100%（zero variance）vs Geo+YaRN 61%-65%。在 10% 的 fair scale=8 比较中，PPL@8K 70.9 vs 82.9，PPL@16K 107.5 vs 157.7。训练时频率优化 × 推理时位置缩放 = 正交优化
...
- 原先 `5%→10%` 的“反对称 scaling”叙事经 multi-seed 校准后不成立；更严谨的表述是 Geo 退化更强、EVQ 更抗退化
...
| 750M Retrieval Divergence +20pp | 实验 B+ | 750M | 1 | ✅ (single-seed supporting evidence) |
| **750M continue @4K: PPL@16K -45.9%, 8K AR exact +77.5pp** | **实验 B+** | **750M** | **1** | **✅ supporting evidence** |
| Passkey mix raw retrieval +10.0pp@4K / +12.7pp@8K | 实验 A | 350M | 3 | ✅ |
| EVQ+YaRN 8K=100% (L=2048) | 实验 A+ | 350M | 6 | ✅ zero variance |
| 5%→10% robustness gap（Geo -13.3pp vs EVQ -3.3pp @8K） | 实验 A | 350M | 3 | ✅ |
...
- **已有 750M+ Pure EVQ (r=0) continue@4K 单 seed 数据**，显示 `16K PPL -45.9%` 与 `8K AR exact 77.5% vs 0%`
...
| PAPER_PLAN_V9.md | ✅ 结构完成，需更新 Phase 11 | `paper_draft/` |
| Paper figures（Fig 1 / Fig 2） | ✅ 已集中到 `figs/` | `paper_draft/figs/` |
| LaTeX 正文 | ❌ 未开始 | — |
| Appendix 证明 | ❌ 未开始 | — |
...
2. **核心实验结果**：L=2048 PPL -13%(3-seed), passkey raw +10.0pp@4K / +12.7pp@8K (3-seed), EVQ+YaRN 100%(6-seed), L=256 PPL -38%(3-seed), EVQ+YaRN -62%
3. **需要帮忙的**：(a) Broadband 近似的 perturbation bound, (b) τ* 推导打磨
4. **不需要帮忙的**：六步推导链本身（5/6 步精确，远超 PE 论文惯例）
5. **接下来要做的**：DSR 实验, ≥1B Pure EVQ 或 750M continue 多 seed/补齐 downstream, 论文 LaTeX 写作
