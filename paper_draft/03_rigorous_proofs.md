...
1. **The $\gamma$ Phase Transition Threshold:** Define $p \in [0, 1]$ as the mixing proportion between structurally dense short-range parsing (e.g., code tasks, $\mathcal{D} \sim \Delta^{-2}, \gamma=2$) versus uniform random long-range padding ($\mathcal{D} \sim \text{const}, \gamma \to 0$). The effective prior maps as $\gamma_{\text{eff}}(p) \approx 2p$. We mathematically predict a critical phase transition near $p^* \approx 0.5$. For $p > 0.5$, the steep continuous convex density prevails. For $p < 0.5$, the prior flattens, abruptly inverting topological superiority back to the flat $\rho=1$ topology of Standard RoPE.
