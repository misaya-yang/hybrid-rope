...
\begin{center}
{\LARGE\bfseries EVQ-Cosh \zh{理论开放问题清单}}\\[4pt]
{\normalsize 2026-03-31}
\end{center}
...
\subsection*{\zh{当前进展}}
\zh{基于} Phase~16 \zh{的} 99 \zh{组实验（}$L \in \{256,512,1024\}$\zh{，}$d_{\mathrm{head}} \in \{32,64,128\}$\zh{），抛物线拟合得到} $\lambda \approx 1.17 \pm 0.13$ (CV$=$11.1\%)\zh{，初步表明} $\lambda$ \zh{近似为常数。对数线性回归给出} $\tau^* \approx 0.967 \cdot d_{\mathrm{head}}^{0.944} \cdot L^{-0.439}$\zh{，指数偏差约} 6\%\zh{。}

\subsection*{\zh{对论文的意义}}
$\lambda$ \zh{的近似普适性已得到初步验证，可将} $\tau^*$ \zh{公式定位为「经验校准的半解析公式」。建议在附录中增加} $\lambda$ \zh{的交叉验证表。完全闭合的解析推导仍为开放问题。}
...
\begin{equation}
  ...
  \label{eq:surrogate}
\end{equation}
\zh{来近似真实的振荡} RoPE \zh{核} $K(\Delta) = \sum_k \cos(\theta_k \Delta)$\zh{。代理在} 12 \zh{种配置上功能验证通过}(Appendix~B)\zh{，但逐点保真度未被声称。}

\zh{已验证：连续密度泛函} $J[\rho]$ \zh{在小} $\tau$ \zh{时确实减少碰撞（}$\tau < 1.4$\zh{时} $\Delta J < 0$\zh{）。离散通道碰撞的增加是密度集中项} $T_1$ \zh{主导的结果，不构成与理论的矛盾。论文的「碰撞}$+$\zh{Fisher 效用权衡」框架在泛函层面是正确的。}
...
\subsection*{\zh{当前状态}}
\zh{当前所有实验的训练长度均满足} $L \leq 2048$\zh{。缩放律} $\tau^* = d_{\mathrm{head}}/\sqrt{L}$ \zh{在} $L \in \{256, 512, 1024\}$ \zh{范围内验证良好（指数偏差约} 6\%\zh{），但在} $L \geq 4096$ \zh{时：}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \end{equation}
  \zh{二阶修正项始终为负（将频率向低端压缩），其幅度由} $u_k(1-u_k)(2-u_k)$ \zh{决定。分析该扰动对注意力传输效用的贡献，判断在小} $\tau$ \zh{时是否仍有意义。}
