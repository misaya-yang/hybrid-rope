...
def main():
    ...
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':['Dense','RoPEMean','SupportRepair','WrongRepair'],
          ...
          'cost_boundary':'Same m16 remote budget; support replaces lowest-scoring unprotected blocks, wrong control matches count and distance bin; diagnostic only'}
            ...
            for row in rows:
                ...
                for method in meta['methods']:
                    ...
                    record={**row,'prompt_ids':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'generated_ids':generated,'output_text':answer,'text_exact':answer.strip()==row['expected'],'whole_numeric_list_exact':bool(re.fullmatch(r'\s*\d+(?:\s*,\s*\d+)*\s*',answer)) and re.findall(r'\d+',answer)==row['references'],
