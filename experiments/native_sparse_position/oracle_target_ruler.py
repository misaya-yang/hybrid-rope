...
from oracle_target_native import Oracle
try:
...
def main():
    ...
    p.add_argument('--methods',nargs='+',choices=['Dense','RoPEOracle','RoPEMax'],default=['RoPEOracle','RoPEMax'])
    ...
    with torch.inference_mode():
        ...
        qual_outputs={}
        for method in ('Dense','RoPEOracle','RoPEMax'):
        ...
        parity={m:bool(torch.equal(qual_outputs[m],qual_outputs['Dense'])) for m in ('RoPEOracle','RoPEMax')}
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('oracle_target_native.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':a.methods,
          ...
          'cost_boundary':'Fixed m16 remote budget; exact full-key max/logmass ranking diagnostic, not deployable cheap selector; native reader and frozen prefix unchanged'}
            ...
            for row in rows:
                ...
                for method in meta['methods']:
                    ...
                    record={**row,'prompt_ids':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'prefix_seconds':prefix_seconds,'selector_calls':oracle.calls,'selector_full_key_scan':True}
