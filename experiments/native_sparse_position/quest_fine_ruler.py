...
from quest_fine_native import Oracle,build
try:
...
def main():
    ...
    p.add_argument('--methods',nargs='+',choices=['Dense','QuestFine32'],default=['QuestFine32'])
    p.add_argument('--row-id')
    a=p.parse_args()
    ...
    with torch.inference_mode():
        ...
        qual_outputs={}
        for method in ('Dense','QuestFine32'):
        ...
        parity={m:bool(torch.equal(qual_outputs[m],qual_outputs['Dense'])) for m in ('QuestFine32',)}
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('quest_fine_native.py'),Path(__file__).with_name('pair_envelope.py'),Path(__file__).with_name('mean_native.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':a.methods,
          'all_key_gather_bitwise_logit_parity':parity,'block':32,'sink':64,'local':2048,'remote_topk_per_query_head':32,'max_answer_tokens':128,'model':str(a.model),'model_type':config.model_type,'sparse_layer_indices':sorted(oracle.layers),'input_sha256':hashlib.sha256(a.inputs.read_bytes()).hexdigest(),'code_sha256':frozen_code,
          'boundary':'All original full-attention layers sparse from first question token; native RoPE reader unchanged',
          'cost_boundary':'Strong frontier baseline: literal unpadded Quest min/max FP32 score, actual Quest32 retrieval pages, top32 pages=1024 remote tokens, sink64+local2048 unchanged. Lossless BF16 min/max costs4D BF16 numbers per 64 token positions, versus mixtures4D+4 FP32 numbers; FP32 score roundtrip parity checked per layer and prefix. Retrieval granularity differs from fixed-B64 causal attribution'}
        ...
        with torch.inference_mode(),open(a.output/'outputs.jsonl','x') as stream:
            for row in rows:
                ...
                if current_group!=group:
                    ...
                    prefix_seconds=time.monotonic()-tick;tick=time.monotonic();build(oracle);torch.cuda.synchronize();summary_build_seconds=time.monotonic()-tick;(a.output/(row['row_id']+'_build.json')).write_text(json.dumps(oracle.build_info,indent=2));base_ids=prefix.clone();current_group=group
                ...
                for method in meta['methods']:
                    ...
                    record={**row,'prompt_ids':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'prefix_seconds':prefix_seconds,'selector_calls':oracle.calls,'all_methods_build_seconds':summary_build_seconds}
