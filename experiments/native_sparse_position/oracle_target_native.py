...
class Oracle:
    ...
    def interface(self,module,q,k,v,mask,**kw):
        ...
        qs=q[0,:,0].float() if self.mode in ('RoPEOracle','RoPEMax') else self.q[i][0,:,0].float()
        ks=k[0].float() if self.mode in ('RoPEOracle','RoPEMax') else self.raw[i][0,:,:N].float()
        ...
        if count:
            block_scores=score[:,:nb*self.block].reshape(H,nb,self.block)
            fm=block_scores.amax(-1) if self.mode=='RoPEMax' else block_scores.logsumexp(-1)
