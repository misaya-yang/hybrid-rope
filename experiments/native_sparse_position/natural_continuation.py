...
def main():
    ...
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':['Dense','RoPEOracle','NoPEOracle'],
          'all_key_gather_bitwise_logit_parity':parity,'block':64,'local':2048,'remote_topk_per_query_head':32,'max_answer_tokens':128,'model':str(a.model),'model_type':config.model_type,'sparse_layer_indices':sorted(oracle.layers),'input_sha256':hashlib.sha256(a.inputs.read_bytes()).hexdigest(),'code_sha256':frozen_code,
