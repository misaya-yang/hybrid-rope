...
def main():
    ...
    p.add_argument('--methods',nargs='+',choices=['Dense','RoPEMean','PostMetric4','PreMetric4','MatchedContiguous4','Quest','QuestSplit32'],default=['PostMetric4','PreMetric4','MatchedContiguous4'])
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('mixture_native.py'),Path(__file__).with_name('pair_envelope.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':a.methods,
          ...
          'cost_boundary':'Fixed m16 remote budget; cached FP32 positive 4-mean mixtures with group log-counts, actual-post-key centroid; post/pre metric and post-count-matched contiguous controls. Known clustering-family mechanism control, not claimed new algorithm'}
