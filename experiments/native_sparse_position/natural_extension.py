...
from oracle_native import Oracle
try:
...
def main():
    ...
    a=p.parse_args()
    previous_status=json.loads((a.parent/'status.json').read_text())
    if previous_status['status']!='COMPLETE' or previous_status['model']!=str(a.model):raise ValueError('Parent identity mismatch')
    if previous_status['input_sha256']!=hashlib.sha256(a.inputs.read_bytes()).hexdigest():raise ValueError('Parent inputs changed')
    old={(r['row_id'],r['method']):r for r in map(json.loads,(a.parent/'outputs.jsonl').read_text().splitlines())}
    if len(old)!=previous_status['outputs']:raise ValueError('Parent outputs incomplete')
    parent_sha=hashlib.sha256((a.parent/'outputs.jsonl').read_bytes()).hexdigest()
    ...
    assert rows and all(r['max_new_tokens']==128 for r in rows)
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('oracle_native.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':['Dense','RoPEOracle','NoPEOracle'],
          'all_key_gather_bitwise_logit_parity':parity,'block':64,'local':2048,'remote_topk_per_query_head':32,'max_answer_tokens':384,'parent_outputs_sha256':parent_sha,'extension_contract':'Reuse already terminated completions; replay only capped trajectories, verify all prior tokens, and extend to 384','model':str(a.model),'model_type':config.model_type,'sparse_layer_indices':sorted(oracle.layers),'input_sha256':hashlib.sha256(a.inputs.read_bytes()).hexdigest(),'code_sha256':frozen_code,
    ...
    try:
        with torch.inference_mode(),open(a.output/'outputs.jsonl','x') as stream:
            for row in rows:
                if all(old[(row['row_id'],m)]['ended_eos'] for m in meta['methods']):
                    for m in meta['methods']:
                        record={**old[(row['row_id'],m)],'reused_terminated_parent':True};records.append(record);stream.write(json.dumps(record)+'\n');stream.flush()
                    continue
                ...
                for method in meta['methods']:
                    parent_row=old[(row['row_id'],method)]
                    if parent_row['ended_eos']:
                        record={**parent_row,'reused_terminated_parent':True};records.append(record);stream.write(json.dumps(record)+'\n');stream.flush();continue
                    ...
                    if generated[:len(parent_row['generated_ids'])]!=parent_row['generated_ids']:raise ValueError('Extension replay token drift')
                    ...
                    record={**row,'prompt':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'generated_ids':generated,'extension_replay_prefix_equal':True,'reused_terminated_parent':False,'output_text':answer,'text_exact':any(answer.strip()==x for x in row['references']), 'normalized_exact':any(normalize_text(answer)==normalize_text(x) for x in row['references']), 'qa_f1':qa_f1_score(answer,row['references']),
