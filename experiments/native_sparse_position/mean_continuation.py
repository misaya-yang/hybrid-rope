"""Natural long QA: query-blind prefix, native reader, RoPE/NoPE block means.
...
from mean_native import Oracle
try:
...
def main():
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('mean_native.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':['RoPEMean','NoPEMean'],
          ...
          'cost_boundary':'Reference rebuilds FP32 block means each token; quality test, not runtime evidence; per-query-head diagnostic budget'}
