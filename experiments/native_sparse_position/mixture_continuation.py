"""Positive key-mixture construction, question-blind prefix, unchanged native reader.
...
from mixture_native import Oracle,build
try:
...
def main():
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),Path(__file__).with_name('mixture_native.py')]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':['RoPEMean','MatchedContiguous4','PostMetric4','PreMetric4'],
          'all_key_gather_bitwise_logit_parity':parity,'block':64,'local':2048,'remote_topk_per_query_head':16,'max_answer_tokens':384,'model':str(a.model),'model_type':config.model_type,'sparse_layer_indices':sorted(oracle.layers),'input_sha256':hashlib.sha256(a.inputs.read_bytes()).hexdigest(),'code_sha256':frozen_code,
          'boundary':'All original full-attention layers sparse from first question token; native RoPE reader unchanged',
          'cost_boundary':'Cached BF16 positive 4-mean mixtures, per-query-head m16 budget; Python reference, no speedup claim'}
        ...
        with torch.inference_mode(),open(a.output/'outputs.jsonl','x') as stream:
            for row in rows:
                ...
                if current_group!=group:
                    ...
                    prefix_seconds=time.monotonic()-tick;tick=time.monotonic();build(oracle);summary_build_seconds=time.monotonic()-tick;base_ids=prefix.clone();current_group=group
                ...
                for method in meta['methods']:
                    ...
                    record={**row,'prompt':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'prefix_seconds':prefix_seconds,'summary_build_seconds':summary_build_seconds,'selector_calls':oracle.calls}
