...
def main():
    ...
    for r in rows:
        ...
        original=t.decode(r['prompt_ids'],skip_special_tokens=False)
        if t(original,add_special_tokens=False)['input_ids']!=r['prompt_ids']:raise ValueError('Source token roundtrip differs')
        user=original.split('<|im_start|>user\n',1)[1].rsplit('<|im_end|>',1)[0]
        style=('Return only the requested number, with no punctuation or explanation.' if len(r['references'])==1 else
          'Return only the requested numbers in the question order, separated by a comma and a space. Do not include labels, punctuation at the end, or explanations.')
        text=t.apply_chat_template([{'role':'user','content':user+'\n\n'+style}],tokenize=False,add_generation_prompt=True,enable_thinking=False)
        encoded=t(text,add_special_tokens=False,return_offsets_mapping=True)
        r={**r,'original_prompt_sha256':r['prompt_sha256'],'prompt_ids':encoded['input_ids'],'input_tokens':len(encoded['input_ids']),
           'prompt_sha256':hashlib.sha256(bytes(str(encoded['input_ids']),'ascii')).hexdigest(),'expected':', '.join(r['references'])}
        if r['input_tokens']+128>32768:raise ValueError('Native length exceeded')
        ...
        needed={j for span in spans for j in range(*span['token_span'])}
        mandatory=set(range(min(64,cut)))|set(range(max(64,cut-2048),cut))
        eligible=set(range(1,(cut-2048)//64));support=sorted(support&eligible)
        covered=mandatory|{j for b in support for j in range(b*64,(b+1)*64)}
        if not needed<=covered:raise ValueError('Evidence intersects uncovered local-boundary tokens; oracle is not applicable')
        last_n=r['input_tokens']+128
        covered_throughout=set(range(min(64,cut)))|set(range(max(64,last_n-2048),cut))|{j for b in support for j in range(b*64,(b+1)*64)}
        if not needed<=covered_throughout:raise ValueError('Evidence can enter the uncovered boundary during continuation; oracle not applicable')
        ...
        assert len(wrong)==len(support)<16
    ...
    (a.output/'manifest.json').write_text(json.dumps({'rows':len(prepared),'source':str(a.source),'source_sha256':hashlib.sha256(a.source.read_bytes()).hexdigest(),
      'selection':'All native32K multikey2 and multiquery examples with remote input-located support; no outcome selection; source records/query unchanged, standard native chat and explicit complete numeric output instruction; development adaptation, not official RULER score','block':64,'local':2048,'remote_topk_per_query_head':16},indent=2))
