...
from envelope_native import Oracle,build,SELECTORS
try:
...
def main():
    ...
    p.add_argument('--methods',nargs='+',choices=['Dense','RoPEMean','Quest','PairEnvelope','RandomPair','QuestSplit32'],default=['RoPEMean','Quest','PairEnvelope','RandomPair','QuestSplit32'])
    p.add_argument('--row-id');p.add_argument('--diagnose',action='store_true')
    a=p.parse_args()
    ...
    if a.row_id:rows=[r for r in rows if r['row_id']==a.row_id]
    ...
    for row in rows:
        ...
        if a.diagnose:
            positions=[row['input_tokens']-1]
            keys=re.search(r'for (.*?) mentioned in the provided text',text[cutchar:]).group(1)
            for key in re.split(r',\s*(?:and\s+)?|\s+and\s+',keys):
                ix=text.index(key.strip(),cutchar);end=ix+len(key.strip())
                positions.append(max(i for i,(lo,hi) in enumerate(enc['offset_mapping']) if hi>ix and lo<end))
            row['diagnostic_positions']=sorted(set(positions))
    rotary=[m for name,m in model.named_modules() if name.endswith('rotary_emb') and hasattr(m,'inv_freq')]
    if len(rotary)!=1:raise ValueError('Native rotary table ambiguous')
    omega=rotary[0].inv_freq.detach().float().cpu()
    ...
    with torch.inference_mode():
        ...
        qual_outputs={}
        for method in ('Dense','RoPEMean','Quest','PairEnvelope'):
        ...
        parity={m:bool(torch.equal(qual_outputs[m],qual_outputs['Dense'])) for m in ('RoPEMean','Quest','PairEnvelope')}
    ...
    frozen_code={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),*[Path(__file__).with_name(n) for n in ('envelope_native.py','pair_envelope.py','mean_native.py')]]}
    meta={'status':'RUNNING','pid':os.getpid(),'questions':len(rows),'methods':a.methods,
          ...
          'cost_boundary':'Unchanged native reader, m16 physical64-token remote blocks per query head; no evidence injection; cached FP32 Quest/RPEE/random/QuestSplit32 descriptors have explicitly different bytes'}
        ...
        with torch.inference_mode(),open(a.output/'outputs.jsonl','x') as stream:
            for row in rows:
                ...
                if current_group!=group:
                    ...
                    prefix_seconds=time.monotonic()-tick;build_info=build(oracle,omega=omega);(a.output/(row['row_id']+'_build.json')).write_text(json.dumps(build_info,indent=2));base_ids=prefix.clone();current_group=group
                ...
                for method in meta['methods']:
                    oracle.reset(method,cut)
                    if a.diagnose and method=='RoPEMean':oracle.configure_diagnostics(row['support_blocks'],query_positions=row['diagnostic_positions'],compare_modes=SELECTORS,row_id=row['row_id'])
                    else:oracle.configure_diagnostics(None)
                    cache=copy.deepcopy(base);tick=time.monotonic();last=None
                    ...
                    record={**row,'prompt_ids':None,'method':method,'input_tokens':ids.shape[1],'prefix_tokens':cut,'question_ingest_tokens':tail.shape[1],
                      ...
                      'generated_ids':generated,'output_text':answer,'text_exact':answer==row['expected'],'trimmed_text_exact':answer.strip()==row['expected'],'whole_numeric_list_exact':bool(re.fullmatch(r'\s*\d+(?:\s*,\s*\d+)*\s*',answer)) and re.findall(r'\d+',answer)==row['references'],
                      'ended_eos':generated[-1] in eos,'continuation_seconds':time.monotonic()-tick,
                      'prefix_seconds':prefix_seconds,'selector_calls':oracle.calls,'diagnostic_cost':oracle.diagnostic_stats,'summary_build_seconds':build_info['seconds']}
                    ...
                    diagnostics=oracle.pop_diagnostics()
                    if diagnostics:(a.output/(row['row_id']+'_'+method+'_diagnostics.json')).write_text(json.dumps(diagnostics,indent=2))
