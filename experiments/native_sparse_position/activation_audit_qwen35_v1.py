...
def main():
    ...
    model=AutoModelForImageTextToText.from_pretrained(a.model,dtype=torch.bfloat16,attn_implementation='sdpa').cuda().eval()
    ...
    rotary=[m for m in model.modules() if m.__class__.__name__=='Qwen3_5TextRotaryEmbedding']
    ...
    layers={m.layer_idx:m for m in model.modules() if m.__class__.__name__=='Qwen3_5Attention'}
    ...
    for i in chosen:
        ...
        def qhook(mod,args,out,i=i):raw[i]['q']=out.transpose(1,2).detach()
        def khook(mod,args,out,i=i):raw[i]['k']=out.transpose(1,2).detach()
        hooks.extend([m.register_forward_pre_hook(pre,with_kwargs=True),m.q_norm.register_forward_hook(qhook),m.k_norm.register_forward_hook(khook)])
    ...
    def interface(module,q,k,v,mask,**kwargs):
        if module in [layers[i] for i in chosen]:
            ...
            qcheck,kcheck=apply_rotary_pos_emb(qn,kn,cos,sin)
    ...
    result={'status':'COMPLETE','evidence':'Dense-trajectory operator diagnosis; no answer improvement or speedup claim',
      'record_count':len(records),'records':records,'seconds':time.monotonic()-started,'peak_cuda_bytes':torch.cuda.max_memory_allocated(),
