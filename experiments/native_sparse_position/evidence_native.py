...
class Oracle:
    def __init__(self,model,block=64,local=2048,topk=32):
        ...
        self.forced_blocks=[];self.forced_insertions=0
    ...
    def interface(self,module,q,k,v,mask,**kw):
        ...
        if count:
            ...
            for b in self.forced_blocks:
                if b>=nb or not bool(eligible[b]):raise ValueError('Forced evidence outside eligible remote set')
                present=(chosen==b).any(-1)
                protected=torch.zeros_like(chosen,dtype=torch.bool)
                for other in self.forced_blocks:protected |= chosen==other
                replace=(~protected).to(torch.int32)*torch.arange(1,count+1,device=q.device)[None]
                slot=replace.argmax(-1)
                if bool(((~present)&(replace.max(-1).values==0)).any()):raise ValueError('No same-budget replacement slot')
                rows=torch.arange(H,device=q.device)[~present]
                chosen[rows,slot[~present]]=b
                self.forced_insertions+=int((~present).sum())
