...
if [[ ! -f "${extra_dir}/manifest.json" ]] || \
  ...
  /root/miniconda3/bin/python -m experiments.llama3_60dir_20260911.prepare_planb_panel \
    ...
    --counts-by-cap 4096:40,8192:40,16384:40 \
    --depth-targets 0.10,0.30,0.50,0.70,0.90 \
    --qa-base-offset 7000 \
    --seed 20260927 >"${experiment_root}/logs/prepare_full13_extra40.log" 2>&1
fi
...
if (
    ...
    or manifest.get("qa_base_offset") != 7000
    or manifest.get("rows_sha256") != sha
    or counts != expected
):
    raise ValueError("OLMo extra40 asset violates the frozen Full-13 contract")
...
if len(base_prompts) != 390 or len(extra_prompts) != 1560 or base_prompts & extra_prompts:
    raise ValueError("OLMo base10 and extra40 prompt identities overlap or duplicate")
...
if None in extra_qa or base_qa & extra_qa:
    raise ValueError("OLMo base10 and extra40 QA source identities overlap")
print(json.dumps({
    "status": "TAILSPLINE_OLMO_FULL50_EXTRA40_READY_V1",
