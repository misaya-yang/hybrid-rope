...
if [[ ! -f "${full13_dir}/manifest.json" ]] || \
  ...
  /root/miniconda3/bin/python -m experiments.llama3_60dir_20260911.prepare_planb_panel \
    ...
    --counts-by-cap 4096:10,8192:10,16384:10 \
    --depth-targets 0.10,0.30,0.50,0.70,0.90 \
    --seed 20260925 >"${experiment_root}/logs/prepare_full13.log" 2>&1
fi

if [[ ! -f "${ppl_dir}/manifest.json" ]] || \
   ! grep -q '"status": "COMPLETE"' "${ppl_dir}/manifest.json"; then
  /root/miniconda3/bin/python -m \
    experiments.fixed_rope_three_interfaces_20260913.prepare_olmo_ppl46 \
    --model "${model_dir}" \
...
make_table() {
  ...
  /root/miniconda3/bin/python -m experiments.fixed_rope_three_interfaces_20260913.tables analytic \
    ...
    --method "${method}" \
    --scale 4 \
    --candidate-id "olmo2_1b_s4_classic_${name}" \
    --model-id OLMo-2-0425-1B-Instruct \
    --role "${role}" \
...
if (
    ...
    or full.get("model_contract") != "generic"
    or full.get("rows") != 390
    or full.get("caps") != list(lengths)
    or full.get("rows_sha256") != sha(rows_path)
    or got != expected
    or len({row.get("prompt_sha256") for row in rows}) != 390
):
    raise ValueError("OLMo Full-13 asset violates the frozen 390-row contract")
...
for task in tasks[:6]:
    for length in lengths:
        ...
        if observed != wanted_depths:
            raise ValueError(f"missing OLMo depth coverage: {task}/{length}/{observed}")
if (
    ppl.get("status") != "COMPLETE"
    or ppl.get("contract") != "TAILSPLINE_OLMO_PPL46_V1"
    or ppl.get("documents") != 46
    or ppl.get("lengths") != list(lengths)
    or ppl.get("lm_array_sha256") != sha(lm_path)
):
    raise ValueError("OLMo PPL46 asset violates its frozen contract")
receipts = {
    name: json.loads((table_root / f"{name}.json").read_text())
    for name in ("tailspline", "mrpro")
}
if {tuple(receipt["band_envelope"]) for receipt in receipts.values()} != {(14, 32)}:
    raise ValueError("OLMo comparison tables do not share canonical band [14,32]")
if {float(receipt["gain"]).hex() for receipt in receipts.values()} != {
    (1.0 + 0.1 * math.log(4.0)).hex()
}:
    raise ValueError("OLMo comparison tables do not share canonical S4 gain")
if len({receipt["table_sha256_float32"] for receipt in receipts.values()}) != 2:
    raise ValueError("OLMo TailSpline and MrPro tables are not distinct")
if receipts["tailspline"]["table"]["construction"].get("fitted_coefficients") != 0:
    raise ValueError("OLMo TailSpline receipt is not the zero-fit exact construction")
print(json.dumps({
    "status": "TAILSPLINE_OLMO_CLASSIC_ASSETS_READY_V1",
    "full13_rows": len(rows), "ppl_documents": ppl["documents"],
    "lengths": list(lengths), "band": [14, 32],
