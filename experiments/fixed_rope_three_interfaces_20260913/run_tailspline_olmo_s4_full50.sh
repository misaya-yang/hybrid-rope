...
experiment_root=/root/autodl-tmp/today_rope_plan_20260914/tailspline_olmo_s4_full50
base_root=/root/autodl-tmp/today_rope_plan_20260914/tailspline_olmo_s4_classic
...
"${repo_dir}/experiments/fixed_rope_three_interfaces_20260913/prepare_tailspline_olmo_full50_assets.sh"
...
PY
  ...
  /root/miniconda3/bin/python -m experiments.olmo_recovery_20260912.recovery_v2_eval \
    ...
    --table-label "olmo2_1b_s4_full50_${arm}_extra40" \
    --out "${run_dir}" \
...
for arm in ("tailspline", "mrpro"):
    ...
    if status != {"status": "COMPLETE", "rows": 1560, "lm_rows": 0}:
        raise ValueError(f"incomplete OLMo extra40 arm: {arm}/{status}")
    ...
    if len(combined) != 1950 or len(prompts) != 1950 or counts != expected:
        raise ValueError(f"OLMo full50 coverage drift: {arm}")
    ...
    if contract.get("static_table") != receipt.get("table"):
        raise ValueError(f"OLMo full50 installed table drift: {arm}")
...
if prompt_sets["tailspline"] != prompt_sets["mrpro"]:
    raise ValueError("OLMo full50 arms are not paired on exactly the same prompts")
record = {
    "status": "TAILSPLINE_OLMO_FULL50_VALIDATED_V1",
