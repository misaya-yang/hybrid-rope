...
from .runtime import configure,step
...
def exercise(kind):
 ...
 if wrapper:
  assert record['native_teacher_kl']>=0 and np.array_equal(model.model.rotary_emb.inv_freq.detach().numpy(),student)
...
def exact_peft_resume(kind):
 ...
 with tempfile.TemporaryDirectory() as td:
  path=Path(td)/'resume';save_bundle(path,wrapper,opt,{'arm':'test','micro_sequences':1,'cpt_tokens':16,'gradient_accumulation':1})
  step(model,wrapper,opt,cpt+1,row,row,native_table=native,weights=weights,chunk_size=3,amp=False);expected={n:p.detach().clone() for n,p in model.named_parameters() if p.requires_grad}
  ...
  actual={n:p.detach() for n,p in restored.named_parameters() if p.requires_grad}
  assert actual.keys()==expected.keys() and all(torch.equal(actual[n],expected[n]) for n in actual)
 return 'PASS'
def run():
 result={}
