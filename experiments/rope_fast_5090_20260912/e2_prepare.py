...
WEIGHT_SHA256 = "36d044c73655bb904f822915e6294ba3dae8e6e1af5e703e9d452f2d6a3a294f"
...
def main() -> None:
    ...
    if sha_file(model / "tokenizer.json") != TOKENIZER_SHA256:
        raise ValueError("canonical tokenizer identity changed")
    # The full weight digest is checked by e2_run immediately before GPU use.
    rows: list[dict] = []
    sources = []
    seen = set()
    generation = None
    for source in args.prepared:
        ...
        for name, expected in manifest["prepared_files"].items():
            if name in {"screen.jsonl", "generation_config.json", "tables.json"} and sha_file(source / name) != expected:
                raise ValueError(f"prepared artifact drift: {source / name}")
        ...
        source_tables = json.loads((source / "tables.json").read_text())
        for name, expected in HISTORICAL_TABLE_SHA256.items():
            if source_tables[name]["tensor_sha256"] != expected:
                raise ValueError(f"historical {name} table drift in {source}")
        ...
        for row in batch:
            ...
            if len(row["prompt_ids"]) != row["input_tokens"] or digest(row["prompt_ids"]) != row["prompt_sha256"]:
                raise ValueError("prompt token identity drift")
            ...
            rows.append(kept)
        sources.append({"name": source.name, "manifest_sha256": sha_file(manifest_path),
                        "screen_sha256": sha_file(source / "screen.jsonl"), "rows": len(batch)})
    ...
    tables = build_tables()
    if tables["native_g1"]["tensor_sha256"] != HISTORICAL_TABLE_SHA256["Native"] or \
       tables["mrpro_g4"]["tensor_sha256"] != HISTORICAL_TABLE_SHA256["MrPro"] or \
       tables["bm_g4"]["tensor_sha256"] != HISTORICAL_TABLE_SHA256["MrProBM"]:
        raise AssertionError("new E2 construction does not reproduce historical deployment tensors")
    ...
    code_names = [
        "experiments/rope_fast_5090_20260912/e2_run.py",
        "experiments/rope_fast_5090_20260912/e2_score.py",
        "scripts/experiments/olmo_fast_screen/runtime.py",
        "scripts/experiments/cross_audit/tables.py",
    ...
    manifest = {
        ...
        "model": {"id": MODEL_ID, "revision": REVISION, "path": str(model),
                  "weight_file": "model.safetensors", "weight_sha256": WEIGHT_SHA256,
        ...
        "required_software": {"torch": "2.8.0", "transformers": "5.15.1", "tokenizers": "0.22.2"},
        "required_device": {"name_contains": "5090", "cuda_capability": [12, 0], "torch_arch": "sm_120", "precision": "bfloat16"},
        "code_files": {name: sha_file(root / name) for name in code_names},
        "files": {"inputs.jsonl": sha_file(inputs_path), "tables.json": sha_file(args.output / "tables.json"),
                  "generation_config.json": sha_file(args.output / "generation_config.json")},
