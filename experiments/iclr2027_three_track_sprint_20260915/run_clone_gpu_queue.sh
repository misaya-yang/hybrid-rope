...
native8k=${root}/runs/native_classic8k
if [[ ! -f "${native8k}/status.json" ]]; then
  ${python_bin} -m experiments.olmo_recovery_20260912.recovery_v2_eval \
    --data "${classic}/assets/ppl46/manifest.json" --model "${model}" --arm Native \
    --extra-panel "${classic}/assets/full13/rows.jsonl" --only-extra-panels --skip-lm \
    --length-cap 8192 --prefill-chunk-size 8192 --batch-size 1 \
    --out "${native8k}" --execute >"${root}/logs/native_classic8k.log" 2>&1
fi
${python_bin} -m experiments.iclr2027_three_track_sprint_20260915.native_task_reference_report \
  --panel "${classic}/assets/full13/rows.jsonl" \
  --run "${native8k}/generations.jsonl" \
  --out "${root}/reports/native_classic8k_reference.json"
