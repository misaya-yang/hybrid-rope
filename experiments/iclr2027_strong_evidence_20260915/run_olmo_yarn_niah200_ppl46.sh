...
panel=${plan}/tailspline_olmo_s4_16k_ruler200_clean/assets/panels/16384/inputs.jsonl
ppl=${plan}/tailspline_olmo_s4_classic/assets/ppl46/manifest.json
generation_root=${plan}/tailspline_olmo_s4_16k_ruler200_clean/runs
lm_root=${plan}/tailspline_olmo_s4_classic/runs
table=${plan}/official_yarn_quick/olmo2_1b_s4_16k/tables/yarn.json
root=${plan}/official_yarn_olmo_niah200_ppl46
...
if [[ ! -f "${root}/run/status.json" ]]; then
  command=("${python_bin}" -m experiments.olmo_recovery_20260912.recovery_v2_eval
    ...
    --length-cap 16384 --lm-length-cap 16384
    --prefill-chunk-size 8192 --lm-prefill-chunk-size 0
    --batch-size 1 --longest-first
    --static-table-json "${table}"
    --table-label olmo2_1b_s4_16k_official_static_yarn_niah200_ppl46
...
report_command=("${python_bin}" -m experiments.iclr2027_strong_evidence_20260915.matched_three_method_quick_report
  --condition olmo2_1b_s4_16k_niah200_ppl46
  --target-length 16384 --rows-per-task 200 --ppl-documents 46
...
out={'status':'OLMO2_1B_S4_OFFICIAL_STATIC_YARN_NIAH200_PPL46_COMPLETE_V1','method_identity':'official static YaRN, zero-training installation','niah_tasks':8,'rows_per_task':200,'generation_rows':1600,'ppl_documents':46,'owners':owners}
