...
qa_assets=${qa_root}/infinitebench_en_qa_assets
qa_out=${qa_root}/infinitebench_en_qa_evaluation
...
runtime=${qa_root}/runtime/prefill_131072_${gpu_uuid}.json
gen_chunk=$("${python_bin}" - "${runtime}" <<'PY'
import json,sys
x=json.load(open(sys.argv[1]));g=x.get('recommended_generation_chunk')
if x.get('status')!='PREFILL_CHUNK_BENCHMARK_COMPLETE_V2' or g is None: raise SystemExit('missing 128K runtime receipt')
...
if [[ ! -f "${qa_out}/status.json" ]]; then
  "${python_bin}" -m experiments.iclr2027_strong_evidence_20260915.run_natural_long \
    ...
    --scale 4 --lengths 131072 --rows-per-task 50 \
...
receipt={'status':'QWEN25_3B_256K_HEALTH_PLUS_128K_QA_COMPLETE_V1',
         'ppl_niah_receipt_sha256':hashlib.sha256(base.read_bytes()).hexdigest(),
         'natural_qa_scale':4,'natural_qa_length_cap':131072,
         'natural_qa_256k_status':'PARKED_PENDING_128K_RESULT',
