#!/usr/bin/env python3
"""
NeurIPS Phase 1: Prior-Softmax Adaptation Experiment (v2 - Transformers 5.x Compatible)
...
class AttentionMonitor:
    ...
    def compute_avg_distance(self, attn_weights: torch.Tensor) -> float:
        ...
        distances = torch.abs(query_positions.unsqueeze(0) - avg_pos)
...
def patch_model_for_prior(model, lambda_val: float, use_prior: bool = True):
    ...
    def patched_eager_attention_forward(module, query, key, value, attention_mask, **kwargs):
        ...
        log_distance_prior = -torch.log1p(distance)
...
def save_results(results: List[Dict], baseline_ppl: float):
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for r in results:
            ...
            writer.writerow([
                group, lam, use_lora, f"{r['best_ppl']:.2f}",
                f"{rel:+.1f}%", r['best_step'], r['stopped_early'], r['stop_reason']
            ])
