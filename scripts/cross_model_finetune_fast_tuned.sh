...
TRAIN_SCRIPT="${TRAIN_SCRIPT:-scripts/train_cross_model_lora_fast_tuned.py}"
DATA_DIR="${DATA_DIR:-/root/autodl-tmp/wikitext_data}"
OUTPUT_ROOT="${OUTPUT_ROOT:-artifacts/cross_model_fast_tuned}"
...
ANCHORED_SLOPE_RAW="${ANCHORED_SLOPE_RAW:-20}"
...
run_task() {
  ...
  if [[ "${method}" == "anchored_sigmoid" ]]; then
    cmd+=(
      --anchor_factor "${ANCHOR_FACTOR_DEFAULT}"
      --slope_raw "${ANCHORED_SLOPE_RAW}"
      --center_ratio "${ANCHORED_CENTER_RATIO}"
    )
  fi
