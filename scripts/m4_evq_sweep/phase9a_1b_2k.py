#!/usr/bin/env python3
"""
Phase 9A Group 2: 1B model, L_train=2048, base=500K, 500M tokens, seed=42.
Runs: Geometric, EVQ τ=1.5, Hybrid τ=1.5 r=16
τ*=64/√2048≈1.41, using τ=1.5 (historically wins Geo by 10-19% on 50M/125M/350M)
"""
...
SEQ_LEN   = 2048        # L_train=2048 for Group 2
TAU       = 1.5         # τ*=64/√2048≈1.41
...
CFG_1B = dict(
    ...
    train_tokens=TOKENS,
    lr=3e-4,
    batch_size=16,      # effective (micro=8 × accum=2)
    micro_batch_size=8,
    grad_accum=2,
)
...
def main():
    print("#" * 70)
    print(f"  Phase 9A Group 2: 1B, L=2048, tau=1.5 (base=500K, 500M tokens)")
    print("#" * 70)

    cfg = CFG_1B.copy()

    # micro_bs=8, grad_accum=2, effective=16 — safe for 96GB with 1.7B model
    ...
    print("\nLoading training data (500M tokens, seq=2048)...")
    ...
    runs = [
        ("geo_1b_2k",              geometric_inv_freq()),
        ("hybrid1.5_r16_1b_2k",   hybrid_evq_inv_freq(DIM, BASE, TAU, r=16)),
        ("evq1.5_1b_2k",          evq_cosh_inv_freq(DIM, TAU, BASE)),
    ]
    ...
    # ── Summary ───────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"  PHASE 9A GROUP 2: 1B, L=2048, base=500K, 500M tokens, seed=42")
    print(f"{'='*70}")

    geo_r = results["geo_1b_2k"]
    ...
    print(f"\n  --- 350M reference (L=2048, base=500K) ---")
    print(f"  EVQ τ=1.5 historically wins Geo by 10-19% on retrieval")
    print(f"  Scaling law: τ*=64/sqrt(2048)=1.414, using τ=1.5")

    # Verdict
    evq_r = results.get("evq1.5_1b_2k", {})
    hyb_r = results.get("hybrid1.5_r16_1b_2k", {})
    ...
    print(f"\n  Verdict:")
    if geo_ret:
        print(f"    EVQ τ=1.5 vs Geo:    {(evq_ret/geo_ret-1)*100:+.1f}%  {'WIN' if evq_beats else 'LOSS'}")
        print(f"    Hybrid τ=1.5 vs Geo: {(hyb_ret/geo_ret-1)*100:+.1f}%  {'WIN' if hyb_beats else 'LOSS'}")

    summary = dict(
        passkey_mix_ratio=PASSKEY_MIX_RATIO,
        phase="9A-group2", model="1B", base=BASE, seed=SEED, tokens=TOKENS,
    ...
    save_json(WORK / "phase9a_group2_summary.json", summary)
    print(f"\n  Saved: {WORK}/phase9a_group2_summary.json")
