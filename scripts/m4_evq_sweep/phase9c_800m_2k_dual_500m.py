#!/usr/bin/env python3
"""
Phase 9C: ~800M model, L_train=2048, base=500K.
...
# ~800M: 1600 × 18L, head_dim=64, SwiGLU 4x
CFG_800M = dict(
    vocab_size=50304,
    hidden_size=1600,
    num_layers=18,
...
def run_single(tag, inv_freq, cfg, train_data, val_data, filler, tok):
    ...
    print(f"\n{'='*70}\n  {tag}  (base={BASE:.0f}, 800M, L={SEQ_LEN}, {TOKENS/1e6:.0f}M tokens, seed={SEED})\n{'='*70}")
...
def main():
    print("#" * 70)
    print("  Phase 9B Pilot: 800M, L=2048, Geo vs Hybrid(τ=1.5,r=16), 500M tokens")
    print("#" * 70)
    ...
    runs = [
        ("geo_800m_2k_pilot", geometric_inv_freq()),
        ("hybrid1.5_r16_800m_2k_pilot", hybrid_evq_inv_freq(DIM, BASE, TAU, r=16)),
    ]
    ...
    geo = results["geo_800m_2k_pilot"]
    hyb = results["hybrid1.5_r16_800m_2k_pilot"]
    ...
    print(f"\n{'='*70}")
    print("  PILOT SUMMARY (800M, 2 runs)")
    print(f"{'='*70}")
    ...
    summary = dict(
        passkey_mix_ratio=PASSKEY_MIX_RATIO,
        phase="9C-800M", model="800M", seq_len=SEQ_LEN, tokens=TOKENS,
        runs=["geo", "hybrid1.5_r16"], seed=SEED,
        results=results,
    )
    save_json(WORK / "phase9c_800m_2k_dual_500m_summary.json", summary)
    print(f"\nSaved: {WORK}/phase9c_800m_2k_dual_500m_summary.json")
