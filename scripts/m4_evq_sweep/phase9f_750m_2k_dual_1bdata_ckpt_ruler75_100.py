...
def run_single(tag, inv_freq, cfg, train_data, val_data, filler, tok, ckpt_fractions):
    ...
    print(f"  RULER checkpoint steps: {ruler_checkpoint_steps}")
    ...
    def on_step_end(step_num, steps_total):
        ...
        with torch.no_grad():
            ...
            if step_num in ruler_checkpoint_steps:
                print(f"    [CKPT-RULER] Multi-needle at step={step_num} ({frac:.1%})")
...
def main():
    ...
    save_json(WORK / "phase9f_750m_2k_dual_1bdata_ckpt_ruler75_100_summary.json", summary)
    print(f"\nSaved: {WORK}/phase9f_750m_2k_dual_1bdata_ckpt_ruler75_100_summary.json")
