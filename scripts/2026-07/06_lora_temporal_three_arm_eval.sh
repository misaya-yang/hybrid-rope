...
LEGACY_EXPECTED_SEED="${EVQ_LONGALPACA_EXPECTED_SEED:-42}"
GEO_EXPECTED_SEED="${EVQ_GEO_LONGALPACA_EXPECTED_SEED:-$LEGACY_EXPECTED_SEED}"
EVQ_EXPECTED_SEED="${EVQ_EVQ_LONGALPACA_EXPECTED_SEED:-$LEGACY_EXPECTED_SEED}"
...
"$PYTHON" -u "$REPO_ROOT/experiments/lora_evq_v2/eval_temporal_holdout_three_arm.py" \
  ...
  --expected_geo_seed "$GEO_EXPECTED_SEED" \
  --expected_evq_seed "$EVQ_EXPECTED_SEED" \
  --output "$OUTPUT"
