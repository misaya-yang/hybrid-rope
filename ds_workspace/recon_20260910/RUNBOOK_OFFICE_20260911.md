...
对角相同、沿 (1,1) 差 **199 倍**。**更便宜且更好的路线是在少数显式方向上测投影 Fisher**
（预算方向 / 同预算形状方向 / gain / 可选 EVQ 残差方向）——3 维 = 参考 + 6 次对称前向。
**这也是我们 `SNR<1` 问题的解**：单槽梯度不可测，但**方向梯度让多个槽协同移动**。
...
2. **投影 Fisher**（§4 末）——需要新脚本，**这是修复 SNR 问题的正路**
