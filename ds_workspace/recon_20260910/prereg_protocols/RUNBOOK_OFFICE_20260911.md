...
### ★ 仪器确定性：`D_pattern` 没有噪声底（我方独立核验）

同一臂 `turns_a1_b16` 在 **01:58 与 06:47 两次跑**（各 350 行）：

```
correct 不同: 0/350        output_text 不同: 0/350   ⟹ 逐字节相同
```

> **⟹ 解码是确定性的。任何两次运行之间的逐行差都不是采样噪声，全部是表造成的。**
> 这比 Pro 计划自己的论证更强——它只假设 D_pattern 可比，实际是**零噪声底**。

### ⚠ 对 §3 表的两处口径更正（子代理 B）

1. **`condEVQ` 的"S=42"与"中心 22"是同一条恒等式**（`S = 64 − μ`），**不是两条独立验证**。
   独立成立的是：τ=2.0301373113（残差 0.0）、**S=42 精确**（来自 `φ(u)+φ(1−u)=1` 配对求和 = (n+1)/2，
   **非巧合**）、方差 18.78373383、有序/单调/端点平台同赢家、残差方向 a∈[0,1] 全程 S≡42。
2. **`transport_a1b64` 的 μ_Q 与 S_Q 是恒等式**（`μ_Q = a + b·μ_s`、`S_Q ≡ 64 − μ_Q`），
   **不是独立经验证据**，S_Q 零额外信息。数学对，但它不构成对 Pro §7 的独立支持。

### ★ 支持跑 `condEVQ` 的一个实质论据

`condEVQ` 的 **var(ε) = 18.78 恰好落在 `ctl_C42`(15.6) 与 `ctl_C42V24`(24.0) 之间**，
而那一对**实测差 10.73pp（p<1e-4）**（见下）。
**⟹ 这个残差方向恰好沿一条已被验证与性能相关的轴**——这是跑它的理由，胜过"曲线漂亮"。

### ⚠ `step42` / `condEVQ` 之外：Pro §7 的产物**已被测过且为负**

迁移表与 **Qwen 原生 `turns_a1_b64`** 在 m 空间 **max|Δm| = 0.002**（不是巧合——
迁移式保平均 log 圈数，而 turn-window 族正是按圈数放带）。
**而那一臂在 Qwen2.5-3B 36 行 RULER 上实测 −10.74pp vs MrRoPE（@131072 −14.17pp），是四个 turns 臂里最差的。**
**⟹ §7 的数学对，产物经验上是负的。**
