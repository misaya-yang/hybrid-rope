...
> **但成员并不一致**：`turns_a1_b64` 在 held-out 上是 **+4.40pp**（18胜/7负，t=+1.11，保住约 1/3），
> `b3_lo14` 是 **−0.00pp**。**选择面板的排名不能预测 held-out 的排名。**
...
| 8 | **held-out：`b3_lo14`=BM（−0.00pp），但 `turns_a1_b64` 仍 +4.40pp** ⟹ 选择效应被放大，但存在更小的真实效应 | ★★ 72 行配对 | `HOLDOUT_VERDICT` |
