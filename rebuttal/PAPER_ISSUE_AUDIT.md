...
4. Figure/Table mismatch was verified and is now fixed in the working `paper/main.pdf`: Table 21 remains the coherent Gold-NLL table, and Figure 8 now plots Gold-answer NLL / EVQ relative NLL change. See `rebuttal/FIGURE_TABLE_AUDIT.md`.
...
| I15 | QuALITY figure/table mismatch | Fixed in current `paper/main.pdf`: Table 21 reports Gold-NLL; Figure 8 now uses regenerated `fig5_downstream_qa` NLL content under an NLL caption (`paper/appendix/a3_supporting_results.tex:71-98`; `rebuttal/FIGURE_TABLE_AUDIT.md`) | True P0 trust issue, now resolved | If asked, concede the stale/mislabeled figure and say it has been corrected | Keep QuALITY as supporting probability-space evidence, not a downstream accuracy claim |
...
| --- | --- | --- | --- |
| Submitted Figure/Table audit | Resolve Figure 8/Table 21 mismatch | `rebuttal/FIGURE_TABLE_AUDIT.md` now verifies the fix in current `paper/main.pdf` | Reviewer distrusts all numeric presentation |
