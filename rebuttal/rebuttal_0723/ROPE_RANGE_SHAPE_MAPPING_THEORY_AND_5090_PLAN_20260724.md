...
`rebuttal/rebuttal_0723/geo_rope_contract.py:45-120`。本次还直接从历史
...
这些值和
`rebuttal/rebuttal_0723/FREQUENCY_DEFINITION_MANIFEST.json:6-143`
...
| FMRoPE | Std-Geo 训练 \(b=L_{\rm tr}\)，推理将 \(b\) 调到目标长度 | 是；本地实现见 `fmrope_125m_l256/protocol.py:152-158,235-252` |
...
(`fmrope_evq_combo_l256/protocol.py:136-215`)。它与纯 FMRoPE 的区别不只
是 shape：
