#!/usr/bin/env python3
"""Frozen 500M-token protocol for the matched FMRoPE / EVQ L=256 experiment."""
...
class ExperimentSpec:
    ...
    requested_train_tokens: int = 500_000_000
...
def anchored_cosh_inv_freq(
    ...
    *,
    ...
    """Cosh interior spacing with the exact native-Geo sampled range."""
    n_freqs = int(head_dim) // 2
    phi = evq_cosh_phi(
        n_freqs, tau=float(tau), midpoint=True, dtype=torch.float64
    )
    shape = (phi - phi[0]) / (phi[-1] - phi[0])
    log_span = (n_freqs - 1) / n_freqs * math.log(float(base))
    return torch.exp(-log_span * shape).to(dtype=dtype)
...
def training_inv_freq(
    ...
    *,
    ...
    if arm == "anchored_cosh_tau4_fmrope_range":
        return anchored_cosh_inv_freq(
            spec.head_dim,
            spec.fmrope_train_base,
            spec.evq_tau,
            dtype=dtype,
...
def runtime_frequency(
    ...
    *,
    ...
    if arm == "anchored_cosh_tau4_fmrope_range":
        base = (
            spec.fmrope_train_base
            if condition == "fixed_train_range"
            else float(length_i)
        )
        inv = (
            training_inv.clone()
            if condition == "fixed_train_range"
            else anchored_cosh_inv_freq(
                spec.head_dim,
                base,
                spec.evq_tau,
        ...
        return inv, 1.0, {
            "identity": "range-anchored Cosh allocation diagnostic",
            "operator": condition,
            "training_base": spec.fmrope_train_base,
            "inference_base": float(base),
            "tau": spec.evq_tau,
            "range_reference": "native Std-Geo sampled endpoint/span",
            "target_length": length_i,
            "requires_declared_target_length": (
                condition == "target_matched_range"
            ),
            "claim_boundary": (
                "post-submission matched-range diagnostic; not the raw "
                "submitted midpoint EVQ-Cosh implementation"
            ),
        }
