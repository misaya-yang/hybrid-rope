...
qa_evq, qa_native, ruler_evq, ruler_native, output = map(
...
"$python_bin" - "$gate_json" "$registered_root" <<'PY'
import hashlib
import json
import pathlib
import sys
...
receipt = {
    "status": "SHARED_QK_COMPLETION_PIPELINE_COMPLETE_V1",
    "shared_full_ran": bool(json.loads(gate_path.read_text())["passed"]),
    "artifacts": artifacts,
}
output = registered_root.parent / "general_qk_shared_completion_receipt.json"
temporary = output.with_suffix(".json.incomplete")
temporary.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n")
temporary.replace(output)
print(json.dumps(receipt, indent=2, sort_keys=True))
