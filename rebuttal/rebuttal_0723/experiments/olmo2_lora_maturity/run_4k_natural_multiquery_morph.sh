...
trainer="$CODE_ROOT/rebuttal/rebuttal_0723/experiments/olmo2_lora_maturity/train_4k_natural_multiquery_morph.py"

preflight() {
  CUDA_VISIBLE_DEVICES="" "$PYTHON_BIN" -m "$module.preflight_4k_natural_multiquery_morph" \
...
train() {
  ...
  CUDA_VISIBLE_DEVICES=0 "$PYTHON_BIN" -m "$module.train_4k_natural_multiquery_morph" \
    --checkpoint "$CHECKPOINT" \
    --parent-adapter "$PARENT_ADAPTER" \
    --prepared-data "$PREPARED_DATA" \
    --routing-data "$ROUTING_DATA" \
