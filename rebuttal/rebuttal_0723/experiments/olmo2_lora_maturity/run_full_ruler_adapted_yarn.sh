...
run_arm \
  "native_lora_yarn2" \
  official_yarn 2 "$NATIVE_ADAPTER" 8192 &
wait_for_slot
run_arm \
  "native_lora_yarn4" \
  official_yarn 4 "$NATIVE_ADAPTER" 16384 &
wait_for_slot
run_arm \
  "evq_lora_yarn2" \
  evq_official_yarn 2 "$EVQ_ADAPTER" 8192 &
wait_for_slot
run_arm \
  "evq_lora_yarn4" \
  evq_official_yarn 4 "$EVQ_ADAPTER" 16384 &
...
echo "adapted Native/EVQ plus official-YaRN evaluation complete: $OUTPUT_ROOT"
