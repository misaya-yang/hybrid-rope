...
case "$MODE" in
  prepare)
    ...
    if [[ -n "${FMR_SOURCE_MANIFEST:-}" ]]; then
      PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
        -m rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.prepare \
      ...
      PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
        -m rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.prepare "${PREPARE_ARGS[@]}"
    fi
    ;;
  ...
  preflight)
    ...
    cd "$REPO_ROOT"
    PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" tests/test_fmrope_125m_l256.py
    ...
    PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
      ...
      --full_hash_check
    PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
      -m rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.run_experiment preflight \
...
from rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.prepare import sha256_file
from rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.run_experiment import code_fingerprint
...
PY
    ...
    for arm in $ARMS; do
      PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
        -m rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.run_experiment train \
        --arm "$arm" \
        --data_manifest "$DATA_MANIFEST" \
    ...
    if [[ "${FMR_EVALUATE:-1}" == "1" ]]; then
      PYTHONPATH="$REPO_ROOT" "$PYTHON_BIN" \
        -m rebuttal.rebuttal_0723.experiments.fmrope_125m_l256.run_experiment evaluate \
        --data_manifest "$DATA_MANIFEST" \
        --work_dir "$FMR_WORK_DIR" \
