#!/usr/bin/env python3
"""Train and evaluate the EVQ-Cosh x FMRoPE combination arm."""
...
from rebuttal.rebuttal_0723.fmrope_evq_combo_l256.prepare import (
    sha256_file,
...
def code_fingerprint() -> str:
    paths = (
        ...
        REPO_ROOT / "rebuttal/rebuttal_0723/geo_rope_contract.py",
        REPO_ROOT / "rebuttal/rebuttal_0723/FREQUENCY_DEFINITION_MANIFEST.json",
        REPO_ROOT / "scripts/lib/rope/schedules.py",
        REPO_ROOT / "scripts/lib/rope/official_yarn.py",
        REPO_ROOT / "tests/test_fmrope_evq_combo_l256.py",
    )
    digest = hashlib.sha256()
    for path in paths:
        if not path.is_file():
...
def summarize_records(
    ...
    display_rows = (
        (
            "EVQ tau=4 + fixed base=256",
            "evq_cosh_tau4_fmrope_base256",
            "fixed_train_base",
        ),
        (
            "EVQ tau=4 + target base=L",
            "evq_cosh_tau4_fmrope_base256",
            "target_matched_base",
        ),
    )
    lines = [
        "# EVQ-Cosh x FMRoPE L=256 result",
        "",
        ...
        "",
        "In-domain identity control: PASS (fixed base=256 equals "
        "target-matched base at L=256).",
        "",
        "| condition | "
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    reference = metadata_by_arm[ARMS[0]]
    ...
    payload = {
        ...
        "summary": summary,
        "claim_boundary": (
            "single-seed matched diagnostic of a new EVQ-Cosh x FMRoPE "
            "combination; it is not an official FMRoPE implementation"
        ),
    }
