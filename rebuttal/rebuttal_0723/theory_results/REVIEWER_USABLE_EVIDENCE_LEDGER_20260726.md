...
| `E-OLMO-RULER-FAMILY` | `RDz6s.1`, `RzWsa.3/.4`, `R27bE.2/.5`, `AC.2` | A subsequent single-seed continuation of the matched EVQ parent, still using only physical 4K backward passes, reaches official 13-task RULER macro `37.51%/21.29%/6.13%` at 4K/8K/16K; at 8K it reaches VT `31%`, CWE `22%`, and FWE `60%`. | `SUPPORTING` | The continuation uses explicit RULER-generator-family supervision and ordinary answer-only CE. Evaluation rows are disjoint, but generator families are shared, and no matched Native arm received this continuation mixture. | `OLMO2_1B_4K_RULER_FAMILY_ADAPTATION_20260726.md` and curated JSON |
