...
| `G-OLMO-QUERY-GAP-PRE-REPAIR` | `NEGATIVE_GUARDRAIL` | The original query-gap +100 adapter reproduces `95/100` 8K and `51/100` 16K first-number exact, but its pre-repair strict raw-token re-evaluation gives `0/100` complete expected-string plus terminal EOS at both lengths; 16K also has retrieval and format failures. This is a historical baseline, not the final +32 adapter. | Applying the pre-repair result to the final adapter, or claiming that the original +100 failure was EOS-only. | `EVQ_QUERY_GAP_FINAL_DIAGNOSTIC.md` + `evq_query_gap_final_diagnostic_20260727/` |
| `G-OLMO-QUERY-GAP-FINAL-BOUNDARY` | `NEGATIVE_GUARDRAIL` | The final EVQ +100+32 chain reaches strict full-string-plus-EOS `100/100`, `98/100`, and `60/100` at 4K/8K/16K, but the 16K beyond-training-gap subset is `31/66` and the 13-family 4K retention matrix contains a localized `niah_single_2` regression from `0.70` to `0.55`. | Uniform 16K capability, clean unseen-task transfer, or no-forgetting. | `EVQ_QUERY_GAP_FINAL_DIAGNOSTIC.md` + `evq_query_gap_realized_eos32_20260728/FINAL_METRICS_AND_LINEAGE.json` |
...
| `D-NATIVE-IMPORTANCE-PROTECTED-EVQ` | `DESIGN_ONLY` | A LeRoPE-inspired mature-model retrofit now has offline code and fail-closed gates: first measure exact Native post-RoPE pair-ablation attention KL, then protect a small stable Native subset while using EVQ and masked Q/K restoration elsewhere. No diagnostic, training, or capability result exists yet. | Treating the analytic `2.205L` anchor, offline implementation, or a future training loss as evidence for submitted EVQ-Cosh. | `OLMO2_NATIVE_IMPORTANCE_PROTECTED_EVQ_HYPOTHESIS_20260728.md` |
