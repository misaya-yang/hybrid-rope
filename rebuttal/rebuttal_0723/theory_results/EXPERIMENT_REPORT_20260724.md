...
The implementation is now code-ready in `../experiments/mla_scarcity_5090/`: sixteen focused
...
## 10. Generalized frequency-band and p-RoPE diagnostic

This checkpoint-only seed-42 diagnostic reuses the three matched 500M-token
checkpoints from Section 6. The FMRoPE phase predictors (x^*=3.657) and
(x^*=4.493) select the discrete channel minimizing

\[
\left|L_{\rm train}\omega_j-x^*\right|.
\]

Pre-RoPE Q/K pair norms were collected on 16 selection and 32 disjoint test
windows at (L_{\rm train}=256). The causal control then replaced exactly
three selected rotary pairs with NoPE while keeping all weights and every
other pair unchanged. FMRoPE uses its registered target-base mapping at
inference; Paper-Geo and EVQ retain their raw checkpoint tables. Positive
delta below means that the deletion worsens held-out tail NLL.

### Band prediction

| checkpoint | empirical Q/K band | predicted channels | minimum error |
| --- | ---: | --- | ---: |
| Paper-Geo, base 500K | 10 | 10 / 9 | 0 |
| EVQ-Cosh, tau 4 / base 500K | 23 | 23 / 22 | 0 |
| FMRoPE, base 256 | 26 | 25 / 23 | 1 |

Thus the generalized phase predictor accurately locates the high-norm channel
specialization for all three schedules. This is a channel-location result, not
yet a causal-utility result.

### Equal-width p-RoPE intervention

| checkpoint | length | predicted/empirical band | adjacent control | far control |
| --- | ---: | ---: | ---: | ---: |
| Paper-Geo | 256 | +0.5842 | +0.0678 | -0.0002 |
| Paper-Geo | 1K / 4K / 8K | -0.1591 / -0.1600 / -0.1485 | +0.0467 / -0.1104 / -0.1128 | +0.0000 / -0.0000 / -0.0003 |
| EVQ-Cosh | 256 | +0.3336 | +0.0314 | +0.2176 |
| EVQ-Cosh | 1K / 4K / 8K | -0.2337 / -0.1265 / -0.0545 | +0.0735 / -0.0101 / -0.0198 | +0.1884 / -0.0024 / -0.0337 |
| FMRoPE target-base | 256 | +0.1263 | +0.0527 | +0.2737 |
| FMRoPE target-base | 1K / 4K / 8K | +0.6362 / +0.0640 / +0.0218 | +0.1845 / +0.1960 / +0.1717 | +0.8028 / +0.3333 / +0.0671 |

The band window is the largest of the three equal-width interventions in only
2/12 checkpoint-length cells, both in-domain Paper-Geo/EVQ cells. For raw
Paper-Geo and EVQ, deleting the trained high-norm band significantly
**improves** 1K--8K NLL; the paired window-bootstrap intervals for those band
deltas exclude zero. Under FMR target-base retargeting, deletion generally
hurts, but the adjacent or far control is more important than the high-norm
band at every length.

**Decision: `DOES_NOT_SUPPORT_BAND_BRIDGE`.** The FMR phase proxy predicts
where Q/K norm concentrates, but that norm band is not a universal causal
usefulness band for extrapolation. Consequently, the geometric observation
that Cosh allocates low density near the FMR band cannot currently be promoted
to the performance root cause, and a band-centered allocation based only on
Q/K norm is not authorized by this result. This is a single-seed oracle
diagnostic, not a downstream-capability or paper-level claim.

## 11. Per-pair causal spectrum and held-out pruning

Two checkpoint-only follow-ups tested whether a finer analytic proxy could
replace the failed three-pair band intervention. They reuse the same seed-42
500M Paper-Geo, EVQ-Cosh and FMRoPE checkpoints, immutable frequency tables,
validation tokens and frozen anchors as Section 10. No weights were trained or
modified.

### 11.1 No static scalar predicts causal importance

Each of the 32 rotary pairs was independently replaced by NoPE. Its held-out
tail-NLL change defines a local causal spectrum. Neither the pre-RoPE Q/K norm
nor the exact discrete two-dimensional phase-covariance eigenvalue predicts
that spectrum across schedules and lengths:

| aggregate over 12 checkpoint-length cells | median Spearman correlation |
| --- | ---: |
| Q/K pair norm vs causal delta | -0.167 |
| phase-covariance utility vs causal delta | -0.045 |

The in-domain cells can have positive correlations, but they reverse under
extrapolation. From 256 to 8K, Paper-Geo and EVQ have 21/32 and 23/32 pairs,
respectively, whose deletion effect changes sign; FMRoPE has only 2/32. The
256-vs-8K causal-spectrum Spearman correlations are -0.742 (Paper-Geo), -0.437
(EVQ) and +0.108 (FMRoPE). Thus a norm-only or phase-only static band score is
not a valid allocation objective for these trained models.

Frequency-assignment swaps strengthen that conclusion. Swapping the trained
band with an equal-width adjacent or far band preserves the exact frequency
multiset, yet changes NLL by at least 0.05 in 22/24 checkpoint-length cells.
The frozen model therefore binds learned channel semantics to specific
frequencies; post-hoc frequency histograms are not interchangeable even when
their support is identical.

### 11.2 Independently selected interference generalizes

The second follow-up used 16 untouched selection anchors to identify pairs for
which the per-pair NoPE delta had a strictly negative 95% bootstrap interval.
It then jointly removed only those pairs and evaluated once on the 32 disjoint
test anchors. Negative delta means lower NLL than the full-RoPE checkpoint.

| checkpoint | length | full NLL | selected-pair NoPE NLL | delta | selected pairs |
| --- | ---: | ---: | ---: | ---: | ---: |
| Paper-Geo | 1K | 5.6580 | 5.3622 | -0.2959 | 8 |
| Paper-Geo | 4K | 6.5363 | 6.3055 | -0.2308 | 14 |
| Paper-Geo | 8K | 6.7458 | 6.4657 | -0.2802 | 16 |
| EVQ-Cosh | 1K | 5.4117 | 4.8933 | -0.5183 | 5 |
| EVQ-Cosh | 4K | 6.4526 | 6.2808 | -0.1718 | 12 |
| EVQ-Cosh | 8K | 6.6616 | 6.5532 | -0.1084 | 18 |
| FMRoPE target-base | 1K | 3.4305 | 3.4305 | +0.0000 | 0 |
| FMRoPE target-base | 4K | 5.4846 | 5.4153 | -0.0694 | 1 |
| FMRoPE target-base | 8K | 6.0747 | 6.0302 | -0.0446 | 1 |

All nonzero entries in the table have paired test-anchor intervals excluding
zero. At 256 tokens the strict rule selects no pair for any checkpoint, so the
intervention preserves the in-domain baseline exactly. The result is therefore
not selection-on-test and not merely a local single-pair effect: raw Geo/EVQ
extrapolation contains reproducible joint frequency-pair interference, whereas
target-aware FMRoPE largely suppresses it.

**Decision: `HELDOUT_PRUNING_SIGNAL`, but not a new method claim.** This result
changes the most plausible mechanism. The main issue is not that Cosh failed to
place a high-norm band at the FMR-predicted index; it did place that band. The
stronger explanation is that features learned under one frequency assignment
become destructive after their phases move out of distribution. EVQ changes
which channels suffer that interference, while target-aware FMR retargeting
reduces it much more effectively. A future training-time allocation should
optimize robustness of learned channel-frequency binding across target lengths,
not simply maximize density near a single analytic band.
