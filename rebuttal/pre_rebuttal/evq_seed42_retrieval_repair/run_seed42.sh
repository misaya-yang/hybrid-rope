...
require_gpu_lock() {
  ...
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate preflight-complete \
  ...
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate model-receipt \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
...
verify_result() {
  ...
  shift 4
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate verify-result \
    --kind "$kind" \
    --result "$output" \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
...
check_gate() {
  ...
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate check-gate \
    --gate "$gate" \
    --status "$status" \
    --stage "$stage" \
    --segment "$segment" \
...
run_controlled_eval() {
  ...
  "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate controlled \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
    --adapter-dir "$adapter" \
...
run_passkey_eval() {
  ...
  "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate passkey \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
    --adapter-dir "$adapter" \
...
run_temporal_eval() {
  ...
  "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate temporal \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
    --adapter-dir "$adapter" \
...
run_capability_eval() {
  ...
  "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate capability \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
    --adapter-dir "$adapter" \
...
run_training() {
  ...
  require_gpu_lock
  "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.train \
    --model-name "$EVQ_REPAIR_MODEL" \
    --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
...
run_stage_gate() {
  ...
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate gate \
    --stage "$stage" \
    --segment "$segment" \
...
run_final() {
  ...
  require_file "$CAPABILITY_RECEIPT"
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate capability-preflight \
    --capability-dir "$EVQ_REPAIR_CAPABILITY_DIR" \
  ...
  "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate final-report \
...
case "$command_name" in
  prepare)
    if [[ -d "$DATA_DIR" ]]; then
      "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.prepare_data \
        --output-dir "$DATA_DIR" --validate-only
    else
      "$PYTHON_BIN" -u -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.prepare_data \
  ...
  preflight)
    ...
    "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate model-hash \
      --model-name "$EVQ_REPAIR_MODEL" \
      --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
    ...
    "$PYTHON_BIN" -m py_compile \
      "$REPO_ROOT/rebuttal/pre_rebuttal/evq_seed42_retrieval_repair/protocol.py" \
      "$REPO_ROOT/rebuttal/pre_rebuttal/evq_seed42_retrieval_repair/prepare_data.py" \
      "$REPO_ROOT/rebuttal/pre_rebuttal/evq_seed42_retrieval_repair/train.py" \
      "$REPO_ROOT/rebuttal/pre_rebuttal/evq_seed42_retrieval_repair/evaluate.py" \
    ...
    bash -n "$REPO_ROOT/rebuttal/pre_rebuttal/evq_seed42_retrieval_repair/run_seed42.sh"
    "$PYTHON_BIN" -m pytest \
    ...
    "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.prepare_data \
      --output-dir "$DATA_DIR" --validate-only
    "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.train \
      --model-name "$EVQ_REPAIR_MODEL" \
      --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
      --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
    ...
    for stage in r8 r16; do
      "$PYTHON_BIN" -m rebuttal.pre_rebuttal.evq_seed42_retrieval_repair.evaluate preflight \
        --model-name "$EVQ_REPAIR_MODEL" \
        --model-manifest "$EVQ_REPAIR_MODEL_MANIFEST" \
        --longalpaca-manifest "$EVQ_REPAIR_LONGALPACA_MANIFEST" \
        --adapter-dir "$EVQ_REPAIR_PARENT_ADAPTER" \
