...
cpu_preflight() {
  ...
  "${PYTHON_BIN}" -m rebuttal.pre_rebuttal.frequency_adaptation_8b.train \
    --model-name "${MODEL}" \
...
eval_checkpoint() {
  ...
  "${PYTHON_BIN}" -m rebuttal.pre_rebuttal.frequency_adaptation_8b.evaluate \
    --model-name "${MODEL}" \
    --model-manifest "${MODEL_MANIFEST}" \
...
case "${command}" in
  prepare)
    ...
    "${PYTHON_BIN}" -m rebuttal.pre_rebuttal.frequency_adaptation_8b.prepare_data \
      --tokenizer "${MODEL}" \
