...
### 规则：每次实验跑完后，必须立刻生成结构化的 markdown 报告并归档。

**这是强制要求，不是可选操作。** 实验结果如果只存在 JSON/log 里而没有人可读的报告，后续 agent 和人类都会重复分析、遗漏关键发现。

### 报告命名规范

```
results/<实验类别>/YYYY-MM-DD_<实验名称>_report.md
```

示例：
- `results/core_text/2026-03-12_phase21b_quality_454m_report.md`
- `results/staged_training/2026-03-10_454m_3stage_multiseed_report.md`
- `results/downstream/2026-03-12_quality_n2086_fulltest_report.md`
...
- 模型: [tier, 架构]
- RoPE: [geo/evq, τ=?, base=?]
- 训练: [steps, lr, batch_size, seq_len]
- 数据: [数据集, 样本数]
- YaRN: [是否使用, scale, 实现版本(progressive/ntk)]

## 关键结果

[核心数据表格，必须包含 baseline 对比]

## 发现与结论

[用 2-5 句话总结：实验证明了什么/否定了什么/下一步该做什么]

## 原始数据路径

[JSON/checkpoint/log 的完整路径]
```

### 为什么这很重要

1. **防止重复劳动**: Phase 21b 的结果分散在多个 JSON + 聊天记录里，每次新 session 都要重新分析一遍
2. **防止数据失真**: n=200 的 +6pp 差异在 n=2086 下确认是噪声，如果有归档报告就能早发现
3. **保护实验配置**: 很多 bug（τ 值、YaRN 实现）是事后才发现的，报告里记录完整配置才能追溯
4. **加速论文写作**: 论文 §Experiments 可以直接从报告中提取数据，不需要再去翻 JSON

### Checklist（实验结束后）

- [ ] 生成 `YYYY-MM-DD_<实验名>_report.md` 并放到对应的 results 目录
- [ ] 报告包含完整配置（τ, YaRN 实现, checkpoint 来源, eval protocol）
- [ ] 报告包含 baseline 对比表格
- [ ] 报告包含原始数据路径
- [ ] 如果结果与预期不符，记录可能的原因分析
