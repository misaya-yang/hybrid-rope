...
> **最后更新**：2026-03-06（理论-叙事-图表矩阵重构；Passkey Mix 多 seed 口径统一；EVQ+YaRN 主图归位；Phase15 750M continue 写入）
...
**长上下文外推真正的瓶颈，不是推理时如何缩放位置，而是训练时如何分配 RoPE 频率。**  
Geometric RoPE 是 `τ=0` 的退化点；EVQ 给出闭式、零额外参数的训练时最优频率分配，而 **EVQ + YaRN 明显优于 Geo + YaRN**，说明 EVQ 不是另一个独立 trick，而是**解锁 inference-time scaling 上限的训练底座**。

### 0.2 正文最该讲的 3 条 claim

1. **Closed-form optimality**  
RoPE frequency allocation 是一个变分逆问题；其解满足二阶 ODE，并得到闭式 `EVQ-cosh`。Geometric 是 `τ→0` 的零温极限，而非天然最优设计。

2. **Extreme extrapolation beats learnable PE**  
在 DAPE-style PE-dominant regime 里，EVQ 用 `0` 额外参数击败 learnable PE：`128→8K` 时 `EVQ PPL@8K = 333.7`，优于 `DAPE 455.3`。这说明 **PE quality itself** 足以决定远程外推质量。

3. **EVQ unlocks YaRN**  
当前最强系统级结论不是 `EVQ > Geo`，而是 **`EVQ + YaRN >> Geo + YaRN`**。在 `10%` fair scale=`8` 比较中：
   - `retrieval@8K`: `100%` vs `61%-65%`
   - `PPL@8K`: `70.9 vs 82.9`
   - `PPL@16K`: `107.5 vs 157.7`
...
| **P2** | `750M continue@4K` full EVQ | 证明继续拉长训练后，EVQ 长程收益没有被洗掉 |
| **P2** | video temporal transfer | 抬高论文上限，证明不是 text-only artifact，但当前不替代 text 主线 |

### 0.4 当前最强的一句话证据

- **方法主锤**：`EVQ+YaRN@8K = 100% across 6/6 seeds`
- **理论主锤**：`Geometric = τ=0`，`τ*=d_head/√L`
- **PE-dominant 主锤**：`L=256, EVQ4+YaRN 99.6 vs Geo+YaRN 260.2` at `32×` (`-61.7%`)
- **更大规模 supporting evidence**：`750M continue@4K` 下 `PPL@16K 24.4 vs 45.1` (`-45.9%`)，且 `8K AR exact 77.5% vs 0%`

### 0.5 摘要/开场应该怎么说

不要以“我们提出一个新的 positional encoding”开头。  
应该直接说：

> **Standard RoPE fails not because inference-time scaling is too weak, but because its training-time geometric frequency allocation is a degenerate point of a broader variational optimum. We derive a closed-form solution, EVQ-cosh, and show that it not only beats learnable/adaptive PE in extreme extrapolation, but more importantly unlocks the full power of YaRN: EVQ+YaRN substantially outperforms Geo+YaRN on long-context extrapolation.**
...
**EVQ solves training-time RoPE frequency allocation in closed form. This not only wins in PE-dominant extreme extrapolation, but more importantly unlocks inference-time scaling: `EVQ+YaRN >> Geo+YaRN`.**

### 14.2 三层主故事（按重要性排序）

1. **理论锚点**：RoPE frequency allocation is a variational inverse problem with a closed-form solution; Geometric is the `τ=0` degenerate case, not the generic optimum.
2. **PE-dominant 验证**：在最干净的 extreme extrapolation regime 中，EVQ 用闭式频率分配直接打赢 Geometric 与 DAPE-style learnable baselines，证明 “PE quality itself” 足以决定长程 PPL。
3. **系统级杀手锏**：真正限制 YaRN 的不是 inference-time scaling 本身，而是训练时 geometric frequency allocation。EVQ 提供更好的训练底座，因此 `EVQ+YaRN` 远强于 `Geo+YaRN`。
...
| **P2** | 750M dynamics + 4K-continue full EVQ | 单 seed，但轨迹与最终 continue head-to-head 一致 | supporting mechanism / larger-scale support，不当主锤 |
| **P2** | video temporal transfer | 2-seed 支持迁移 + EVQ×YaRN synergy | 上限抬升项，当前仍应附录优先 |

### 14.4 正文叙事顺序（建议 8 步，不再平均发力）

1. **变分逆问题**：RoPE frequency allocation is a variational inverse problem with a closed-form solution.
2. **Geometric 是退化点**：standard RoPE corresponds to `τ=0`; EVQ is the continuous family that contains it as a degenerate case.
3. **Waterbed 与碰撞块**：解释为什么代价有限、为什么 `base=10K` 会进入 dead zone。
4. **PE-dominant 极限外推**：在 128-token / 256-token regime 中证明 PE 质量本身决定远程 PPL，并直接对标 DAPE-style learnable PE。
5. **跨规模 raw gain**：50M-350M 一致证明 EVQ 不是 toy artifact。
6. **Capability-preserving**：未训练 retrieval 不破坏，已训练 retrieval 稳定增强。
7. **🔴 EVQ+YaRN 超线性协同**：这是当前最值钱的系统级结论，必须成为主图中心。
8. **视频 temporal 迁移**：作为跨模态上限证明，而不是替代 text 主线。

### 14.5 当前主文图表组合（按重要性重排）

| 资产 | 定位 | 主承载信息 |
|------|------|-----------|
| **Figure 2** `docs/paperdraft/figs/fig2_evq_yarn_synergy.pdf` | **主图** | `EVQ+YaRN >> Geo+YaRN`；training-time frequency optimization 解锁 inference-time scaling |
| **Figure 3（主图）** | **第二主图** | PE-dominant regime / scaling law direct validation；128-token extreme extrapolation 对标 DAPE-style learnable PE + Phase 11 raw/YaRN 双确认 | `docs/paperdraft/figs/fig3_pe_dominant_scaling.pdf` |
| Figure 1 `docs/paperdraft/figs/fig1_frequency_dynamics.pdf` | supporting 图 | 频率重分配机制 + 750M dynamics supporting evidence |
...
- **750M continue @4K / single-seed supporting evidence**：`PPL@16K 24.4 vs 45.1` (`-45.9%`)，`8K AR exact 77.5% vs 0%`
...
- `750M retrieval divergence`：只作为 supporting mechanism，不提升为 multi-seed 主 claim
...
| "为什么不用 YaRN 就好" | **核心点正相反**：`EVQ+YaRN >> Geo+YaRN`，说明真正瓶颈是训练时 geometric frequency allocation |
...
| "短程会退化" | 多规模 raw PPL 与 750M/Phase 11 共同显示：代价小且可解释，收益集中在真正长程段 |
