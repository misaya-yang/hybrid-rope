...
% CHANGELOG (v8):
% - Added 750M 1B-token Phase9F training dynamics (Geo vs Hybrid, 4 checkpoints).
% - New Figure 1: 3-panel (frequency allocation, PPL dynamics, passkey dynamics).
% - Core finding: Geo passkey regression vs Hybrid monotonic improvement (+20pp @8K).
% - Added 5%->10% passkey mix asymmetric scaling result.
% - Added training efficiency: Hybrid@50% > Geo@100% in OOD PPL.
% - Scale extended from 50M-350M to 50M-750M.
% - Updated abstract, experiments, limitations, conclusion.
...
Empirically, from-scratch training at four scales (50M--750M) on both TinyStories and FineWeb-Edu consistently reduces extrapolation perplexity by 10--19\% at 16K with $\le\!0.4\%$ short-context cost.
At 750M scale (1B tokens), we uncover a striking \emph{retrieval divergence}: while both geometric and EVQ exhibit comparable OOD perplexity degradation (waterbed effect), geometric RoPE's passkey retrieval \emph{regresses} from 70\% to 60\% during training, whereas EVQ's \emph{monotonically improves} from 65\% to 80\%---a +20pp gap that widens with training.
A passkey-mix experiment isolates the extrapolation benefit: EVQ doubles 4K retrieval (42\%$\to$82\%, +40pp), and increasing mix ratio from 5\% to 10\% reveals an antisymmetric scaling where geometric loses $-$22pp while EVQ gains +22pp, demonstrating that frequency allocation quality---not data quantity---is the bottleneck for length generalization.
A collision-block analysis predicts---and base-10K experiments confirm---a dead zone where EVQ cannot help.
The entire method reduces to replacing one line of \texttt{inv\_freq} initialization, with zero hyperparameters.
\end{abstract}
...
\begin{itemize}
  ...
  \item \textbf{Comprehensive from-scratch validation (50M--750M):} four-scale experiments consistently show 10--19\% extrapolation PPL improvement with $\le$0.4\% short-context cost. At 750M scale with 1B tokens, we discover a \emph{retrieval divergence}: geometric passkey retrieval regresses after 50\% training while EVQ improves monotonically, reaching a +20pp gap. A passkey-mix experiment demonstrates +40pp retrieval gain and an antisymmetric data-scaling phenomenon (Geo $-$22pp / EVQ +22pp), establishing a Capability-Preserving Property (Proposition~4).
\end{itemize}
...
\subsection{From-scratch PPL scaling (50M--750M)}
...
\bottomrule
\end{tabular}
\caption{From-scratch EVQ vs.\ Geometric across scales and datasets. Short-context cost $\le+0.4\%$; long-context improvement 10--19\% at 50M--350M. The 750M Hybrid ($\tau{=}1.5$, $r{=}16$ partial warp) shows slightly worse OOD PPL than Geometric, but this is compensated by dramatically better positional retrieval (Table~\ref{tab:750m_dynamics}). Note: 750M uses Hybrid (partial warp) while 50M--350M use full EVQ (all channels warped).}
\label{tab:scaling}
\end{table}

Table~\ref{tab:scaling} demonstrates consistent extrapolation improvement across three model scales and two datasets for full EVQ. The asymmetric waterbed trade-off is evident: short-context cost is within noise ($\le+0.4\%$), while long-context gains are substantial (10--19\%). The 750M Hybrid result (partial warp, Section~\ref{sec:750m_dynamics}) trades 5.7\% OOD PPL for +20pp passkey retrieval improvement and fundamentally different training dynamics.
...
This is one of the paper's strongest results: at 2K both methods are perfect, so the 4K gap ($42\%\to 82\%$, +40pp) is \emph{purely} an extrapolation benefit from frequency redistribution. The PPL waterbed pattern (+0.8\% short, $-4.4\%$ long) independently confirms the theory.
...
\paragraph{Antisymmetric data scaling.}
Increasing passkey mix ratio from 5\% to 10\% (same total tokens) reveals a striking antisymmetric effect: Geometric 4K retrieval \emph{drops} from 64\% to 42\% ($-$22pp), while EVQ \emph{rises} from 60\% to 82\% (+22pp). More retrieval training data makes Geometric \emph{worse} at extrapolation---it overfits to the training-length retrieval pattern---while EVQ converts the same data into improved generalization. This demonstrates that \textbf{frequency allocation quality, not data quantity, is the bottleneck for length generalization}.

\subsection{750M training dynamics: retrieval divergence}
\label{sec:750m_dynamics}

To study how frequency allocation affects learning dynamics over extended training, we train 750M models (H=1536, 18 layers, 24 heads) for 1B tokens on FineWeb-Edu with 3\% passkey mix, evaluating at 4 checkpoints (25\%, 50\%, 75\%, 100\% of training). We compare Geometric against Hybrid EVQ ($\tau{=}1.5$, $r{=}16$: the first 16 high-frequency channels retain geometric spacing, the remaining 16 low-frequency channels use EVQ warp).

\begin{table}[t]
\centering
\begin{tabular}{lcccccc}
\toprule
& \multicolumn{2}{c}{PPL@8K} & \multicolumn{2}{c}{Passkey@8K} & \multicolumn{2}{c}{AR Exact} \\
Checkpoint & Geo & Hybrid & Geo & Hybrid & Geo & Hybrid \\
\midrule
25\% & \textbf{97.8} & 104.1 & \textbf{55\%} & 45\% & 1.3\% & 1.3\% \\
50\% & \textbf{99.4} & 104.4 & \textbf{70\%} & 65\% & 3.8\% & \textbf{7.5\%} \\
75\% & \textbf{108.8} & 115.2 & 60\% & \textbf{75\%} & 7.5\% & \textbf{20.0\%} \\
100\% & \textbf{115.0} & 121.6 & 60\% & \textbf{80\%} & 16.3\% & \textbf{30.0\%} \\
\bottomrule
\end{tabular}
\caption{750M training dynamics (1B tokens, FineWeb-Edu + 3\% passkey, seed=42). Both methods exhibit comparable OOD PPL degradation (waterbed), but their passkey trajectories \emph{diverge}: Geometric regresses from 70\%$\to$60\% after 50\%, while Hybrid monotonically improves to 80\%. AR exact match nearly doubles (30\% vs.\ 16.3\%).}
\label{tab:750m_dynamics}
\end{table}

\textbf{The retrieval divergence} (Figure~\ref{fig:main}) is the paper's most striking finding. Despite comparable PPL waterbed magnitudes (Geo +17.6\%, Hybrid +16.8\% at L=8K from 25\%$\to$100\%), the two methods' positional retrieval trajectories are fundamentally different:

\begin{itemize}
\item \textbf{Geometric regression}: Passkey@8K peaks at 70\% (50\% checkpoint) then \emph{regresses} to 60\%, never recovering. Extended training actively \emph{damages} long-range retrieval.
\item \textbf{Hybrid monotonic improvement}: Passkey@8K rises from 45\%$\to$65\%$\to$75\%$\to$80\%, never regressing. EVQ's frequency spacing preserves positional distinguishability even as OOD perplexity degrades.
\end{itemize}

This reveals that Geometric RoPE is not merely ``statically suboptimal'' (Corollary~1) but \textbf{dynamically degenerative}: its low-frequency phase collisions worsen during training as the model overfits to the training-length distribution, actively destroying extrapolation capability. EVQ's expanded low-frequency spacing prevents this collapse.

\paragraph{Training efficiency.} Hybrid at 50\% training (500M tokens) already surpasses Geometric's full 1B-token result in OOD PPL: 104.4 vs.\ 115.0 at 8K ($-$9.3\%) and 225.7 vs.\ 253.2 at 16K ($-$10.9\%). EVQ achieves better extrapolation with half the compute budget.

\paragraph{In-distribution cost.} At 100\% training, Hybrid matches Geometric in-distribution: PPL@1K = 17.53 vs.\ 17.55 ($-$0.14\%), PPL@2K = 21.65 vs.\ 21.98 ($-$1.51\%). Frequency reallocation incurs zero short-context cost.

\begin{figure}[t]
\centering
\safeincludegraphics[width=\textwidth]{fig1_neurips.pdf}
\caption{\textbf{EVQ frequency allocation and training dynamics at 750M scale.} (a)~Log-frequency allocation: EVQ ($\tau{=}1.5$) expands low-frequency spacing and compresses high-frequency spacing relative to Geometric. (b)~OOD perplexity at L=8K: both methods exhibit comparable waterbed degradation during training, with in-distribution PPL (L=2K, dashed) closely matched. (c)~Passkey retrieval at L=8K: Geometric regresses after 50\% training ($-$10pp), while Hybrid monotonically improves (+10pp over the same period), reaching a final +20pp gap. This \emph{retrieval divergence} despite symmetric PPL waterbed is the central empirical finding.}
\label{fig:main}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Waterbed trade-off is intrinsic.} The waterbed inequality \eqref{eq:waterbed} guarantees that any deviation from geometric incurs \emph{some} frequency-band cost. At 750M, the Hybrid (partial warp, $r{=}16$) OOD PPL is 5.7\% worse than Geometric at 8K--16K. The full EVQ (all channels warped) at 350M shows the opposite ($-$13.3\% at 16K), suggesting the optimal warp boundary $r^*$ requires careful calibration. The retrieval benefit (+20pp) may justify the PPL cost in retrieval-heavy applications, but users should evaluate this trade-off for their specific use case.

  \item \textbf{Model scale.} From-scratch experiments span 50M--750M. While the PE theory depends only on $L$, $d_{\mathrm{head}}$, and $\tau$ (not model size), validation at $>$1B parameters would strengthen the empirical case. RoPE frequency allocation papers in this subfield typically validate at 125M--350M scale.
  ...
  \item \textbf{Warp boundary $r^*$.} The hybrid design partitions channels into geometric (high-freq) and EVQ (low-freq). The analytical formula $r^*=(d/2\ln b)\ln(L/2\pi)$ has not yet been validated via sweep experiments. The discrepancy between 350M full-EVQ and 750M partial-warp results may partly reflect suboptimal $r$ choice.

  \item \textbf{Statistical power.} The 750M training dynamics and 5\%/10\% passkey scaling results are single-seed. While effect sizes are large (+20pp, $\pm$22pp), multi-seed confirmation would strengthen confidence. RULER multi-needle evaluation (8 trials) has insufficient statistical power for reliable comparison.

  \item \textbf{Downstream task coverage.} The passkey-retrieval mix demonstrates extrapolation benefit on a specific retrieval task. Standard long-context benchmarks (LongBench, SCROLLS) on instruction-tuned models remain future work.
\end{itemize}
...
Empirically, from-scratch training at four scales (50M--750M) on two datasets consistently confirms the theory's predictions. The most striking finding emerges at 750M scale: while both geometric and EVQ methods exhibit comparable OOD perplexity degradation (the waterbed effect), their positional retrieval trajectories fundamentally diverge---geometric RoPE \emph{regresses} after 50\% training while EVQ improves monotonically, reaching a +20pp gap. This \emph{retrieval divergence} reveals that geometric RoPE is not merely statically suboptimal but dynamically degenerative: continued training actively damages its long-range capability. A passkey-mix experiment provides complementary evidence: EVQ doubles 4K extrapolation accuracy (42\%$\to$82\%), and an antisymmetric data-scaling phenomenon (Geo $-$22pp / EVQ +22pp when doubling mix ratio) demonstrates that frequency allocation quality, not data quantity, is the bottleneck for length generalization.

The entire method replaces one line of \texttt{inv\_freq} initialization with zero hyperparameters. The degradation safety guarantee (Theorem~2: worst case $=$ geometric) eliminates the risk of catastrophic failure. Future work includes warp-boundary ($r^*$) validation, layer-wise frequency allocation, task-conditional priors, and validation at billion-parameter scale.
...
\paragraph{H.4 750M training dynamics (Table~\ref{tab:750m_dynamics}, Figure~\ref{fig:main}).}
\begin{table}[H]
\centering
\begin{tabular}{p{0.28\linewidth}p{0.66\linewidth}}
\toprule
Item & Configuration \\
\midrule
Model & 750M (hidden=1536, layers=18, heads=24, head\_dim=64, FFN=6144) \\
Dataset & FineWeb-Edu (streaming) + 3\% passkey mix \\
Methods & Geometric vs Hybrid ($\tau{=}1.5$, $r{=}16$) \\
Tokens & 1B per method (15258 steps) \\
Checkpoints & 25\%, 50\%, 75\%, 100\% of training \\
Seed & 42 \\
Eval: PPL & L $\in$ \{1K, 2K, 4K, 8K, 16K\}, 4 chunks per checkpoint, 6 chunks final \\
Eval: Passkey & L $\in$ \{1K, 2K, 4K, 8K\}, depth=0.5, 20 trials/checkpoint, 40 trials final \\
Eval: RULER & 5 needles, L $\in$ \{2K, 4K, 8K\}, 8 trials at 75\%+100\% \\
GPU & RTX PRO 6000 96GB \\
Training time & 16.4h total (491.8min Geo + 492.0min Hybrid) \\
\bottomrule
\end{tabular}
\label{tab:app_h_750m}
\end{table}

\paragraph{H.5 Competitor comparison (Table~\ref{tab:competitors}).}
\begin{table}[H]
\centering
\begin{tabular}{p{0.28\linewidth}p{0.66\linewidth}}
\toprule
...
\paragraph{H.6 $\tau^*$ scaling law sweep (Table~\ref{tab:tau_scaling}).}
\begin{table}[H]
\centering
\begin{tabular}{p{0.28\linewidth}p{0.66\linewidth}}
\toprule
...
\paragraph{H.7 Collision-block dead-zone validation (Table~\ref{tab:collision_10k}).}
\begin{table}[H]
\centering
\begin{tabular}{p{0.28\linewidth}p{0.66\linewidth}}
\toprule
...
\paragraph{H.8 EVQ-cosh implementation.}
