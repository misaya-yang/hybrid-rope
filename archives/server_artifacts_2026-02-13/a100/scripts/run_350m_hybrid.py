"""
350M Hybrid Experiment: alpha=0.2 (0.8*geo_10k + 0.2*anchpoly_p3.9_omf0.3)
Based on 50M results showing PPL=12.35 for alpha=0.2
"""
...
SEED = 42
WORK_DIR = "/opt/dfrope/results/350m_hybrid"
...
# Training: ~100M tokens
TRAIN_CFG = {
    "seq_len": 2048,
    "total_tokens": 100_000_000,
    "batch_size": 16,      # Reduced from 32 to avoid OOM
    "grad_accum": 2,       # Effective batch = 32
    "lr": 2e-4,            # Lower LR for larger model
    "weight_decay": 0.1,
    "warmup_frac": 0.02,
    "max_grad_norm": 1.0,
}
...
def hybrid_freq(K, p=3.9, omega_min_factor=0.3, alpha=0.2, theta=10000):
    """
    Hybrid: alpha * anchpoly + (1-alpha) * geo.
    alpha=0.2: 0.8*geo_10k + 0.2*anchpoly_p3.9_omf0.3
    This gave PPL=12.35 in 50M experiments.
    """
...
def train(model, train_data, cfg, save_path):
    ...
    steps_per_epoch = n_chunks // (cfg["batch_size"] * cfg["grad_accum"])
    total_steps = (n_chunks // cfg["batch_size"])
    warmup_steps = int(total_steps * cfg["warmup_frac"])
    print(f"  Training: {total_steps} steps ({steps_per_epoch} grad_accum steps)")
    scaler = GradScaler()
    t0 = time.time()
    losses = []
    ...
    for step in range(total_steps):
        ...
        if (step + 1) % 100 == 0:
            avg_loss = sum(losses[-50:]) / len(losses[-50:]) if losses else 0.0
...
def run_experiment(name, inv_freq, train_data, eval_data, all_results):
    ...
    all_results["experiments"][name] = {
        "losses": losses[::50] if losses else [],
        "eval": eval_results
    }
...
def main():
    ...
    print("="*60)
    print("  350M HYBRID EXPERIMENT")
    print("  alpha=0.2 (0.8*geo_10k + 0.2*anchpoly_p3.9_omf0.3)")
    print("  50M result: PPL=12.35")
    print("="*60)
    ...
    # Run hybrid experiment with alpha=0.2
    print("\n>>> 350M Hybrid: alpha=0.2")
    name = "hybrid_350m_alpha0.2"
    inv_freq = hybrid_freq(K, p=3.9, omega_min_factor=0.3, alpha=0.2, theta=10000)
    run_experiment(name, inv_freq, train_data, eval_data, all_results)
    
    # Summary
    print(f"\n{'='*80}")
    print("  350M HYBRID EXPERIMENT COMPLETE")
    print(f"{'='*80}")
    print(f"\nResults saved to {WORK_DIR}/results.json")
    
    if "experiments" in all_results and "hybrid_350m_alpha0.2" in all_results["experiments"]:
        eval_data = all_results["experiments"]["hybrid_350m_alpha0.2"].get("eval", {})
        for length, ppl in eval_data.items():
            print(f"  PPL@{length} = {ppl:.2f}")
