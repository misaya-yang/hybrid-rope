"""
Round 3: A100 极速 RoPE 频率分布搜索 - 50M Model
搜索 omf=0.3 附近的胜者区域
"""
...
def hybrid_freq(K, p=3.9, omega_min_factor=0.3, alpha=0.5, theta=10000):
    ...
    anchpoly = anchored_polynomial_freq(K, p, omega_max=1.0, omega_min=omega_min)
...
def print_table(all_results, benchmark_ppl):
    """Print real-time comparison table."""
    print(f"\n{'='*80}")
    print(f"  RESULTS TABLE (vs benchmark anchpoly_p3.9_omf0.3)")
    print(f"{'='*80}")
    print(f"{'Config':<25} | {'2048':>8} | {'16384':>8} | {'Improvement':>12}")
    print("-" * 80)
    
    bench_2048 = benchmark_ppl.get("2048", 0) if benchmark_ppl else 0
    bench_16384 = benchmark_ppl.get("16384", 0) if benchmark_ppl else 0
    ...
    for name, data in all_results["experiments"].items():
        ...
        if bench_16384 > 0 and ppl_16384 > 0:
            improvement = (bench_16384 - ppl_16384) / bench_16384 * 100
        ...
        marker = " [B]" if "p3.9_omf0.3" in name and "geo" not in name else ""
        print(f"{name:<25} | {ppl_2048_str:>8} | {ppl_16384_str:>8} | {imp_str:>12}{marker}")
    print("=" * 80)

def main():
    Path(WORK_DIR).mkdir(parents=True, exist_ok=True)
    print("="*60)
    print("  Round 3: A100 RoPE Frequency Search (50M)")
    print("="*60)
    ...
    # geo_10k
    inv_freq = geometric_freq(K, 10000)
    run_experiment("geo_10k", inv_freq, train_data, eval_data, all_results)
    print_table(all_results, None)
    
    # anchpoly_p3.9_omf0.3 (Benchmark)
    inv_freq = anchored_polynomial_freq(K, p=3.9, omega_max=1.0, omega_min=omega_min_base * 0.3)
    benchmark_ppl = run_experiment("anchpoly_p3.9_omf0.3", inv_freq, train_data, eval_data, all_results)
    print_table(all_results, benchmark_ppl)
    
    # ============ GROUP A: omf tuning ============
    print("\n>>> Phase 2: Group A - omega_min factor tuning (p=3.9)")
    for omf in [0.1, 0.15, 0.2, 0.25, 0.35, 0.4]:
        name = f"anchpoly_p3.9_omf{omf}"
        omega_min = omega_min_base * omf
        print(f"\nConfig: {name}, omega_min={omega_min:.2e}")
        inv_freq = anchored_polynomial_freq(K, p=3.9, omega_max=1.0, omega_min=omega_min)
        run_experiment(name, inv_freq, train_data, eval_data, all_results)
        print_table(all_results, benchmark_ppl)
    
    # ============ GROUP B: p tuning ============
    print("\n>>> Phase 3: Group B - p value tuning (omf=0.3)")
    for p in [3.5, 3.6, 3.7, 3.8, 4.0, 4.1, 4.2, 4.3]:
        name = f"anchpoly_p{p}_omf0.3"
        inv_freq = anchored_polynomial_freq(K, p=p, omega_max=1.0, omega_min=omega_min_base * 0.3)
        run_experiment(name, inv_freq, train_data, eval_data, all_results)
        print_table(all_results, benchmark_ppl)
    
    # ============ GROUP C: Hybrid ============
    print("\n>>> Phase 4: Group C - Hybrid distributions")
    ...
    if benchmark_ppl and best_name != "anchpoly_p3.9_omf0.3":
        improvement = (benchmark_ppl["16384"] - best_ppl) / benchmark_ppl["16384"] * 100
        print(f"   Improvement vs benchmark: {improvement:.1f}%")
    
    print(f"\n✅ All results saved to {WORK_DIR}/results.json")
