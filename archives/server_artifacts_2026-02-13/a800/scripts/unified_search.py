"""
Unified RoPE Frequency Distribution Search
Identical code runs on both machines, different config splits.
"""
...
WORK_DIR = "/opt/dfrope/results/unified_search"
...
def main():
    ...
    # ===== ALIGNMENT CHECK (both machines run this first) =====
    align_configs = [
        ("geo_10k_ALIGN", geometric_freq(K, 10000)),
        ("geo_500k_ALIGN", geometric_freq(K, 500000)),
    ]

    # ===== SPLIT A: Machine A (A100) =====
    split_a_configs = [
        # Sigmoid theta=10k fine-tuning
        ("sig_s6_m0.5_omf0.3_t10k", sigmoid_freq(K, 10000, 6, 0.5, 0.3)),
        ("sig_s7_m0.5_omf0.3_t10k", sigmoid_freq(K, 10000, 7, 0.5, 0.3)),
        ("sig_s9_m0.5_omf0.3_t10k", sigmoid_freq(K, 10000, 9, 0.5, 0.3)),
        ("sig_s10_m0.5_omf0.3_t10k", sigmoid_freq(K, 10000, 10, 0.5, 0.3)),
        ("sig_s8_m0.4_omf0.3_t10k", sigmoid_freq(K, 10000, 8, 0.4, 0.3)),
        ("sig_s8_m0.6_omf0.3_t10k", sigmoid_freq(K, 10000, 8, 0.6, 0.3)),
        # Sigmoid theta=30k/50k
        ("sig_s8_m0.5_omf0.3_t30k", sigmoid_freq(K, 30000, 8, 0.5, 0.3)),
        ("sig_s8_m0.5_omf0.3_t50k", sigmoid_freq(K, 50000, 8, 0.5, 0.3)),
        # Anchored Poly + high theta
        ("anchpoly_p3.9_omf0.3_t100k", anchored_poly_freq(K, 100000, 3.9, 0.3)),
        ("anchpoly_p3.9_omf0.3_t500k", anchored_poly_freq(K, 500000, 3.9, 0.3)),
    ]

    # ===== SPLIT B: Machine B (H800) =====
    split_b_configs = [
        # Sigmoid + high theta
        ("sig_s8_m0.5_omf0.3_t100k", sigmoid_freq(K, 100000, 8, 0.5, 0.3)),
        ("sig_s8_m0.5_omf0.3_t500k", sigmoid_freq(K, 500000, 8, 0.5, 0.3)),
        ("sig_s5_m0.5_omf0.3_t100k", sigmoid_freq(K, 100000, 5, 0.5, 0.3)),
        ("sig_s5_m0.5_omf0.3_t500k", sigmoid_freq(K, 500000, 5, 0.5, 0.3)),
        # Hybrid + high theta
        (
            "hybrid_a0.2_t100k",
            hybrid_freq(geometric_freq(K, 100000), anchored_poly_freq(K, 100000, 3.9, 0.3), 0.2),
        ),
        (
            "hybrid_a0.2_t500k",
            hybrid_freq(geometric_freq(K, 500000), anchored_poly_freq(K, 500000, 3.9, 0.3), 0.2),
        ),
        # Mixed: sigmoid + geo_500k
        ("mix_sig8t10k_geo500k_a0.2", hybrid_freq(geometric_freq(K, 500000), sigmoid_freq(K, 10000, 8, 0.5, 0.3), 0.2)),
        ("mix_sig8t10k_geo500k_a0.5", hybrid_freq(geometric_freq(K, 500000), sigmoid_freq(K, 10000, 8, 0.5, 0.3), 0.5)),
        # Sigmoid omf tuning on high theta
        ("sig_s8_m0.5_omf0.1_t100k", sigmoid_freq(K, 100000, 8, 0.5, 0.1)),
        ("sig_s8_m0.5_omf0.5_t100k", sigmoid_freq(K, 100000, 8, 0.5, 0.5)),
    ]

    # Select configs based on split
    if split == "A":
        configs = align_configs + split_a_configs
    elif split == "B":
        configs = align_configs + split_b_configs
    elif split == "BOTH":
        configs = align_configs + split_a_configs + split_b_configs
    else:
        print(f"Unknown split: {split}. Use A, B, or BOTH.")
        sys.exit(1)

    # Run all
    all_results = {}
    for name, freq in configs:
        result = run_one(name, freq, train_data, val_data)
        all_results[name] = result

        # Save after each (crash safety)
        with open(f"{WORK_DIR}/results_{split}.json", "w") as f:
            json.dump(all_results, f, indent=2)

        # Check if we beat geo_500k
        ppl_16k = result.get("16384", 999)
        if ppl_16k < 26.48:
            print(f"\n  *** WINNER FOUND: {name} = {ppl_16k} < 26.48 ***\n")

    # Final leaderboard
    print(f"\n{'='*60}")
    print(f"  LEADERBOARD (Split {split})")
    print(f"{'='*60}")
    ranked = sorted(all_results.items(), key=lambda x: x[1].get("16384", 999))
    print(f"  {'Rank':<5} {'Config':<35} {'PPL@2048':<10} {'PPL@16384':<10}")
    print(f"  {'-'*65}")
    for i, (name, r) in enumerate(ranked):
        p2 = r.get("2048", "N/A")
        p16 = r.get("16384", "N/A")
        marker = " ***" if isinstance(p16, float) and p16 < 26.48 else ""
        print(f"  {i+1:<5} {name:<35} {p2:<10} {p16:<10}{marker}")
