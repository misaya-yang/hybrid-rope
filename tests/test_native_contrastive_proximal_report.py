...
from experiments.native_contrastive_proximal_20260915 import report_ruler


def make_rows(arm: str, *, delta: float = 0.0) -> list[dict]:
    rows = []
    for task_index, task in enumerate(report_ruler.TASKS):
        for index in range(report_ruler.ROWS_PER_TASK):
            row_id = f"{task}-{index}"
...
def write_run(path, rows):
    ...
    (path / "status.json").write_text(json.dumps({
        "status": "COMPLETE", "rows": report_ruler.EXPECTED_ROWS, "lm_rows": 0,
    }))
...
def test_load_run_requires_complete_full13_panel(tmp_path):
    ...
    assert len(report_ruler.load_run(tmp_path)) == report_ruler.EXPECTED_ROWS
...
def test_load_run_rejects_duplicate_identity(tmp_path):
    ...
    with pytest.raises(ValueError, match="duplicate"):
        report_ruler.load_run(tmp_path)


def test_paired_task_bootstrap_preserves_task_equal_weighting():
    native = {row["row_id"]: row for row in make_rows("native")}
    ncp = {row["row_id"]: row for row in make_rows("ncp", delta=0.1)}
    result = report_ruler.paired_task_bootstrap(ncp, native, draws=1000)
    assert result["delta"] == pytest.approx(0.05, abs=1e-12)
    assert result["paired_task_equal_bootstrap_ci95"][0] > 0.0
    assert result["candidate_only_wins"] == 390
    assert result["native_only_wins"] == 0
    assert result["ties"] == 390
