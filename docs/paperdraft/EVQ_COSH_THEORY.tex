...
\author{paperdraft extract from \texttt{docs/paperdraft/CORE\_THEORY.md}}
\date{2026-03-05}
...
Given $N$ discrete frequency channels, we use midpoint quantiles
\begin{equation}
u_k=\frac{k+\tfrac{1}{2}}{N},\qquad k=0,1,\dots,N-1,
\end{equation}
...
high fidelity (reported $R^2_{\mathrm{mid}}>0.99$) and that the remaining matrix
...
Let $u_k=(k+\tfrac{1}{2})/N$. Then $\phi_k(\tau)=\phi(u_k;\tau)$ satisfies
...
\begin{proposition}[Endpoint anchoring and ``no $r$'' fact]
For any $\tau>0$, the EVQ warp satisfies $\phi(0;\tau)=0$ and $\phi(1;\tau)=1$.
In particular, the highest-frequency channel ($u\approx 0$) satisfies
$\omega=b^{-\phi}=1$ exactly, matching geometric RoPE at $k=0$.
\end{proposition}
\begin{proof}
...
For $N=d_{\mathrm{head}}/2$ and $u_k=(k+0.5)/N$:
\begin{equation}
