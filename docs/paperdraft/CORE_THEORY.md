...
4. **Waterbed + Hybrid**："Waterbed explains the cost; Hybrid isolates high-freq to achieve Pareto-optimal"
5. **方差降低**："Hybrid reduces seed variance 8× (passkey), 2× (PPL)"
6. **碰撞块**："Net gain ∝ (1-c)/lnb, predicts dead zone at low base (confirmed)"
7. **Geo 动态退化 (Figure 1)**："Geometric dynamically degrades — passkey -10pp regression after 50%, while Hybrid monotonically improves to +20pp"
8. **Retrieval divergence**："PPL waterbed is symmetric, but passkey trajectories diverge: Geo regresses, Hybrid improves"
...
- ✅ **检索距离重分布**：40-trial 全局 retrieval 相同 81.87%，但 Hybrid 向 L=8K 推移 +12.5pp
...
| "Hybrid 是 hack" | r* 解析解 + Riemann-Lebesgue 严格优越性 |
...
| "为什么不用 YaRN" | PI inference-time PPL@2K=191.7（崩溃）+ PE baselines 对比 (进行中) |
...
- **750M OOD PPL Hybrid 反而更差（+5.7%@16K）**——与 350M 结果方向相反。需注意 Hybrid (r=16 partial warp) ≠ 全通道 EVQ。论文中需清楚区分
