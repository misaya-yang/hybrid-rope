...
> evidence 处理；详见 `rebuttal/rebuttal_0723/01_REBUTTAL_PLAYBOOK.md`。
...
τ 是唯一控制参数，τ=0 时退化为 geometric RoPE (Theorem 2)。默认 operating rule 使用 τ* = d_head / √L_train，用于选择经验优良 basin，而非全局最优声明。
...
- Metric: teacher-forced NLL-gap retrieval rate unless a row is explicitly marked as autoregressive exact match
...
## 3. Model Architecture Tiers

所有实验使用相同的 GPT-2-style decoder-only architecture (无 bias, RMSNorm, SiLU activation):

| Tier | Layers | Heads | d_model | d_head | FFN | Params | 用于哪些 Phase |
|------|--------|-------|---------|--------|-----|--------|--------------|
| 50M | 6 | 8 | 512 | 64 | 2048 | ~50M | Phase 8, 14, 16 |
| 125M | 12 | 12 | 768 | 64 | 3072 | ~125M | Phase 8, 11b, 14, 16 |
| 350M | 24 | 16 | 1024 | 64 | 4096 | ~350M | Phase 8, 11, 14 |
| 454M | 24 | 16 | 1024 | 64 | 4096 | ~454M | Phase 11c, 17b, 17c, 21b |
| 500M | 28 | 16 | 1024 | 64 | 4096 | ~500M | Phase 8 |
| 750M | 18 | 24 | 1536 | 64 | 6144 | ~750M | Phase 9f, 13a, 15, 21b |

> **关键设计选择**: 所有 tier 的 d_head=64，这使得 τ* = 64/√L_train 成为通用的单参数公式。

配置定义位置:
- 50M-500M: `scripts/core_text_phases/run_evq_sweep.py` TIER_CONFIGS (line 74-135)
- 454M: `scripts/core_text_phases/phase21b_quality_eval_clean.py` (line 39-56)
- 750M: `scripts/core_text_phases/phase21b_quality_eval_clean.py` (line 39-56)
