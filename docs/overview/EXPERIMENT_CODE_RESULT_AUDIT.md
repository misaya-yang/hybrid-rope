...
| --- | --- | --- | --- | --- |
| Table 2 EVQ x YaRN, 454M, 10% passkey mix | `scripts/core_text_phases/run_evq_sweep.py`, `scripts/supporting_eval/eval_passkey_scratch.py`, figure reader `scripts/figures/fig2_evq_yarn_orthogonality.py`; `phase14c_multiscale_evq_yarn.py` is supporting only. | Healthy for reported metric: PK helper writes NLL-gap JSON, figure reads curated 10% 3-seed JSON, and wording is teacher-forced NLL-gap. | Current branch now tracks the exact six-run archival source at `data/results_5090b/evq_yarn_10pct_allseeds.json` and its portable full-payload snapshot. | `A` for values and raw evaluation replay; checkpoint/data hashes remain unavailable. |
| Table 4 / Fig. 3(a) PE-dominant 128-to-8K diagnostic | `scripts/core_text_phases/phase11b_125m_dape.py` trains/evals Geo, EVQ, and DAPE-style rows and writes per-run `result.json` plus `all_results.json`. | Healthy for panel/table scope. Seed scope stays explicit: the matched Geo/DAPE/EVQ comparison is seed 42; fixed EVQ tau=5 and learnable tau have three-seed evidence. | Current: `data/curated/fig3_extreme_128.json` plus raw-backed `data/curated/primary2_l128_fixed_tau5_3seed.json`. | `A` for Table 4 / panel (a); `B` for fixed-EVQ multi-seed only, not matched control replication. |
...
| Primary MLA 8K/500M 3-seed stress test | `scripts/core_text_phases/run_gqa_evq_experiment.py` trains MLA/GQA/MHA and writes per-run `results.json`, `inv_freq.npy`, and work-dir `summary.json`; `scripts/core_text_phases/eval_extended_3seeds.py` evaluates 3 seeds and writes `eval_3seeds_full_results.json`. | Eval explicitly uses checkpoint-loaded `inv_freq` before YaRN and resolves current/historical run IDs. The tracked aggregate deterministically reconstructs the evaluator JSON byte-for-byte. | Current: `data/curated/eval_3seeds_full_results.json`, exact SHA256 `1e44d30...30953`, plus the annotated portable snapshot. | `A` for eval values and exact evaluator JSON; `B` for per-run training JSON/checkpoint bundle. |
