...
| 1B/4K MLA reversal and YaRN+FT row | `run_350m_4k_1b.sh` / `run_350m_4k_v2_1b.sh` launch `run_gqa_evq_experiment.py`; `scripts/core_text_phases/yarn_finetune_eval.py` performs baseline, inference-only YaRN, YaRN+FT, and writes `yarn_ft_s*_seed*_results.json`. | Code support exists and now resolves both current and historical run IDs. Launch wrappers remain historical because they encode external server assumptions. | Current compact branch has `results/PHASE18_YARN_FT_REPORT.md` and `results/PHASE19_TAU1_vs_GEO_REPORT.md`, but no exact 1B/4K baseline or YaRN+FT JSON was found. Visible archival branch also does not close this specific JSON gap. | `C`: code-backed and report-backed, but not JSON-backed in the current branch. |
...
| Phase 21b QuALITY downstream support | `scripts/core_text_phases/phase21b_quality_eval_clean.py`; full-eval report `docs/exp/2026-03-12_phase21b_454m_full_eval_report.md`. | The clean NLL code path and n=2086 report agree with Table 21, but the compact repo does not contain the corresponding per-length Gold-NLL JSON. The current `phase21b_quality_454m_report.json` is an older n=200 accuracy-only pilot and must not be used as Table 21 provenance. | Report-backed Table 21 values; older pilot JSON retained only to explain the stale Figure 8. | `C` supporting-only; not reviewer-grade JSON-backed. |
