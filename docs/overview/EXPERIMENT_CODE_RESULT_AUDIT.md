...
| Primary MLA 8K/500M 3-seed stress test | `scripts/core_text_phases/run_gqa_evq_experiment.py` trains MLA/GQA/MHA and `scripts/core_text_phases/eval_extended_3seeds.py` writes the per-seed evaluation aggregate. | Eval code uses checkpoint-loaded `inv_freq` before YaRN and resolves current/historical run IDs. | Reviewer-facing recovery is `data/curated/table18_mla_3seed_aggregate.json`; original per-seed JSON/checkpoints are unavailable. | Aggregate-only: printed means/std preserved; paired deltas and checkpoint provenance unrecovered. |
| 1B/4K MLA reversal and YaRN+FT row | Historical launchers and `scripts/core_text_phases/yarn_finetune_eval.py` can produce baseline/YaRN+FT JSONs. | Code support exists; launch wrappers remain historical and server-specific. | Original report, exact JSONs, checkpoints, and data hashes are unavailable; only paper/archive summaries survive. | `C`: code-backed and summary-backed limitation, not artifact-backed evidence. |
...
| Phase 21b QuALITY downstream support | `scripts/core_text_phases/phase21b_quality_eval_clean.py` and result subdirs under `results/core_text/phase21b/quality_454m/`. | Code/result linkage is strong for supporting downstream NLL; keep capacity caveats. | Current: `results/core_text/phase21b/phase21b_quality_454m_report.json` and per-length `result.json` files. | `A` supporting-only. |
...
the exact JSONs behind `PHASE18_YARN_FT_REPORT.md`.
