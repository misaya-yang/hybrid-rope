...
| `paper/appendix/a4_supporting_experiments.tex`, 1B/4K MLA paragraph | Could sound like strong evidence despite missing exact JSON in compact branch. | `results/PHASE18_YARN_FT_REPORT.md` reports the row; `run_gqa_evq_experiment.py` and `yarn_finetune_eval.py` can produce the JSONs, but compact branch lacks exact baseline/YaRN+FT JSON. | Leave supporting-only. Do not cite in rebuttal as JSON-backed until exact JSON is recovered/regenerated. |
