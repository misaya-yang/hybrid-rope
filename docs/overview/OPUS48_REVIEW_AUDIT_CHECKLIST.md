...
| O48-13 | P0 | Open | 1B MLA raw EVQ reverses: EVQ worse at 8K/16K; EVQ+YaRN+FT only mildly better at target. | The paper supporting row retains the seed-42 limitation; the historical report itself is not in the compact checkout. | Treat as limitation and root-cause target. Do not use this row as broad support. |
...
- Report artifact: not tracked in the compact checkout; the paper appendix retains the scoped summary.
...
High-risk statements from the retired paper-local rebuttal playbook must not be
restored when a reviewer response is drafted:
