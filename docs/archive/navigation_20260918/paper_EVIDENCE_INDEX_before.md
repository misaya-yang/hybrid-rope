...
[9月18日修订owner](../revision_20260918/README.md)对应当前35页工作稿。A65 Kanana官方部署比较和单侧minimum-bending解释已经入稿；A59/A60/A61及A54/A62沿用9月17日已入稿身份。NTS2、CA-NCP及其安全后续保留成立边界和负结果，不列作当前方法贡献。
...
| TailSpline怎样构造，扩展窗口质量如何？ | A37、A39、A40、A46、A49 | §5.1 / Fig. 3；§6.1 / Table 1 / Fig. 4；Appendices B, D |
| 自然任务、原生参考与等位移形状有什么结果？ | A45、A50–A53 | §6.1 / Tables 1–2 / Appendix D |
| 对MrPro、官方YaRN和更大模型的迁移证据是什么？ | A59–A61、A65 | [关键实验罗盘](../../../docs/research/next_stage_20260912/KEY_EXPERIMENT_COMPASS_20260914.md)及各结果owner |
| z怎样改善原生窗口内任务表现，哪些后续失败？ | A54、A62–A64 | §6.2 / Appendix E；负结果owner |
| 学习期有什么支持？ | A09、A10、A12 | §6.3 / Fig. 5 / Appendix F |

本页是逐资产证据注册表，不再复制执行队列。快速了解当前强结果先读[关键实验罗盘](../../../docs/research/next_stage_20260912/KEY_EXPERIMENT_COMPASS_20260914.md)；核对具体数字再进入对应owner或下方资产来源。

## 全部资产

| ID / 重要性 | 资产与科学问题 | 证据身份 / 论文位置 | 来源 |
|---|---|---|---|
| A01 / P0 | **固定支持三seed识别**：只动内部指数能否改变学习与外推 | 三seed配对；带raw hash摘要；§3.2 / Appendix C.2 | [EXACT_RANGE_151M_3SEED_RESULT_20260820.json](EXACT_RANGE_151M_3SEED_RESULT_20260820.json) (tracked) |
| A02 / P0 | **M4多形状factorial**：设计价值是否超出一条曲线 | 12配置×3seed；摘要；§3.2 / Appendix C.4 | [m4_exact_range_factorial_evidence_20260726.json](../../../rebuttal/rebuttal_0723/theory_results/m4_exact_range_factorial_evidence_20260726.json) (tracked) |
...
| A08 / P0 | **成熟同支持冻结识别**：已有权重下变量是否仍有效 | held-out与curated摘要；§3.1 / Fig. 1b / Appendix C.5 | [frozen_fixed_support_mature_20260823.json](../../../data/curated/frozen_fixed_support_mature_20260823.json) (tracked)<br>[SAME_SUPPORT_FROZEN_CHECKPOINT_RESULTS_20260823.json](../attention-aware-retrofit/evidence/SAME_SUPPORT_FROZEN_CHECKPOINT_RESULTS_20260823.json) (tracked) |
...
| A42 | Hybrid-RoPE理论深化算子链 | [结果owner](../../../docs/research/next_stage_20260912/THEORY_DEEPENING_CPU_VERIFICATION_20260915.md)、[固定JSON](../../../experiments/iclr2027_three_track_sprint_20260915/theory_deepening_checks.json)；新增minimum-bending/frame本地审计已登记；15类算子检查及单侧最小弯曲恒等式通过；frame审计不支持稳定性优势，不从CPU推出任务排序或通用最优表 |
...
- **A01**：Cosh训练系列的固定支持识别，仅改变30内点，三seed在2×/4×/8×均改善；支持范围与内部配置分别控制。
...
- **A42**：有限softmax/value、gain仿射可恢复条件、T−C边际配对、稀疏访问／压缩／旋转value等算子关系通过独立CPU核验。TailSpline严格最小化的是故意不计高频入口的单侧bending目标；双侧目标对应BM。frame lower bound没有得到稳定优势。标准恒等式本身不作为新颖性主张；没有路径干预时使用`acts through`而非经验性的`mediated by`，不从CPU检查推出任何方法胜负。
...
| A65 | Kanana官方runtime YaRN的64K/128K直接比较 | [结果owner](../../../experiments/kanana_yarn_tailspline_64k_20260918/RESULT.md)；64K Full-13×10 TailSpline/MrPro/YaRN为`72.65/70.33/65.64%`；128K完整上下文InfiniteBench En.QA TailSpline/YaRN为`19.59/17.85%`，差`+1.74pp` |
