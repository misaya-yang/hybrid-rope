> 历史检索快照（2026-09-14）：下文的当前状态和授权仅指原记录时点。最新路线见 [index.md](index.md)；本页不构成执行队列。

# 当前研究：边界、低频终点与中频过渡

**当前状态（2026-09-14）：精确TailSpline是唯一新候选；后续统一评测使用Llama-3-8B。**
...
| 任务 | 入口 |
|---|---|
| 当前唯一方法与GPU主线 | [TailSpline方法与统一评测](TAILSPLINE_ROPE_METHOD_AND_UNIFIED_EVAL_20260914.md)：one-sided TailSpline与symmetric BM边界审计；Llama S4先做冻结同参数对照，再进入PPL、NIAH/passkey、full-13 RULER |
