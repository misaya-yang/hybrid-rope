...
候选AUC相对BM、C42、MrPro分别为`+2.27/+0.52/+0.70pp`，95%区间分别为
`[-1.65,+6.02]`、`[-2.26,+3.25]`、`[-3.30,+4.75]pp`，全部跨0。候选worst还比
C42低`4.70pp`。因此独立追加块没有确认低6对MrPro或C42的优势，也没有出现明确的
反向AUC证据；准确状态是未决。
...
所以本轮确认后的严格结论是：候选在累计面板上的区间AUC可靠超过BM；相对MrPro和
同band C42只有正点估计，尚未确认。累计结果包含已经开封的低6，不能冒充独立
holdout；追加块必须单独保留，不能用累计均值掩盖它的跨零结果。

## 4. Native保持

追加块72个32K prompts上，候选`81.81`、Native`77.18`，差`+4.63pp`，95%区间
`[-1.44,+11.04]pp`；方向与低6一致但单块未确认。累计108个匹配prompts上，候选
...
1. 在第三checkpoint上，冻结mix075组合配置可靠超过BM的累计AUC；对MrPro和
   same-band C42尚未确认。由于C42的gain也不同，不能把累计正点估计全部归因于
   transition形状，更不能写成已经全面超过MrPro。
