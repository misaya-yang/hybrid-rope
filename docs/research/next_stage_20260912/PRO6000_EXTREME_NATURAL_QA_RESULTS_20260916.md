...
1. **论文主区间仍然成立。** Qwen2.5-3B在Native 32K到128K（S4）的完整长书
   En.QA上，TailSpline为`19.14%`，MrPro为`15.90%`，差`+3.24pp`；按7个来源
   context聚类的95%区间为`[+0.20,+7.41]pp`。
2. **极限长度表现按能力分化。** Qwen2.5-3B在256K（S8）的三项single-NIAH
...
原始逐行生成保留在实验数据盘。本报告未将confirm40、256K Natural-QA、
Llama32K截断控制或GLM计为已完成结果。Qwen官方静态YaRN已完成生成，正式比较另等报告；
Llama clean32K YaRN仍是不同的待做合同。下一版取舍见[准备清单](PAPER_NEXT_REVISION_PREPARATION_20260916.md)。
