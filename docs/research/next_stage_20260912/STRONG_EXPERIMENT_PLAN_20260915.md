...
本文保留X1–X8原始执行规格；下文“未生成/停放/下一项”是制定时状态，不能作为当前队列。
已完成结果见[实验罗盘](KEY_EXPERIMENT_COMPASS_20260914.md)，当前取舍见
[下一版准备](PAPER_NEXT_REVISION_PREPARATION_20260916.md)，运行状态见
[执行owner](../../../experiments/iclr2027_three_track_sprint_20260915/SERVER_TASK_LAYERS.md)。
NCP已有OLMo原生任务总体正结果；Qwen S4自然QA/S8检索已完成。
Qwen YaRN已完成生成；这不等于Llama clean32K X8已完成。
本轮只准备文档，本文命令不因此获得新执行授权。

## 历史规格与回执


更新：2026-09-15。**交付状态：执行规格、X1--X7通用入口及NCP原生窗口验证入口已完成；入口测试不等于模型实验完成。**
...
**推荐主线：原生零校准增强 + 跨模型clean矩阵 + 自然长文 + clean等位移对照。**
S16/128K是并行的规模迁移检查，不以最大s统领论文。
NCP作为X5后的新增开发验证；原生增强是后续研究主线之一，尚未得到的模型收益不替代现稿已有证据。
具体理论先行的准备、机制面板与CPU代码见[本轮协调计划](NATIVE_ORAL_PREPARATION_PLAN_20260915.md)；
已有Native-Z5结果保留原有研究身份。下方旧表中的执行状态需按当前结果owner核对，不能当实时队列。
...
| NCP / X5后 | OLMo原生4K Full13×60，NCP对Native | 780次生成；复用已完成Native 780条 | 构表、运行、配对报告入口已实现并测试；服务器已冻结表，GPU未跑 | 无校准原生窗口增强的开发判别 |
...
X1/X2/X4/X5/X6与NCP合计 **11,440次新增生成**，另加X3的2N次与S16的20个LM文档前向。
这不是要求先全部完成才更新论文；每个完整实验独立形成结果。

### 工作量与调度

按标称输入上限估算，X1约32M、X2约256M、X4约96M、X5约16M、NCP约3.2M、X6约34M输入tokens；
真实长度通常更短。这些只是容量上界，**不等于相同GPU时间**：长序列注意力成本非线性。
利用现有日志或正式任务前32条的实际耗时更新ETA，继续原任务，不另开重复测速网格。

- 当前32GB已授权队列：X3（已完成）→ X4 → X5 → NCP；NCP完成并保存报告后检查其他作业再关机。
...
NCP只使用OLMo公开Native频率表和原生长度构表，固定端点、频率槽位和gain=1，不读取模型
权重、激活、梯度、校准文本或任务分数。复用`olmo_native_halfturn_phase`中已完成的
Full13×60 Native输出，只新增同一780条prompt上的NCP生成；主要终点是NCP−Native的配对
任务等权差和区间。这是开发面板上的真实模型判别，不冒充独立确认或跨checkpoint结论。

入口为`experiments/native_contrastive_proximal_20260915/run_ruler_gate.sh --execute`；输出根为
`/root/autodl-tmp/today_rope_plan_20260914/olmo_native_contrastive_proximal`。默认调用只打印计划，
执行入口持有同一个GPU锁、验证可复用Native 780条、支持NCP续跑，完整后生成
`reports/ncp_vs_native_ruler4k.json`。
...
| `native_contrastive_proximal_20260915/` | NCP公开参数构表、单臂续跑及Native配对报告 | FP32表身份固定；只新增NCP 780条；CPU风险改善不冒充模型结果 |
