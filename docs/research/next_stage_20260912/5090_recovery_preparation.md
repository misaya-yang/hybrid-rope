...
当前入口：`ssh -p 24904 [REDACTED_EMAIL]`。原40403是准备时的旧实例；实例克隆可以改变SSH端口，后续以用户最新告知为准，不追回旧端口、不据此重做准备。用户保证克隆模型、数据和代码资产准确，**后续不做SHA校验，也不因历史代码/manifest/权重SHA不一致阻塞启动、恢复、评价或统计。** 不反复读取大文件计算哈希，不为重新绑定字节身份重建同一数据包。
...
- 5090入口：`ssh -p 24904 [REDACTED_EMAIL]`。
