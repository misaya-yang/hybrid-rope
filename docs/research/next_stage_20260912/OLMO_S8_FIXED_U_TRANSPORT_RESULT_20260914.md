...
- worst-length：fixed-m `0.11852`，fixed-u `0.12593`，固定面板差`+0.00741`；95%区间
  `[-0.05741,+0.07778]`作为端点稳定性；
...
转化为总体任务优势：正式AUC低`5.154pp`，虽然32K端点高`0.741pp`。因此该规则不扩大到
Llama/Qwen，也不扫alpha、band、tail或gain；结论来自预定区间AUC，而不是让bootstrap
否决或批准固定面板分数。
