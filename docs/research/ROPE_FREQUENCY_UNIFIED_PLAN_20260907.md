...
> **2026-09-08 收尾修正：** 本夜队列及自动评测停止，原目标未解决。本文件保留
> 方法与论文核对沿革；旧实验启动建议全部失效。读[整夜报告](ROPE_OVERNIGHT_EXPERIMENT_REVIEW_20260908.md)
> 及HANDOFF，再按作者最新要求审查完整材料、推导可区分预测和准备实验。
...
## 22:37 UTC 作者修正：先原理与1.5B最小验证

后文“完整13项后判断”已撤销。3B队列停止于9项；88行Mr比较建议未执行。
当前只执行[小模型协议](ROPE_QWEN15_MINIMAL_MECHANISM_20260907.md)：旧有效完整p2
的lag采样混叠有可复现来源，以固定数值修正版对照MrPro和同gain控制，保存真正
生成决策状态；禁止把背景统计下降或曲线合法当能力收益。零训练、LoRA、稀疏
注意力的顺序不变。所有旧矩阵描述均为历史记录，不是续接启动指令。
...
1. 先零训练：执行作者提供的[低频去载波](ROPE_CARRIER_REMOVAL_PILOT_20260907.md)，
   ...
   [逐槽原生相位约束候选](ROPE_NATIVE_SECTOR_CARRIER_20260907.md)作为实验3已停止于9项，
