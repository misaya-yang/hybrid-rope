...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{evq_warp_curves.pdf}
\caption{Effect of $\tau$ on EVQ frequency allocation ($N{=}64$). \textbf{(a)}~Warp curves $\phi_k(\tau)$ versus uniform quantile $u_k$: larger $\tau$ pulls low-index (high-frequency) positions upward, concentrating density in the high-frequency band. At $\tau{=}0$ the curve coincides with the diagonal (geometric RoPE). \textbf{(b)}~Deviation $\Delta\phi_k=\phi_k(\tau)-u_k$ quantifies the redistribution magnitude; the peak occurs at the high-frequency end and grows monotonically with $\tau$, directly controlling the interference--resolution trade-off formalised by the waterbed inequality~\eqref{eq:waterbed}.}
\label{fig:evq_warp}
\end{figure}

Figure~\ref{fig:evq_warp} visualises the effect: as $\tau$ increases from $0$ (geometric) to $2.0$, the warp progressively enriches high-frequency channels at the expense of mid-band resolution---the precise trade-off quantified by the waterbed inequality in Section~\ref{sec:waterbed}.
