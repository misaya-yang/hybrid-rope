...
For fixed target endpoints $(a',R')$ and native log-frequencies $x_k^N$,
\[
D=\sum_k(x'_k-x_k^N)=Ka'+R'\sum_k z'_k-\sum_k x_k^N.
\]
Thus total displacement is determined by the allocation once the native table
and target range are fixed. Matching it selects a subset of allocations for
a finer comparison; the fixed-range intervention already identifies the effect
of changing the complete internal configuration.
...
\paragraph{Arms.} \emph{Matched geometric control (Geo):} fixed training support,
...
\]
Here $z_k=k/(K-1)$ for Geo and the anchored Cosh shape for its paired
...
length, whereas target-matched range favors the geometric control. The paired design thus
...
provide paired Geo and anchored-\evq{} checkpoints whose training tables
...
\caption{\textbf{Allocation changes training-window and extended-length loss jointly.} Each point is one of the twelve prespecified M4 configurations, averaging three paired seeds; all four nonuniform main arms are shown. Both axes are differences from the matching geometric arm, so negative values improve loss. OOD weights are proportional to $\log_2(r+1)$ for $r\in\{2,4,8\}$, as in the original protocol. The horizontal coordinate is reconstructed from recorded four-decimal PPL; the vertical coordinate uses the recorded full-precision weighted NLL. The configurations form a prespecified design grid.}
\label{fig:m4-operating-points}
\end{figure}

The joint view shows simultaneous training-window and OOD improvements at multiple operating points. The measured trade-offs vary across allocation shapes, strengths and model configurations.
...
These inverse frequencies are learned without fixing the sampled endpoints, so this comparison varies realized support as well as interior placement. This parameterization comparison uses $15$M training tokens.

\subsection{Historical native-grid shape control}
\label{sec:native-grid-control}
A separate three-seed study compares the native standard-grid RoPE reference with a span-matched native-grid Cosh table. It uses $151{,}898{,}880$ parameters, base $500{,}000$, head dimension $64$, training length $128$ and $14{,}974{,}976$ tokens. The same study compares additional analytic and kernel-derived shapes at matched span and RMS deformation. Selection uses $16$ anchors and evaluation $32$ disjoint test anchors. This is a short-training shape study, separate from the $500$M-token fixed-support replication.
\begin{table}[ht]
\centering\small
\caption{Historical native-grid three-seed mean tail NLL. The complete length response retains the training window.}
\begin{tabular}{@{}lrrrrrrr@{}}
\toprule
Table & $128$ & $256$ & $512$ & $1$K & $2$K & $4$K & $8$K \\
\midrule
Native Std-RoPE & 5.5046 & 5.5026 & 5.6563 & 5.8456 & 6.0530 & 6.2566 & 6.3928 \\
Native Cosh & 5.4645 & 5.4473 & 5.5434 & 5.6966 & 5.8457 & 6.0664 & 6.2935 \\
\bottomrule
\end{tabular}
\end{table}
The recorded seed-level $95\%$ intervals for Cosh-minus-Std-RoPE NLL at $512/1$K/$2$K/$4$K/$8$K are $[-0.200,-0.026]$, $[-0.183,-0.115]$, $[-0.241,-0.174]$, $[-0.260,-0.121]$, and $[-0.141,-0.058]$. All three seeds favor Cosh at each of those lengths. This supports the conclusion that the historical extrapolation improvement is not confined to a midpoint geometric comparator. The same report also includes other useful nonuniform shapes. The source archive preserves the reported values and historical source identity in \path{figs/allocation_value_inputs.json}; the original training streams are separate.
\FloatBarrier

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_evidence_overview.pdf}
\caption{\textbf{Allocation gains depend on the support policy.} (a,b) Cosh-minus-Geo tail NLL for the same three paired $151.9$M models: retargeted support ($b=L_{\rm eval}$) versus retained training support ($b=256$). Thin lines show seeds; thick lines their means; $1\times=256$ tokens. Geo is the matched geometric control. (c) Frozen matched-support tasks: OLMo's nine held-out and Qwen's four development tasks, $20$ inputs/task, with shared endpoints, amplitude, weights and inputs within each model.}
