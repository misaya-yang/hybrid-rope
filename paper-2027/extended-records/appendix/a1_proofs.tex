...
\caption{\textbf{Slow pairs share positional directions.} Full-pair canonical overlap on $b=256$, $K=32$, $L=256$ under uniform separations. The slowest eight pairs have $r_2=2.11$ despite $\omega L\in[1.19,4]$.}
...
This is the discrete specialization of Theorem~\ref{thm:obstruction}, using
standard similarity rather than a new classification of position encodings.

\paragraph{Counterexamples and excluded extensions.}
A single trace is insufficient: $(\arccos .9,\arccos .1)$ and
$(\arccos .7,\arccos .3)$ have the same one-step trace, but different
rotation spectra and different two-step traces. Outside $(0,\pi)$, replacing
$\omega$ with $\omega+2\pi$ preserves every integer-position rotation while
changing rotations at fractional positions. Frequencies at zero or $\pi$
introduce real eigenvalue blocks; a partial rotary operator adds an identity
subspace. The stated corollary excludes these cases. Nonlinear Q/K
normalization need not commute with a candidate linear compensation.
Neither exact non-equivalence nor a slot-permutation loss proves that
retraining cannot adapt. The supplied CPU checker verifies compensation,
repeated frequencies, same-trace/different-spectrum examples, integer
aliasing, and the limits of a scalar gain; it measures no model performance.
...
Let $S_\rho(t)=\int_t^1\rho(\phi)\,d\phi$. The Cosh density-design objective is
\begin{equation}
\Capp[\rho]=\frac\alpha2\int_0^1\rho(\phi)^2\,d\phi+
\frac\beta2\int_0^1S_\rho(t)^2\,dt,
\qquad \int_0^1\rho=1,\quad\rho\ge0.
\label{eq:Capp}
\end{equation}
With unit mass, the first term favors spreading frequencies. Moving density
toward slower exponents increases tail mass over the thresholds it crosses;
the second term penalizes that concentration. These terms specify the
extrapolation transport prior.

\begin{theorem}[Unique density minimizer]\label{thm:ode}
For $\alpha>0$ and $\beta\ge0$, the unit-mass nonnegative $L^2$ minimizer of Eq.~\eqref{eq:Capp} is uniquely $\rho_\tau(\phi)=\tau\cosh[\tau(1-\phi)]/\sinh\tau$, with the uniform limit at $\tau=0$.
\end{theorem}
This subsection proves the theorem directly from the stated
...
\end{equation}
\paragraph{Existence.}
View $\mathcal{C}_{\mathrm{app}}$ as a functional on $L^2([0,1])$ and minimize over $\mathcal{A}=\{\rho\in L^2([0,1]):\rho\ge0\ {\rm a.e.},\int_0^1\rho=1\}$. The $\alpha\|\rho\|_2^2$ term is coercive and weakly lower semicontinuous, the Green-kernel term is positive semidefinite and continuous, and the mass/nonnegativity constraints are weakly closed. A minimizer therefore exists by the direct method. Strict convexity gives uniqueness. The Euler solution derived below is strictly positive, so the nonnegativity constraint is inactive a posteriori and the classical $C^2$ derivation is justified.
...
\end{equation}
which establishes Theorem~\ref{thm:ode} and the density used in \S\ref{sec:construction}. Mass normalization enters the
...
\paragraph{Convexity / PSD of $\mathcal{C}_{\mathrm{app}}$.} For $\alpha{>}0$ and $\beta{\geq}0$, the objective is convex because both quadratic terms are positive semidefinite. The diagonal $\alpha{\int}\rho^2$ is manifestly PSD on $L^2$, and the Green-kernel cross term satisfies the identity
\begin{equation}
...
\FloatBarrier
\subsection{The density objective in interval coordinates}
\label{sec:cosh-gap-form}
For a strictly positive density, let $Q=F^{-1}$ and $h(u)=Q'(u)>0$ with $\int_0^1h=1$. Then $\rho(Q(u))=1/h(u)$ and $S_\rho(Q(u))=1-u$. Changing variables in the declared density objective gives the exact identity
\[
\mathcal C_{\rm app}=\frac12\int_0^1\left[\frac\alpha{h(u)}+\beta(1-u)^2h(u)\right]du.
\]
Thus Cosh can equivalently be viewed as an interval-allocation design. Its quantile derivative is
\[
h_\tau(u)=\frac{\sinh\tau}{\tau\sqrt{1+(1-u)^2\sinh^2\tau}}.
\]
This equivalent interval formulation connects density allocation to frequency spacing. TailSpline instead designs increments of native-relative displacement; the two objectives address learning and frozen deployment.
...
\FloatBarrier
\subsection{Balance of the density objective}
\label{sec:self-consistency}

The density objective $\mathcal{C}_{\mathrm{app}}$ contains two integral terms $T_1(\tau) = \int_0^1 \rho^2\,d\phi$ and $T_2(\tau) = \int_0^1\!\!\int_0^1 \rho(\phi)\rho(\psi)\min(\phi,\psi)\,d\phi\,d\psi$, where $\rho(\phi) = \tau\cosh(\tau(1{-}\phi))/\sinh\tau$ is the normalized cosh density.

\begin{theorem}[Density-objective balance]
...
\begin{proof}
The Green's function $g(\phi) = \int_0^1 \rho(\psi)\min(\phi,\psi)\,d\psi$ satisfies $g''(\phi) = -\rho(\phi)$ with $g(0) = 0$, $g'(1) = 0$ (from the $\min$ kernel structure; see~\S\ref{sec:proofs}). Define $h(\phi) = \rho(0) - \rho(\phi)$. Then $h'' = -\rho''$, and under the Cosh ODE $\rho'' = \tau^2\rho$ this gives $h'' = -\tau^2\rho$, with $h(0) = 0$ and $h'(0) = -\rho'(0)$.

Define $f(\phi) = \tau^2 g(\phi) - h(\phi)$. Then $f'' = \tau^2 g'' - h'' = -\tau^2\rho - (-\tau^2\rho) = 0$, so $f$ is linear in $\phi$. The boundary values give $f(0) = \tau^2 g(0) - h(0) = 0$ and $f'(0) = \tau^2 g'(0) + \rho'(0) = \tau^2 - \tau^2 = 0$ (using $g'(0) = \int_0^1\rho = 1$ and $\rho'(0) = -\tau^2$ from the Cosh density). Hence $f \equiv 0$, yielding $\tau^2 g(\phi) = h(\phi) = \rho(0) - \rho(\phi)$, i.e.,
\begin{equation}
...
Cosh family is varied only through $\tau$; (ii) full-RoPE MHA has
...
\end{aligned}
\end{equation}
The stiffness itself also has an exact expression:
\begin{equation}
S_{\chi^2}(\tau)=\frac1{\dhd}\left[\frac{\sinh\tau\,\arctan(\sinh\tau)}{\tau^2}-1\right].
\label{eq:stiffness-exact}
\end{equation}
Indeed, unit mass reduces its defining integral to $\int\rho_\tau^{-1}-1$, and integrating $\operatorname{sech}$ gives Eq.~\eqref{eq:stiffness-exact}. The quartic term is a local expansion: at $\tau=\sqrt2,2,4$, it exceeds the exact stiffness by factors approximately $1.52,1.97,3.52$. It is not a finite-$\tau$ accuracy guarantee for the operating rule.
