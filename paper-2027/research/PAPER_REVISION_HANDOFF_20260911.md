...
- ⚠ 数学校验（改稿必须带；库内登记处=外部 Pro 评审 `external-reviews/pro-materials-20260908/RoPE_ICLR2027_Major_Revision_20260906.md` §5.2，含同式推导）：核心实验 min ωW = 256·256^(−31/32) = **1.1892 > 1**（网格约定 k/K 已对代码链核实，非 endpoint-inclusive）⟹ "极慢频 rank 塌缩"渐近**不覆盖核心实验**（无通道 ωW≤1）；理论表述必须用一般有限 Gram 弱方向，不能用慢频极限特例充当解释。
