...
状态：**已按作者确认的方案应用到主稿**。本次先读取服务器已完成报告及对应原始生成行，随后完成论文整合；未改动远端任务。服务器文件时间已到9月16日，本文件沿用本轮研究目录的9月15日标识。
...
- TailSpline承担外推质量提升：完整窗口表呈现L/2L/4L，正文自然说明轻微原生窗口损失。
  不把TailSpline的8K结果写成赢原始Native；不反复使用“赢MrRoPE”式措辞，方法比较由表图清楚展示。
...
**主文写法：TailSpline提升长上下文任务质量，同时仅有轻微的原生窗口性能损失。** L/2L/4L三点T/P差值+2.76、+3.39、+11.72pp由表图呈现；8K表内保留原始RoPE参考，不把相对MrPro改善误写成原生增强，也不另设代价小节。
...
本轮尚未形成独立NCP LM健康报告的状态只记录在研究材料，正文不额外插入待办或自我否定段落。
任务收益按已测指标表述，不扩写为全部语言能力提升。
...
1. **摘要**：维持无数字；写清跨模型外推质量、自然QA与等位移证据，并用一句纳入NCP的in-window任务收益。TailSpline的原生表现可自然表述为轻微损失，不能写成原生增强。
...
4. **§6主结果**：按“TailSpline窗口质量、跨模型与自然问答 → 等位移配置证据 → NCP原生窗口小节 → Cosh辅助学习/外推”组织。Llama L/2L/4L表完整展示；自然说明轻微原生窗口损失，不另设代价小节。
5. **附录**：新增OLMo逐任务/输出行为、clean T/C、clean Native诊断、LongBench v2、NCP推导与完整任务分析。NCP主要结论在正文；旧classic保留其历史合同。
6. **同步更新现有判断**：摘要、引言、结论体现z的extension和in-window作用；TailSpline性能与NCP原生增强分别归属各自构造。原“Native-window trade-off”段落并入统一长度评价，改为简洁的轻微损失描述。
