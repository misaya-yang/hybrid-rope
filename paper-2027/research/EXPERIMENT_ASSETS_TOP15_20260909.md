...
|---|---|---|---|
| 1 | **151.9M 三种子固定范围训练**：固定频率端点后，内部指数分配改变外推表现。 | 499,974,144 tokens/arm；seeds 42/137/256；固定范围 Cosh−Geo NLL 在 512/1K/2K 为 −0.281/−0.176/−0.146，9/9 seed×length 同向；256 为 +0.026。 | 核心识别证据，首图与正文 §4.1。target-retargeted 反转是同一实验的部署分析，保留完整结果而不另计。 |
...
| 3 | **成熟冻结模型同范围分配对照**：已有权重下，仅改变内部指数即可大幅改变 RULER 表现。 | 相同 endpoints/gain/inputs/decoder；OLMo held-out 9×20 rows：geometric/ramp/residual 0.56/61.04/60.47%；Qwen development 4×20：57.75/64.00/66.50%。 | 正文 §5。主要结论是 allocation recovery，不能把 residual 与 ramp 的差异称作已分离的优越性。 |
...
| 7 | **50.9M M4 固定范围 factorial**：非均匀分配效应跨 base、训练长度和 head dimension 存在。 | 12 configurations×3 seeds，180主臂+12边界臂；预指定1.25×Cosh改善10/12，deformation-matched Exp改善9/12；Cosh-rule 与 Exp 的差异未分离。 | 正文一段、附录完整表。保留多构造结果，tau multipliers/Exp/边界检查属于同一实验族。 |
...
| 13 | **1.485B OLMo 从公开step-0初始化训练**：分配的长程作用延续到约1.5B训练规模。 | 2.097B counted tokens，128 document-disjoint PG-19 docs；16K PPL182.73→159.64，126/128 documents NLL同向；2K/4K有小幅成本。 | 补入正文一句，附录完整图。Geo用AI2分布式trainer、EVQ用HF单GPUloop；同初始化/recipe不等于bitwise配对轨迹。 |
