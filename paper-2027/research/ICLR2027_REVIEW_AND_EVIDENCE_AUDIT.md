...
> NeurIPS-withdrawal / dual-submission half (§4.1a item 12) is not, and the
...
`paper-2027/research/ICML2027_RESEARCH_SYNTHESIS_20260819.md` §11 says the *next*
...
| `research_notes/iclr2027/02_PAPER_REVISION_PLAN.md` | The August plan: reposition to "substrate + mechanism + protected retrofit" | — | Superseded by `ICML2027_RESEARCH_SYNTHESIS_20260819.md` | (c) Historical. Note that its Pillar iii (P-EVQ) never ran, and the synthesis memo correctly dropped it |
...
12. **Dual submission.** `research_notes/iclr2027/00_README.md` §3 item 6 and
    `SUBMISSION_CHECKLIST.md:60-66`: NeurIPS notification is 2026-09-24; the ICLR
    abstract deadline is 2026-09-18. Submitting before withdrawal is a policy
    violation that forfeits the objective outright. **This is an author decision
    and the only item in this audit that must be resolved before any writing
    effort is spent.**
...
|---|---|---|---|
| **1** | **Resolve the venue.** Switch to `iclr2027_conference.sty`, relax the body to 9 pages, replace the Impact Statement with an AI-use statement — *and settle the NeurIPS-withdrawal question before spending any further writing effort.* | `main.tex:30,111-112`, `sections/06_impact.tex`, `SUBMISSION_CHECKLIST.md` | Everything else is contingent on this. It also frees one full body page, which is the budget for items 2-5. |
| **2** | **Add the exact post-hoc transplant obstruction as a numbered proposition** (`Aᵀ R(ω′Δ) B = R(ωΔ) ∀Δ ⇒ |ω′|=|ω|`), and use it to reframe the mature-model in-window cost as a *predicted* consequence rather than a defect. | `sections/03_theory.tex` (new subsection + `\label{prop:transplant}`), `sections/05_discussion.tex`, `appendix/a1_proofs.tex` | The paper's biggest apparent weakness becomes a theorem-backed structural result. `ICML2027_RESEARCH_SYNTHESIS_20260819.md` §12 ranks this #1; `INWINDOW_PRESERVATION_ROUTE_AUDIT_20260731.md` §5 gives the exact five-step framing. Zero GPU, ~15 lines, proof already written. |
