...
The denominator turns the spectral budget into an exact dimension count:
...
construction: when the derived allocation is evaluated under the closed-form
exact cosine-feature kernel, collision falls by $24$--$92\%$ and its Shannon
...
tractable surrogate, and the exact-kernel diagnostic preserves its predicted
improvement direction. Shape-matched trained controls below show that the gain
comes from non-uniform allocation rather than a curve-specific artifact.

\paragraph{A one-line full-RoPE default.} With $\deff$ the effective
...
$\deff/\sqrt{\Ltr}$ scaling supplies a reference family for standard full-RoPE
text settings, with $\deff{=}d_{\mathrm{head}}$. We fix $c(\Pi){=}1$ before
training as its zero-search default and evaluate it against Geo and neighbouring
multipliers
