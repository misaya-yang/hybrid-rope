...
\S\ref{sec:construction} supplies one reproducible allocation, not a substitute
for either geometry or trained-model evidence.
...
summary of Shannon rank or log-determinant. Block whitening also removes
per-channel energy and conditioning, leaving trained Q/K coefficients outside
the statistic. The exact-range experiment of \S\ref{sec:exp-identify} therefore
does not assume that $\bar c$ predicts loss; it tests the trained effect directly.
...
This common long-context failure mode motivates allocating a finite table, but
it is not asserted to be the specific mechanism of the $b{=}256$ exact-range
control; the full subspace geometry applies away from the asymptotic threshold.
...
stays within a $0.75\times$--$1.5\times$ window around that value. The deployed
