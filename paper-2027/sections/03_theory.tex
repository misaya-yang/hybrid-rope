...
describe almost the same positional directions. Full-subspace geometry makes
that tax phase invariant and applies directly to the $b{=}256$ exact-range
control.
...
log-frequency span. Trained-model controls below evaluate the resulting table.

\paragraph{A one-line frequency table.} With $\deff$ the effective head
dimension and $\Pi$ the training protocol,
\begin{equation}
...
$\deff/\sqrt{\Ltr}$ scaling ties its strength to architecture and training
length; the main text protocols fix $c(\Pi){=}1$ before training. Supporting
sweeps evaluate this zero-search operating prior against Geo and discrete
neighbouring multipliers (\S\ref{sec:exp-tau}). The deployed midpoint table has
