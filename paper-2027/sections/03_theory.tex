\section{Geometry and a Closed-Form Allocation}
\label{sec:theory}
...
\subsection{Why the slow end admits reallocation}
...
This suggests redistributing some exponent samples toward faster rotations
while retaining coverage of the range. Frequency-use studies also show that
...
\subsection{Constructing an exponent distribution}
\label{sec:construction}

Let a positive density $\rho\in C^2([0,1])$, with $\int_0^1\rho=1$,
allocate samples over the exponent interval. Uniform $\rho$ recovers geometric
spacing. To balance concentration against redundant slow-end sampling, we
choose the convex objective
\begin{equation}
\Capp[\rho]
=\frac{\alpha}{2}\int_0^1\rho(\phi)^2\,d\phi
+\frac{\beta}{2}\iint_{[0,1]^2}
\rho(\phi)\rho(\psi)\min(\phi,\psi)\,d\phi\,d\psi,
\qquad \alpha>0,\ \beta\ge0.
\label{eq:Capp}
\end{equation}
The squared-density term discourages concentration in a narrow band.
The interaction penalty grows when both exponents lie toward the slow end.
This surrogate turns the preceding geometric observation into a tractable
allocation problem.
