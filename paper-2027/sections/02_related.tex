...
training-free extensions \citep{tian2026mrrope}. Jet-Long instead combines a
Native local window with a dynamically rescaled remote window, using an
inclusion--exclusion attention merge and on-the-fly cache correction
\citep{tang2026jetlong}. These methods transport a learned model to longer
contexts. Our frozen-checkpoint corollary retains one session-static table and
standard rotary attention; its claim is fixed-support sensitivity to $z$, not
a new dynamic extension operator. Bounds based on the RoPE base characterize
limits of this regime \citep{xu2024base}.
