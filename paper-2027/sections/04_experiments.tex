...
The companion target-retargeted condition changes the preferred allocation,
confirming that support and allocation are distinct but interacting coordinates
...
three head dimensions, and three paired seeds. A $1.25\times$ Cosh warp and a
deformation-matched exponential favour non-uniform allocation in $10/12$ and
$9/12$ structural configurations (App.~Table~\ref{tab:m4}). The $151.9$M
control supplies the replicated direct effect; the factorial tests the broader
prediction that reallocating finite support matters across analytic shapes.
Full results, including the formula point and neighbouring multipliers, appear
...
The training-time table changes what the same range operator can recover. In a
$454$M three-seed MHA study, a fixed \rs{} operator raises $8$K teacher-forced
...
At fixed support and attention amplitude, changing only interior $z$ raises
macro score from $0.56\%$ to $60.47\%$ on OLMo-2 at $16$K and from $57.75\%$
to $66.50\%$ on Qwen2.5 at $64$K. These released-checkpoint interventions show
that allocation remains actionable after pretraining
...
shows that the improved long-position probability depends on the remote source
rather than a local readout artifact. The task-family continuation is reported
