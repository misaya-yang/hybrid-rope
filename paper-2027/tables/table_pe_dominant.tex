...
is $3$-seed (42/137/256). Allocation-shape attribution rests on the
zero-parameter fixed schedules of \S\ref{sec:exp-identify}: the
learned-inv-freq arm carries $32$
trainable parameters, so any gap confounds allocation shape with
parameterisation and optimisation effort. The arm is a layer-shared learnable
...
\citep{karypis2026lerope} rather than the learned positional-\emph{operator}
