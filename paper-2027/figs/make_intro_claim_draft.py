...
The draft is not yet included in the manuscript.
"""
...
plt.rcParams.update({'font.family':'serif','font.serif':['Times New Roman','DejaVu Serif'],
 'mathtext.fontset':'stix','font.size':11,'svg.fonttype':'none','axes.linewidth':.7})
fig=plt.figure(figsize=(9.3,4.1),facecolor='white')
fig.text(.035,.955,'(a) One frequency range, three allocations',fontsize=13,fontweight='bold',color=INK)
fig.text(.545,.955,'(b) Different allocation of distance stretch',fontsize=13,fontweight='bold',color=INK)
fig.text(.035,.87,'Llama-3-8B public grid: native 8K, extension factor 4',fontsize=10.5,color='#59636D')
ax=fig.add_axes([.05,.41,.38,.38])
for y,(name,values) in zip([2,1,0],tables.items()):
    ...
    ax.text(0,y+.25,name,fontsize=11,color=color,fontweight='bold')
...
ax.set_xlabel('Rotary frequency (log scale)',fontsize=11,labelpad=6)
for side in ['left','top','right']:ax.spines[side].set_visible(False)
ax.tick_params(axis='x',labelsize=10,length=3)
fig.text(.05,.12,'Same endpoints and outer bands.\nColored dots: different interior frequency placements.',fontsize=10.5,linespacing=1.35,color=INK)

ax=fig.add_axes([.605,.41,.34,.40])
for name in tables:
...
ax.set_xlabel('Channels in the transition band',fontsize=11,labelpad=6)
ax.set_ylabel(r'Distance stretch $r_k=\omega_k^{N}/\omega_k^{\prime}$',fontsize=11,labelpad=6)
ax.legend(frameon=False,loc='upper left',fontsize=9.5,handlelength=2)
ax.tick_params(axis='both',labelsize=9.5,length=3)
for side in ['top','right']:ax.spines[side].set_visible(False)
fig.text(.77,.20,r'$\Delta\,\omega_k^{\prime}=(\Delta/r_k)\,\omega_k^N$',ha='center',fontsize=14,color=INK)
fig.text(.77,.11,'A physical gap '+r'$\Delta$'+' is mapped to native distance '+r'$\Delta/r_k$'+'.',ha='center',fontsize=10.5,color=INK)
fig.text(.5,.027,'Internal allocation redistributes distance responses within the same frequency range.',ha='center',fontsize=12,fontweight='bold',color=INK)
fig.savefig(HERE/'fig_intro_claim_draft.png',dpi=240,bbox_inches='tight',pad_inches=.07)
fig.savefig(HERE/'fig_intro_claim_draft.svg',bbox_inches='tight',pad_inches=.07)
plt.close(fig)
receipt={'status':'conceptual_draft_not_installed','scope':'Analytic frequency allocation and phase-equivalent distance; no task scores, no model evaluation, no claim of FP32 receipt identity.',
...
(HERE/'intro_claim_draft_inputs.json').write_text(json.dumps(receipt,indent=2)+'\n')
