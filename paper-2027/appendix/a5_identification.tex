...
\begin{figure}[ht]
  ...
  tested target-matched policy reverses the preferred allocation.}
...
Under the tested target-retargeted policy, the ordering reverses. The two
columns therefore establish that the fixed-support result is conditional on
the selected range; they do not identify a support-by-allocation interaction
law. The \texttt{fixed} column isolates allocation when one table serves
...
\section{The \rs{} operator}
\label{sec:ramp-scaler}

Every arm marked $+\rs{}$ uses the repository's fixed-index operator,
inspired by the NTK-by-parts/YaRN family \citep{peng2024yarn}:
high-frequency channels are left nearly unchanged and lower-frequency channels
receive progressively more range scaling. The implementation, cut indices,
and scale $s$ are held fixed across the compared arms. Because the underlying
frequencies differ, the realised phase movement is not matched channel by
channel. The result is therefore a composition test of the same index-space
rule on two training-time tables, not a movement-matched mechanism control.
