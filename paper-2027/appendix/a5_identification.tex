...
Retargeting support changes absolute phase coverage and therefore changes which
interior allocation is preferred. This is the expected interaction between the
two coordinates: target-specific support should be selected jointly with
allocation, while the \texttt{fixed} column isolates allocation when one table
serves multiple lengths (\S\ref{sec:exp-identify}).
