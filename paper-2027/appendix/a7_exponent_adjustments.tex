...
\caption{\textbf{Fixed-support identification and natural-QA use are separate comparisons.} (a) Frozen OLMo $16$K scores on nine held-out RULER tasks ($20$ rows/task), with endpoints, amplitude, weights and inputs shared. (b) Matched BM/MrRoPE-Pro $s=4$ natural QA on $631$ untruncated inputs longer than $4$K, using whole-response token F1. The two panels retain their own task and metric definitions.}
