...
\S\ref{sec:exp-frozen-support} and the BM natural-QA comparison in \S\ref{sec:experiments}. An amplitude $g$ multiplies both the cosine
...
\paragraph{YaRN implementation coordinate.}
The index-linear YaRN baseline uses
$r_k=\operatorname{clip}((k-l)/(h-l),0,1)$ and
$\omega'_k=\omega_k(1-r_k+r_k/s)$, with the recorded correction indices and separate cosine/sine gain. This is the channel-index ramp in the author implementation (revision \texttt{995db5b}) and its Transformers implementation \citep{peng2024yarn}. It is distinct from the historical smoothstep scaler in Appendix~\ref{sec:experiment-details} and the MLA wavelength blend in Appendix~\ref{sec:mla-results}.
...
Transformers index-linear YaRN table and $g=1+0.1\log2=1.0693147181$;
Native uses $g=1$.
...
panel with seed $20260909$; BM's formula was fixed before the confirmation.
Its $4$K/$16$K scores are listed in Table~\ref{tab:bm-models}.
MrRoPE-Uni and official YaRN were subsequently evaluated on these same
inputs, scoring $76.88/32.12\%$ and $54.38/6.94\%$. These are follow-up
controls on the BM--MrPro confirmation panel, not new independent confirmations.
