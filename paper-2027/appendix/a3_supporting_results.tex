...
$\drot{=}\dhd$ convention in Eq.~\eqref{eq:evq-practical}, this
experiment tests frequency allocation at fixed architecture rather than the
operating rule.

The tracked aggregate is backed by the exact recovered three-seed evaluator
JSON and retains every seedwise row. At $16$K, Geo PPL is
$141.1/132.5/142.8$ for seeds $42/43/88$, whereas \evq{} is
$93.7/92.7/100.3$; all three paired directions agree. Original checkpoint and
training-data artifacts are not packaged, so this row establishes the reported
system ordering, not checkpoint-level reproduction or a theorem fixing
$\tau$ from $d_{\mathrm{rope}}$.
...
ordering for this architecture and protocol; the multiplier overlay is the
run-specific MLA wavelength-blend operator, not the paper's pure-$z$ control or
an exact reproduction of another scaling method.
