...
For $\tau>0$, the Cosh density is decreasing, so $Q_\tau$ is convex. Affine
endpoint anchoring on equally spaced quantile nodes gives
\begin{equation}
z_{\tau,k}=\frac{Q_\tau(u_k)-Q_\tau(u_0)}{Q_\tau(u_{K-1})-Q_\tau(u_0)}
\le\frac{k}{K-1},\qquad\omega_{\tau,k}\ge\omega_{\mathrm{Geo},k}.
\label{eq:anchored-cosh-order}
\end{equation}
The inequality follows from convexity relative to the endpoint chord.
...
Table~\ref{tab:allocation-protocols} records allocation and deployment parameters; Table~\ref{tab:hyperparams} gives training settings.
...
the operator used in this experiment, separately from the index-linear YaRN
comparisons in Appendix~\ref{sec:four-method-control}.
...
All five configurations improve long PPL, with the largest relative effect in the eight-pair MLA setting. The ordering across architectures is nonmonotone: KV sharing, latent compression, and rotary-pair count jointly define the setting. These results complement the three-seed $432$M study by demonstrating allocation benefits across several attention architectures.
\FloatBarrier
