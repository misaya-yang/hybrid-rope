...
Cross-modal scope & 129.6M video DiT & temporal Geo/\evq{} & 1 training seed & denoising MSE & \ref{sec:video-dit} \\
\bottomrule
\end{tabular}
}
\end{table}
...
arms. The seed-42 Geo and \evq{} ($\tau{=}1.5$) branches are trained in the
same head-to-head run.
...
\begin{table}[ht]
\caption{DiT head-to-head evaluation (129.6M, seed 42, bidirectional attention, 3D RoPE). Values are denoising MSE at $t{=}0.5$.}
\label{tab:dit-h2h}
\centering
\small
