...
\midrule
Allocation identification & 151.9M scratch & fixed support, $30$ interiors & 3 training seeds & tail NLL & \ref{sec:exact-range-control} \\
Shape/configuration breadth & 50.9M factorial & fixed support, 5 allocations & 12 configs $\times$ 3 seeds & weighted OOD NLL & \ref{sec:exact-range-factorial} \\
Scarce rotary budget & 432M MLA-style & Geo/\evq{}, $K{=}16$ & 3 training seeds & PPL & \ref{sec:mla-results} \\
Full-parameter scale & 750M/1.485B & continued/from-init Geo/\evq{} & one matched pair each & PPL/retrieval & \ref{sec:larger-scale-supporting}/\ref{sec:olmo2-1b} \\
Frozen allocation & OLMo/Qwen & same support and amplitude & 20 rows/task & RULER & \ref{sec:frozen-fixed-support} \\
Adaptation/source use & 1.485B/8B & matched LoRA/source block & one trained pair & QA/RULER/NLL & \ref{sec:olmo2-1b}/\ref{sec:llama8b} \\
Cross-modal scope & 129.6M video DiT & temporal Geo/\evq{} & 2 training seeds & denoising MSE & \ref{sec:video-dit} \\
\bottomrule
\end{tabular}
}
\end{table}
...
arms. For each seed, the Geo and \evq{} ($\tau{=}1.5$) branches are trained in
the same head-to-head run.
...
\begin{table}[ht]
\caption{Base${}=1{,}000$ DiT head-to-head (129.6M, seed 42, shared temporal range transform). The two \evq{} settings are within $0.4\%$ on the reported metrics in this run.}
