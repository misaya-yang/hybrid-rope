...
$2\times$ and $24.97/25.59\%$ at $4\times$, so the result is not universal
task dominance. The QA arms use identical rows, and every post-chat input plus
generation reserve fits within $16$K. Figure~\ref{fig:frozen-fixed-support}
remains the allocation-only contrast.
