...
The mature-checkpoint intervention in Fig.~\ref{fig:frozen-fixed-support}
...
$m_k=(1-\widetilde U_k)^2$ in Eq.~\eqref{eq:movement-allocation}, so less unique
pairs move farther.
...
the values plotted in Fig.~\ref{fig:frozen-fixed-support}. Reference rows are
...
complete deployment policy on natural context; Fig.~\ref{fig:frozen-fixed-support}
