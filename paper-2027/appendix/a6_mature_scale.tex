...
same-support log-linear arm is uniform in exponent space between the endpoints.
The movement-profile ramp is the discrete YaRN-family linear ramp minimising
...
recovery. The derived-minus-log-linear intervals are $[54.88,64.80]$ points for
OLMo and $[0.25,17.50]$ points for Qwen; the paired
derived-minus-movement-profile-ramp intervals contain zero on both models. The
...
learned parameters or training tokens. On a disjoint $128$-document
FineWeb-Edu evaluation, forcing the same long profile at $4$K gives final-$1024$
NLL $2.8774$ versus Native $2.7538$, a difference of $+0.1236$ NLL; the
session policy routes $4$K requests through Native. At $8/16$K, its PG-19 tail NLL is
$3.1060/3.0977$, compared with $3.4404/3.7955$ for official Transformers YaRN
factor four. On full-$200$ Qasper \citep{dasigi2021qasper} and 2Wiki
\citep{ho2020twowiki}, the routed policy's token F1 is $24.57/26.66\%$, versus
$18.03/25.69\%$ for YaRN; applying the long profile to all $200$ 2Wiki rows
scores $27.74\%$. Across the six-task natural-context matrix, the routed method
scores $28.58/24.97\%$ at $2/4\times$, compared with $21.27/25.59\%$ for YaRN.
The QA arms use identical rows, and every post-chat
input plus its generation reserve fits within $16$K. These rows evaluate the
complete deployment policy on natural context; Table~\ref{tab:frozen-fixed-support}
provides the allocation-only contrast.
