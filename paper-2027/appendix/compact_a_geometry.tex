...
is invariant under invertible changes of basis within either pair,
including phase rotation, as shown below.

\paragraph{Basis invariance.}
Under independent invertible basis changes $x_\omega\mapsto x_\omega A_\omega$ and
$x_\nu\mapsto x_\nu A_\nu$, let
...
\begin{theorem}[Complete-pair effective-rank identity]
\label{thm:budget}
...
\end{equation}
with $\epsilon=\max\{|x|,|y|\}\to0$. At $x=0.05,y=0.10$, the analytic
Gram gives exact/leading ratio $1.00058$.
...
Closed-form Gram evaluations agree with independent numerical quadrature.
These examples make the roles of complete pairs and evaluation length explicit.
...
\paragraph{From distance response to content competition.}
