...
The finite-$\tau$ evidence tests a configuration-independent zero-search prior
rather than a per-configuration point optimum. In a staged 99-run study, the
...
Their ordering varies across configurations. These are tested multipliers, not
bounds on a continuous basin. The evidence supports $c{=}1$ as a useful
zero-search prior relative to that Geo baseline; it does not identify a
per-configuration point optimum.
...
a possible attention-conditioned refinement rather than a consequence of the
KL identity. The empirical claims in this paper use the registered
length-based operating rule.
