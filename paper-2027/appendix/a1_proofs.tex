...
\begin{theorem}[Non-identifiability of downstream capability from low-dimensional frequency metrics]
...
Let $\Phi: \mathbb{R}_+^K \to \mathbb{R}^p$ be any structural metric depending solely on the rotary frequency schedule $\Omega$.
Then downstream functional capability $\mathcal{R}(\Omega; \mathcal{M}, \mathcal{D})$ is strictly \textbf{non-identifiable} from $\Phi(\Omega)$ across three distinct mathematical fibers:
\begin{enumerate}
  \item \textbf{Type-I Non-Identifiability (The Permutation Fiber / Degeneracy of Identity):}
  There exist frequency tables $\Omega_A, \Omega_B \in \mathbb{R}_+^K$ such that $\Phi(\Omega_A) = \Phi(\Omega_B)$, yet their downstream risk diverges catastrophically:
  \begin{equation}
  \left| \mathcal{R}(\Omega_A; \mathcal{M}, \mathcal{D}) - \mathcal{R}(\Omega_B; \mathcal{M}, \mathcal{D}) \right| \ge \Gamma_{\mathrm{collapse}} > 0.
  \label{eq:fiber1-statement}
  \end{equation}
  \item \textbf{Type-II Non-Identifiability (The Discontinuity Fiber / Metric Discontinuity):}
  There exist frequency tables $\Omega_C, \Omega_D \in \mathbb{R}_+^K$ such that $\|\Phi(\Omega_C) - \Phi(\Omega_D)\| \ge M > 0$, yet their downstream risk is identical up to machine precision:
  \begin{equation}
  \left| \mathcal{R}(\Omega_C; \mathcal{M}, \mathcal{D}) - \mathcal{R}(\Omega_D; \mathcal{M}, \mathcal{D}) \right| \le \varepsilon_{\mathrm{machine}} \approx 0.
  \label{eq:fiber2-statement}
  \end{equation}
  \item \textbf{Type-III Non-Identifiability (The Hessian Knee Fiber / Extremal Curvature Ill-Conditioning):}
  For any coordinate distance $\mathcal{D}(\Omega, \Omega^*) = \|\mathbf{m}(\Omega) - \mathbf{m}(\Omega^*)\|_p$, there exists a continuous family of tables with $\mathcal{D}(\Omega_\epsilon, \Omega^*) = \epsilon \to 0$ that fail pre-registered in-window retention gates, while tables with $\mathcal{D}(\Omega', \Omega^*) \gg \epsilon$ pass.
\end{enumerate}
\end{theorem}

\begin{proof}[Proof of Theorem~\ref{thm:non-identifiability-appendix}]
We establish the three mathematical fibers sequentially.
...
\end{equation}
During pre-training, projection weights $(W_Q^{(k)}, W_K^{(k)})$ co-adapt to specific slot frequencies $\omega_k$: fast channels ($k_1$) develop large-norm projections for local syntactic dependencies and token identities, while slow channels ($k_2$) carry diffuse long-range semantic tracking.
Because $\omega_{k_1} \gg \omega_{k_2}$, the rotation operator difference satisfies:
\begin{equation}
...
\end{equation}
Transposing slots rotates fast syntactic features by near-zero phase velocities, completely erasing local token ordering, while rotating slow semantic features through rapid radians, destroying associational retrieval.
Expanding the risk functional around logits, the linear gradient term vanishes in expectation over data due to phase decorrelation, and the second-order expansion yields a strictly positive-definite loss penalty:
\begin{equation}
\Gamma_{\mathrm{collapse}} \ge \frac{1}{2} \sum_{k \in \{k_1, k_2\}} \mathbb{E}_{x \sim \mathcal{D}}\left[ \|W_Q^{(k)} x\|^2 \|W_K^{(k)} x\|^2 \right] \cdot \mathbb{E}_\Delta \left[ \sin^2\left( \frac{(\omega_{k_1} - \omega_{k_2})\Delta}{2} \right) \right] > 0.
\label{eq:gamma-collapse-bound}
\end{equation}
In verified repository receipts, permuting the 64-frequency multiset causes OLMo-1B $1\times$ PG-19 tail NLL to more than double from $3.10423$ to $6.86493$ ($\Delta\mathrm{NLL} = +3.760692$) and Qwen-2.5-1.5B 64K core-4 task macro score to plunge from $0.7000$ to flat $0.0000$ across all four benchmark tasks.
Since $\Phi(\Omega_A) = \Phi(\Omega_B)$ while $\Delta\mathcal{R} \ge 3.76$, downstream capability is non-identifiable from $\Phi$.
...
\FloatBarrier
\subsection{Elimination of local Taylor approximations, path-integrated bounds, and physical observables}
...
\end{equation}
The convergence radius of this polynomial series is strictly bounded by the phase condition:
\begin{equation}
\Delta\phi = |\delta\omega| \cdot L \ll 1.
\end{equation}
When evaluating long-context extensions ($L \ge 4{,}096$ to $131{,}072$), phase perturbations $\Delta\phi$ easily exceed $2\pi$.
While the true trigonometric difference is strictly bounded in $[-2, 2]$, the second-order Taylor term $\frac{1}{2}(L \delta\omega)^2$ grows quadratically without bound.

\begin{table}[ht]
\caption{Catastrophic breakdown of 2nd-order Taylor approximation under long-context phase wrapping for slot 19 movement error ($\Delta m_{19} = 0.044109$, $\delta\omega = -6.94 \times 10^{-4}$ rad/token).}
...
\paragraph{The six formal closed-loop physical observables and causal coordinates.}
...
\begin{enumerate}
  ...
  Channels with large $\rho_{h, k}$ carry primary semantic features and dominate loss sensitivity; channels with $\rho_{h, k} \approx 0$ tolerate aggressive frequency modification.
  ...
  \item \textbf{Observable 4: Softmax Attention Distractor Proliferation Ceiling $S_{\mathrm{dilution}}$.}
  For target retrieval needle $j^*$ in context extension $S = L / L_0$ with distractor set $\mathcal{D}_{\mathrm{dist}}$ of size $S L_0 - 1$:
  \begin{equation}
  ...
  \end{equation}
  where $B = s(j^*) - \mathbb{E}[s(i)]$ is the logit margin and $\mu = \mathbb{E}[\exp(s(i) - \mathbb{E}[s(i)])]$.
  Top-1 retrieval ($A(j^*) \ge 0.5$) forces $B \ge \ln S + \ln(L_0 \mu)$.
  Context extension strictly consumes $\ln 2 \approx 0.693$ nats of margin per octave.
  Once $\ln S$ exhausts the model's finite margin $B_{\max}$, retrieval abruptly crashes to zero (governing the $2\times \to 4\times \to 8\times$ cliff at $s \approx 4.01$).
  ...
  $(a, R, \mathbf{z})$ is an algebraic diffeomorphism that decouples support span from interior shape, demonstrating why retargeting $R(L) = ((K-1)/K)\log L$ non-linearly stretches Cosh spacings and reverses model rankings across evaluation lengths.
\end{enumerate}
