...
to $V_0$, rather than comparing the unwhitened Taylor coefficients.  Under the
...
on at least three distances.  This is a limit for a declared fixed measure; it
does not identify which slow channels a trained model uses.
\end{proof}
...
Proposition~\ref{prop:parity-lattice} attains the algebraic rank maximum on the
declared interval while imposing an exact recurrence just outside it.  It does
not say that $K_{\mathrm{par}}$ is a global capacity over arbitrary frequencies,
nor does it survive a change of measure: the causal separation histogram is
triangular rather than uniform and does not make the same lattice orthogonal.
The exact parity construction and the small-model checks are reproduced by
\texttt{analysis/full\_rope\_audit/verify\_small\_models.py}; the uniform-versus-causal
measure comparison is exposed by
\texttt{scripts/analysis/third\_axis\_ceiling.py --decompose}.  These are static
basis statements, not LM-quality or extrapolation predictions.

\subsection{Two static ordering counterexamples}
\label{sec:static-counterexamples}

Even within a fixed declared measure, a cheaper static proxy need not order
tables like the full basis, and an ordering at one interval need not persist at
another.  The following two finite tables are existence counterexamples; no
claim of continuous global optimisation is needed.

\paragraph{Cosine-only collision can choose the lower-rank table.}
Use $L{=}4096$, $b{=}500{,}000$, $K{=}64$, continuous uniform
$\Delta\in[0,L]$, and pin both physical endpoints to
$[2{\times}10^{-6},1]$.  Let $A$ be the best-found schedule returned by the
declared heuristic for cosine-only collision and $B$ the corresponding
best-found schedule for full-subspace collision, using their distinct
harmonic-augmented candidate pools.  The cosine proxy prefers $A$,
\begin{equation}
C_{\cos}(A)=2.62\times10^{-9}
 < 1.44\times10^{-8}=C_{\cos}(B),
\end{equation}
but the phase-invariant block-whitened basis has the opposite order,
\begin{equation}
r_2(A)=64.48 < 128.00=r_2(B).
\label{eq:cosine-order-counterexample}
\end{equation}
Here $C_{\cos}$ is the mean squared normalised cosine--cosine overlap.  The
counterexample isolates why one phase slice cannot replace the full two-plane
object: a small cosine overlap can hide redundancy in the sine and cross terms.

\paragraph{Full-subspace collision can reverse across lengths.}
Under the same base, endpoints, and training-interval convention, the tracked
counterexample search emits two fixed $K{=}16$ tables $A$ and $B$.  Writing
$C_M$ for their mean phase-invariant full-subspace collision when the uniform
measure spans $[0,M]$, their ordering is
\begin{align}
C_L(A)&=0.45837 < 0.61695=C_L(B),\\
C_{2L}(A)&=0.45834 > 0.41132=C_{2L}(B),\\
C_{4L}(A)&=0.45830 > 0.24763=C_{4L}(B).
\label{eq:length-order-counterexample}
\end{align}
Recomputing the same schedules on discrete uniform separations
$\Delta=0,\ldots,M-1$ preserves the reversal.  Thus even the correct
phase-invariant collision is conditional on the declared interval; minimising
it at $L$ does not supply an extrapolation ordering at $2L$ or $4L$.

\paragraph{Numerical owner and reproduction convention.}
The digits in
Eqs.~\eqref{eq:cosine-order-counterexample}--\eqref{eq:length-order-counterexample}
are frozen outputs produced by
\path{scripts/analysis/full_rope_collision_audit.py} with
\texttt{--length 4096 --base 500000 --candidate-grid 1025
--random-schedules 3000}; the counterexample RNG seed is $20260819$ and the
JSON stores each schedule and its float64 hash.  These are best-found points
returned by the declared greedy-plus-exchange heuristic in metric-specific
candidate pools, not certified pool or continuous optima.  The first
inequality is also reported with raw full entropy rank $93.44<120.03$; the
stable-rank ordering above is the quantity governed by
Theorem~\ref{thm:budget}.

Together, the parity recurrence and the ordering reversals separate three
questions: dimension over a declared interval, multiscale coverage outside it,
and behaviour after training.  The fixed-support experiments evaluate the last
question, while Table~\ref{tab:coadapt} shows why learned coefficients must stay
with the basis that trained them.
...
\subsection{Assumption-bound operating calculation and finite tested receipt}
\label{sec:lambda-cv}
\label{sec:tau-scaling}

The variational theorem selects the Cosh \emph{shape}; it does not select a
finite strength for every model or protocol.  The implementable reference used
in the standard full-RoPE text experiments is
\begin{equation}
...
\end{equation}
Here $c{=}1$ is the pre-training zero-search convention, not a fitted optimum;
each protocol records its realised $\tau$, grid, support, and rotary width.

\paragraph{Assumption-bound calculation.}
...
\end{equation}
Stationarity of
$\mathcal F(\tau)=\tfrac12S_{\chi^2}(\tau)-\lambda U(\tau,L)$ gives only the
leading balance
\begin{equation}
...
\end{equation}
Thus $\dhd/\sqrt L$ is an assumption-bound scaling factor, while the
coefficient remains utility-, base-, and convention-dependent.  The reported
experiments use the exact Cosh warp, including where small-$\tau$ truncation is
not accurate; Eq.~\eqref{eq:tau-leading-balance} is not a behavioural selector,
a support-invariance statement, or a derivation of $c{=}1$.

\paragraph{Finite tested receipt.}
...
These finite points support a fallible zero-search operating prior relative to
the tested Geo baseline.  They do not establish a continuous basin, basin
bounds, interpolation guarantees, near-optimality, or a global optimum.
...
\end{equation}
This expansion describes a smooth quantization objective. Its powers of $K$
do not imply that the benefit of EVQ in a trained model increases as rotary
width decreases. Anchoring leaves no interior freedom at $K=2$, when the
geometric and EVQ grids coincide. The MLA result at one fixed $K=16$ also
cannot test a budget interaction. The finite-distance bounds in
App.~\ref{sec:budget-proofs} and the registered factorial experiment keep
these mathematical and empirical questions separate.
