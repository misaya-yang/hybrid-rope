...
\end{enumerate}
This statement is a consequence of Theorem~\ref{thm:obstruction}, not an empirical generalisation. By Theorem~\ref{thm:obstruction} the schedule's equivalence relation is generated by sign and permutation, and a permutation is realised at the operator level by the corresponding block permutation matrix---so permutation-invariant summaries are exactly the ones blind to it. The equivalence is not, however, available to a fixed checkpoint: applying it requires permuting the learned projection columns, which is why (i) is measured as a large risk gap rather than a null one. The two classes in (ii) and (iii) cover the remaining schedule-only summaries that recur in the literature: orbit counts, which are not permutation-invariant and therefore break under float jitter, and coordinate distances, which are permutation-sensitive but curvature-blind. We do not claim the result for every schedule-dependent $\Phi$, since $\Phi(\Omega)=\Omega$ is injective.
\end{theorem}
