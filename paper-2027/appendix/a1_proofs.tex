...
up to sign and permutation; the remaining freedom is a change of basis inside
each two-dimensional block. For integer positions the same argument applies
modulo the $2\pi$ alias.
\end{proof}
...
This subsection proves Theorem~\ref{thm:ode} directly from the collision-only surrogate functional; the forced Fisher extension is treated separately in~\S\ref{subsec:fisher-ext}. Starting from the formal surrogate kernel
\begin{equation}
...
\textbf{(2) Conditional CDF control.} Under the slowly varying Fisher-coefficient assumption in Appendix~\ref{sec:pure-tether-bound}, the forcing profile is confined to a boundary layer: $\|q_s\|_1=(1-e^{-s})/s=O(1/\log b)$. This yields an $O(1/\log b)$ $L^1$/CDF residual, not a pointwise bound. Inverse-CDF inversion can further amplify that residual by $1/\inf\rho_\tau\sim\sinh\tau/\tau$. Thus the pure-tether branch is adopted for its closed form; the residual analysis states when that choice is controlled and does not identify a trained-loss optimum.
...
A stationary-phase treatment of the exact kernel gives a $\phi$-dependent diagonal coefficient rather than the constant coefficient used in $\mathcal{C}_{\mathrm{app}}$. With $\lambda_b{:=}\ln b$, its leading form is $\alpha_{\mathrm{sp}}(\phi){=}(\pi D_0/L\lambda_b)b^\phi$. Writing the finite-grid coefficient as $\alpha_K{:=}1/(2K)$, the discrepancy is
\begin{equation}
...
\end{equation}
which is not a higher-order correction at the deployed finite grids. Replacing $\alpha$ by $\alpha_{\mathrm{sp}}(\phi)$ produces a different variable-coefficient Euler--Lagrange equation. We use constant $\alpha$ because it is the finite-grid surrogate that yields a positive density with an elementary inverse CDF; this is a modelling decision, not a continuum asymptotic claim. Its empirical check is therefore functional: whether the resulting schedule also reduces the separately evaluated exact-kernel collision score.
...
surrogate functionally rather than pointwise against the oscillatory exact kernel
...
Table~\ref{tab:surrogate-validation} answers affirmatively. For a uniform distance prior $D(\Delta) = 1/L$ on $[0, L]$, we compute the exact kernel $K$ at the discrete allocation points for both geometric and EVQ, then evaluate the normalized collision score $C = \sum_{i<j} K_{ij}^2/(K_{ii}K_{jj})$ and effective spectral rank (exponential of the Shannon entropy of the eigenvalue distribution). These diagnostics use the inclusive grid $u_k=k/(K-1)$; they test the allocation direction but are not a direct validation of the deployed midpoint grid. The $12$ reported configurations comprise a six-length text sweep at fixed $b{=}500$K and $K{=}32$, two additional text bases at $L{=}2048$, and four video bases at $L{=}32$ and $K{=}16$; this is not a Cartesian sweep over every listed factor.

\begin{table}[ht]
\caption{Functional surrogate check on the inclusive $u_k{=}k/(K{-}1)$ diagnostic grid. EVQ reduces collision and increases effective rank under the \emph{exact} kernel $K$ in all 12 tested configurations. This is a direction check, not a pointwise approximation or a deployed-grid identity test.}
...
EVQ reduces collision under the exact kernel in all 12 configurations ($24\%$--$92\%$), and effective rank increases in every row. In the fixed-base text sweep, the relative reduction grows as $L$ shortens. The separate base sweeps remain positive but are not monotone in $b$.
...
For the EVQ-Cosh mapping with $K = d_{\mathrm{rot}}/2$ channels, as
$N/K\to0$ the $N$-channel mid-displacement threshold satisfies
\begin{equation}
\tau_{\mathrm{floor}}(N, K) \;=\; 4\sqrt{\frac{N}{K}}\left[1 + \frac{N}{2K} + \frac{241}{120}\left(\frac{N}{K}\right)^{\!2} + O\!\left((N/K)^3\right)\right].
\end{equation}
...
\paragraph{Accuracy.} For $N{=}1$, against the exact floor obtained by solving $|\Delta\varphi(1/2,\tau)|=1/K$, the leading term has relative error $3.85\%$ at $K{=}16$ and $0.10\%$ at $K{=}512$; including both displayed correction terms reduces the error to $0.088\%$ at $K{=}16$, $1.56{\times}10^{-4}\%$ at $K{=}128$, and $2.41{\times}10^{-6}\%$ at $K{=}512$. All constants come from the cosh geometry.
...
(\S\ref{sec:exp-mla}) is a test of the predicted direction, not a controlled
