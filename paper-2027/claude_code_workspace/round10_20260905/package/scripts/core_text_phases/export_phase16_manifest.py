...
def _validate_result_matches_spec(
    spec: dict[str, object], result: dict[str, object], run_id: str
) -> None:
    for field in RESULT_IDENTITY_FIELDS:
        if field not in result:
            raise ValueError(f"{field} missing from surviving result: {run_id}")
        if result[field] != spec[field]:
            raise ValueError(
                f"{field} mismatch in surviving result {run_id}: "
                f"plan={spec[field]!r}, result={result[field]!r}"
            )
...
def load_rows(source: Path) -> list[dict[str, object]]:
    ...
    for plan_name in ("pilot_plan.json", "confirm_plan.json"):
        ...
        for spec in plan:
            ...
            _validate_result_matches_spec(spec, result, run_id)
            config_id = (
