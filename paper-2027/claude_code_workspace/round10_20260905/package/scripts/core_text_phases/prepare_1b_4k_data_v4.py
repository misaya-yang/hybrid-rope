#!/usr/bin/env python3
"""Prepare 1B tokens at seq_len=4096 — v4, FineWeb (non-Edu).

Source: HuggingFaceFW/fineweb (sample-10BT config)
  - State-of-the-art quality filtered CommonCrawl
  - Completely different from Pile/OWT (v1/v2/v3)
  - Long document bias: min_length=200 chars (stricter than v1-v3)
  - ~10B tokens available, sampling ~10%
...
OUT_DIR = Path("/root/autodl-tmp/data/1b_diverse_4k_v4")
SHUFFLE_SEED = 42
MIN_CHARS = 200  # stricter than v1-v3 (50), ensures long docs
...
def stream_tokenize(hf_name, config, text_key, tokenizer, max_tokens, name=""):
    ...
    try:
        for example in ds:
            ...
            if len(ids) < 64:  # skip very short tokenized docs
                continue
            all_ids.extend(ids)
            total = len(all_ids)
            n_docs += 1
...
def main():
    ...
    print(f"Source: FineWeb sample-10BT (non-Edu), seed={SHUFFLE_SEED}, min_chars={MIN_CHARS}")
    print(f"Purpose: v4 for 454M continued pretraining, non-overlapping with v1/v2/v3")
