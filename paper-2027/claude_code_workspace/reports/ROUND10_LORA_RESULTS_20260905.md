# §10 LoRA 固定比较轮次执行报告（2026-09-05，历史）

> **判读纠正（2026-09-08）：** 以下原始数字保留，但原“长输入 exposure 不必要、
> 强烈支持机理 A、LoRA只修接口”的推论撤回。Native `N_compact` 从未运行；
> `Z_compact` 是另一个干预。20/32与23/32的不拒绝检验、Z/Y区间重叠均不证明
> 等效或唯一机制，且16K低于Qwen的Native32K。可保留的观察是：此轮Z短输入
> 适配后在已见合同的16K上获得20/32，不能据此确定换表、内容学习或格式学习
> 的贡献。Y2也不是忠实YaRN，其smoothstep/sqrt增益差异见
> [固定实现核对](../../research/CROSS_AUDIT_EXPERIMENT_PROTOCOL_20260907.md)。
> §4.3、§6与§7中仍保留的原推理仅供复盘，不再作为后续方法或实验的依据。

> **状态（2026-09-06 更新）**：本轮为 Qwen-2.5-1.5B 时代轮次，已被项目重置与
> Round 12 取代（成功标准改为各模型自己的 2×/4×；当时把Y2误称忠实YaRN）。
...
- 执行状态：Z / Y / Z_compact三案完成，Native N_compact未执行。三案的点值
  结果保留；原E3a机制判读已按页首纠正撤回。
...
### 6.2 原预注册判读（2026-09-08已撤回，保留错误推理供复盘）

预注册不会使错误检验有效：下述单样本二项计算把估计的23/32当作已知常数，
还把不拒绝当作等效。正观察不能同时承担“exposure无必要”和“谱预算是唯一
约束”的归因。没有预设等效界的配对证据，也没有所需Native compact控制。
...
原报告曾将E3a解释为“长输入 exposure 不必要”；该归因现已撤回，Qwen窗内
20/32观察本身保留。同配方换到
...
| （E3a 预注册）compact 训练后 far 是否保持 | **far 20/32，≥19 阈值，P(X≤20\|p₀)=0.162 不拒绝** | 原机制判读已撤回；保留观察，不拒绝不等于等效（§6.2） |
