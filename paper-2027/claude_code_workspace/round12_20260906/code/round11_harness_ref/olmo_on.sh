#!/bin/bash
# OLMO_ON: pre-registered secondary exploration (PREGLUCTION_OLMO_Z1 section 6):
# arm N full-layout exposure (native table, LoRA trained on long inputs, no table swap).
# Purpose: separate "table is necessary" from "long-input training alone suffices"
# at 4x extrapolation, cross-family. Compare far strict: ON vs ZF vs T0.
set -u
...
# arm N: no --table, gain default 1.0 (engine: "N arm must deploy original Native table/gain")
...
stage train 7200 $PY $ENGINE train $COMMON --max-seconds 7200 --arm N --placement all_linear --kl-budget .02 --teacher-cache /root/ffn_review_scratch_20260904/teacher_cache_olmo --stop-after-step 128 --output $OUT/train
$PY - <<EOF
import json
