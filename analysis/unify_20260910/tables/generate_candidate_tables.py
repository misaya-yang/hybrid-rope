...
chk("Stack_m28_vs_doc", tstack["met"]["m28"], 0.065, 5e-4, "BUDGET:45 文档值 .065 ✓（精确 0.065359）")
...
IDENTITY = {
 ...
 "ramp_N16": ("**= 队列 0448 MrProN16**（BUDGET:46 冻结定义 q(q+1)/272；GR §6 修正值 max洞 1.461@g38、B=6.000）。G1 公式重建吻合（max|Δm|≈3.3e-8）。**未执行，无分数**。预注册预测：D2 泛函 R=51.15 > MrPro 47.44 ⇒ 预测劣于 MrPro [假设-预注册]；D4 平衡权重 s=.5 下 argmin N′*=16 ⇒ 预测全场最优 [部分证据-边界内决策]——两判据相反，0448 正是判决实验。", "队列 0448（GPU 回来即跑）"),
 "ramp_N15": ("**= 队列 0449 MrProN15**（BUDGET:47，q(q+1)/240；GR §6 max洞 1.476@g37、B=6.667；D4 表中 F_arc=.3667）。G1 重建吻合（≈3.6e-8）。**未执行，无分数**。D2 预测 R=51.51 劣于 MrPro；D4 次选 N′*=15（s∈[.24,.41]）[假设-预注册]。", "队列 0449"),
...
PRED = {  # 只引已预注册的结构预测，不虚构分数
 "ramp_N16": ["D2 泛函 R=51.15（预测劣于 MrPro 78.13）", "D4 argmin N′*=16（预测全场最优）", "预注册判据：长端 78.13<N16≤N15 单调 ⇒ ramp 单参数族成立；N16>max(N15,78.13) ⇒ D4 命中锁定 N′*=16；N16≤78.13 非单调 ⇒ 天花板项活跃，回摆 N′=17 或 16+notch"],
 "ramp_N15": ["D2 泛函 R=51.51（预测劣于 MrPro）", "D4 次选（s∈[.24,.41]）", "判据：N15>N16 ⇒ 沿族下探 14/13 但必须带 mid 缺口"],
 "stack_0446": ["D2 泛函 R=46.55（预测优于全部 5 张已测表，含 s28 46.67）", "D4 判据：≥83%@128K 且 ≥80%@32K ⇒ 双机制可加 ⇒ 主交付改 ramp16⊕notch；若不敌 max(s28,LBS)=83.33 ⇒ notch 与完成不可同预算"],
}
...
A("")
A("任务 (1) 称\"均分桥 $N'\\in\\{13..17\\}$、$N'=17\\approx$ MrPro 恒等检查\"。地面真值的两个锚点唯一钉死其指**径向族**（MrRoPE Eq.14 形，$m_q=q(q+1)/(N'(N'+1))$）：MrPro 部署 = $N'=17$ 径向族 bit-exact [已验证]；0448/0449 冻结定义 = \"MrPro **径向族** $N'=16/15$\" [BUDGET:46-47]。字面均分桥（Eq.13 形，$m_j=\\min(1,(j-23)/N')$）的 $N'=17$ 成员是**已测败点 MrUni**（64.5833/73.3333 [已验证]），非 MrPro。故本文**主交付 = 径向族五表**；均分族五表全部附带（§4 参考节 + CSV/JSON 全量），其 $N'=17$ 表即 MrUni 身份检查。")
A("")
A("### 0.2 勘误与新发现（本轮复算）")
A("")
...
A("2. BUDGET:45-47 的 max 洞 1.86/1.76/1.80× 维持作废，以 GR §6 重算值为准（本生成器逐表复算 MATCH，见 §5.2）。")
...
for t in TABLES:
    A(met_row(t))
A("")
A("径向族族梯度（D4 §5 主搬运）复核 [已验证=代数]：$17\\to16$ 从末 gap g39 抽 $0.111111$ m 单位（$0.15399$ nats，$\\ln4$ 的 11.11%）摊回 g23–38（各 $\\times1.125$），槽 39 完成（$D_{39}\\to131072$）、$F_{\\rm arc}\\ 1.046\\to0.6765$，代价最大洞 $1.448@g39\\to1.461@g38$ 左移；$16\\to15$ 再抽 g38 的 $0.117647$（$0.16306$ nats）×$1.1333$ 摊回，槽 38 完成，$F_{\\rm arc}\\to0.3667$。搬运全程 bank/尾带零触碰（I1/I2 保持，校验节逐表 MATCH）。")
A("")
...
out = {
  "meta": {
    ...
    "errata": [
      ...
      "BUDGET:45-47 max洞 1.86/1.76/1.80× 作废，用 GR §6 重算值 1.4930/1.4608/1.4757",
