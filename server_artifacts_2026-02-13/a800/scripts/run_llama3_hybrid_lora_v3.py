...
def load_long_tokens(tokenizer, target_tokens, seq_len):
    ...
    for ds_name, cfg, split, text_key in candidates:
        try:
            ...
            print(f"[Data] source={ds_name} cfg={cfg} chunks={n} usable_tokens={usable}")
...
def main():
    ...
    # Note: this box has transformers 5.1.0 (sig differs from older releases).
    # Avoid kwargs that are not accepted here.
