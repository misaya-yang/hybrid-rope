...
WORK_DIR = "/opt/dfrope/results/50m_theta_factorial"
...
def train_model(model: GPT, data: torch.Tensor, seed: int) -> dict:
    _set_seed(seed)
    model.train()
...
def run_one(name: str, inv_freq: torch.Tensor, train_data: torch.Tensor, val_data: torch.Tensor, seed: int) -> dict:
    print(f"\n{'='*78}\n  {name} | seed={seed}\n  freq range: [{inv_freq.min().item():.2e}, {inv_freq.max().item():.4f}]\n{'='*78}")
    _set_seed(seed)
    model = GPT(MODEL_CFG, inv_freq).to(DEVICE)
    train_stats = train_model(model, train_data, seed)
...
def aggregate(results: dict) -> dict:
    summary = {}
    for cfg, seed_map in results["experiments"].items():
        by_len = {}
        for _, payload in seed_map.items():
            for L, ppl in payload.get("eval", {}).items():
                by_len.setdefault(L, []).append(float(ppl))
        summary[cfg] = {}
        for L, vals in by_len.items():
            m = sum(vals) / len(vals)
            s = (sum((v - m) ** 2 for v in vals) / len(vals)) ** 0.5
            summary[cfg][L] = {"mean": round(m, 3), "std": round(s, 3), "n": len(vals)}
    return summary


def print_tables(results: dict) -> None:
    print("\n" + "=" * 78)
    print("  PER-SEED TABLE (PPL@2048, PPL@16384)")
    print("=" * 78)
    print(f"{'Config':<26} | {'Seed':<8} | {'PPL@2048':<10} | {'PPL@16384':<10}")
    print("-" * 78)
    for cfg, seed_map in results["experiments"].items():
        for seed_name, payload in seed_map.items():
            e = payload.get("eval", {})
            print(f"{cfg:<26} | {seed_name:<8} | {str(e.get('2048','N/A')):<10} | {str(e.get('16384','N/A')):<10}")

    print("\n" + "=" * 78)
    print("  AGGREGATE TABLE (mean±std)")
    print("=" * 78)
    print(f"{'Config':<26} | {'PPL@2048':<20} | {'PPL@16384':<20}")
    print("-" * 78)
    for cfg, sm in results["summary"].items():
        a = sm.get("2048", {})
        b = sm.get("16384", {})
        a_txt = f"{a.get('mean','N/A')} ± {a.get('std','N/A')}"
        b_txt = f"{b.get('mean','N/A')} ± {b.get('std','N/A')}"
        print(f"{cfg:<26} | {a_txt:<20} | {b_txt:<20}")
...
def main() -> None:
    ...
    print("=" * 78)
    print("  50M FACTORIAL: theta x shape FAIRNESS SWEEP")
    print("  configs: geo_100k/200k/300k/500k + hybrid_t100k/t500k")
    print(f"  seeds: {SEEDS}")
    print("=" * 78)
    ...
    geo_200k = geometric_freq(K, 200000)
    geo_300k = geometric_freq(K, 300000)
    geo_500k = geometric_freq(K, 500000)

    hybrid_100k = hybrid_freq(geo_100k, anchored_poly_freq(K, 100000, p=3.9, omf=0.3), alpha=0.2)
    hybrid_500k = hybrid_freq(geo_500k, anchored_poly_freq(K, 500000, p=3.9, omf=0.3), alpha=0.2)
    ...
    for seed in SEEDS:
        for name, inv in experiments:
            key = f"seed{seed}"
            results["experiments"][name][key] = run_one(name, inv, train_data, val_data, seed)
